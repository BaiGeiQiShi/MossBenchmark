...
cp -r ${CURRDIR}/input_dir ${CURRDIR}/tmp/input_dir
cp -r ${CURRDIR}/tests ${CURRDIR}/tmp/tests
cp -r ${CURRDIR}/input ${CURRDIR}/tmp/input
cp -r ${CURRDIR}/input2 ${CURRDIR}/tmp/input2
cp -r $CURRDIR/input.origin/* $CURRDIR/tmp/

clang -fprofile-instr-generate -fcoverage-mapping -w -o $PROGRAM $PROGRAM.c "-D__msan_unpoison(s,z)" -lpcre
