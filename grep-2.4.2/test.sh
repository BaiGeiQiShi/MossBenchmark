...
if [ ! -f ${BASE_BIN} ]; then
   cp ${BASE_SRC} $SRC
   ./compile.sh $SRC $BASE_BIN "-w -O3 -D__msan_unpoison(s,z) -lpcre"
fi
#Generate Oracle Bin (if needed)
if [ ! -f ${ORIGIN_BIN} ]; then
    cp ${ORIGIN_SRC} $SRC
    ./compile.sh ${SRC} ${ORIGIN_BIN} "-w -O3 -D__msan_unpoison(s,z) -lpcre"
fi
#Generate Oracle Outputs (if needed)
if [ ! -d ${ORIGIN_OUTDIR} ]; then
    mkdir ${ORIGIN_OUTDIR}
    cp ${ORIGIN_SRC} $SRC
    ./compile.sh ${SRC} ${BIN} "-w -O3 -D__msan_unpoison(s,z) -lpcre"                    #NOTE: Do NOT use ORIGIN_BIN here!
