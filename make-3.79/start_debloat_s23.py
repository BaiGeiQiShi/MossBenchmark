...
def COVBLOAT(_rid):
    try:
        ...
        subprocess.run(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -M Cov_info.txt -m {debop_samplenum} -i {iternum} -t moss-out.{_rid} -a {alpha} -e {beta} -k {k} -s ./test.sh {PROGNAME}.c > log/{_rid}.txt",shell=True)
    except subprocess.CalledProcessError as e:
        if(e.returncode==137):pass
        else:raise e
    subprocess.run(["/usr/local/bin/getLog.py",f"{CURRDIR}/log/{_rid}.txt", f"{CURRDIR}/log/stat.{_rid}.txt"])
