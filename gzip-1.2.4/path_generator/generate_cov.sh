...
cp -r ${CURRDIR}/references ${CURRDIR}/tmp/references
...
do
    test_name=${testf##*/} 
    cd $CURRDIR/tmp/WorkingDirectory
    timeout 1s ${testf} $CURRDIR/tmp/$PROGRAM ${OUTDIR} 1 ${CURRDIR}/tmp
    mv $CURRDIR/tmp/WorkingDirectory/default.profraw $CURRDIR/tmp/default.profraw
    cd $CURRDIR/tmp && rm -rf $CURRDIR/tmp/WorkingDirectory/*
    $CURRDIR/path_generator/get_bin_gcov_file.sh $CURRDIR/tmp/$PROGRAM $COV	
