 ...
 * ----------
 */
#define pglz_hist_idx(_s, _e, _mask) (((((_e) - (_s)) < 4) ? (int)(_s)[0] : (((_s)[0] << 6) ^ ((_s)[1] << 4) ^ ((_s)[2] << 2) ^ (_s)[3])) & (_mask))
 ...
 */
static inline int
pglz_find_match(int16 *hstart, const char *input, const char *end, int *lenp, int *offp, int good_match, int good_drop, int mask)
{
  PGLZ_HistEntry *hent;
  int16 hentno;
  int32 len = 0;
 ...
 */
int32
pglz_compress(const char *source, int32 slen, char *dest, const PGLZ_Strategy *strategy)
{
  ...
  while (dp < dend)
  {
     ...
     */
    if (pglz_find_match(hist_start, dp, dend, &match_len, &match_off, good_match, good_drop, mask))
    {
      /*
      ...
      while (match_len--)
      {
        pglz_hist_add(hist_start, hist_entries, hist_next, hist_recycle, dp, dend, mask);
 ...
 */
int32
pglz_decompress(const char *source, int32 slen, char *dest, int32 rawsize, bool check_complete)
{
