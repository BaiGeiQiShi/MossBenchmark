...
{
  struct ECPGstruct_member *ptr, *ne = (struct ECPGstruct_member *)mm_alloc(sizeof(struct ECPGstruct_member));
...
/* Dump a type.
   ...
   size <comma>					   - long size of this field (if
   varchar) arrsize <comma>				   - long number of
   elements in the arr offset <comma>				   - offset to
   the next element Where: type-tag is one of the simple types or varchar.
...
void
ECPGdump_a_type(FILE *o, const char *name, struct ECPGtype *type, const int brace_level, const char *ind_name, struct ECPGtype *ind_type, const int ind_brace_level, const char *prefix, const char *ind_prefix, char *arr_str_size, const char *struct_sizeof, const char *ind_struct_sizeof)
{
  struct variable *var;

  if (type->type != ECPGt_descriptor && type->type != ECPGt_sqlda && type->type != ECPGt_char_variable && type->type != ECPGt_const && brace_level >= 0) {
    char *str;
    ...
    if ((var->type->type != type->type) || (var->type->type_name && !type->type_name) || (!var->type->type_name && type->type_name) || (var->type->type_name && type->type_name && strcmp(var->type->type_name, type->type_name) != 0)) {
    ...
    if (ind_name && ind_type && ind_type->type != ECPGt_NO_INDICATOR && ind_brace_level >= 0) {
      ...
      if ((var->type->type != ind_type->type) || (var->type->type_name && !ind_type->type_name) || (!var->type->type_name && ind_type->type_name) || (var->type->type_name && ind_type->type_name && strcmp(var->type->type_name, ind_type->type_name) != 0)) {
  ...
  case ECPGt_array:
    ...
    case ECPGt_array:
      mmerror(PARSE_ERROR, ET_ERROR, "nested arrays are not supported (except strings)"); /* array of
                                                                                             array */
      break;
    case ECPGt_struct:
    case ECPGt_union:
      ECPGdump_a_struct(o, name, ind_name, type->size, type->u.element, (ind_type == NULL) ? NULL : ((ind_type->type == ECPGt_NO_INDICATOR) ? ind_type : ind_type->u.element), prefix, ind_prefix);
      break;
    default:;
      ...
      ECPGdump_a_simple(o, name, type->u.element->type, type->u.element->size, type->size, struct_sizeof ? struct_sizeof : NULL, prefix, type->u.element->counter);
      ...
      if (ind_type != NULL) {
        if (ind_type->type == ECPGt_NO_INDICATOR) {
          ...
          ECPGdump_a_simple(o, ind_name, ind_type->u.element->type, ind_type->u.element->size, ind_type->size, NULL, ind_prefix, 0);
        }
      }
    }
    break;
...
ECPGdump_a_simple(FILE *o, const char *name, enum ECPGttype type, char *varcharsize, char *arrsize, const char *size, const char *prefix, int counter)
{
  if (type == ECPGt_NO_INDICATOR) {
    ...
    /*
     * Array size would be -1 for addresses of members within structure,* when pointer to structure is being dumped.
     */
...
ECPGdump_a_struct(FILE *o, const char *name, const char *ind_name, char *arrsize, struct ECPGtype *type, struct ECPGtype *ind_type, const char *prefix, const char *ind_prefix)
{
  ...
  for (p = type->u.members; p; p = p->next) {
    ECPGdump_a_type(o, p->name, p->type, -1, (ind_p != NULL) ? ind_p->name : NULL, (ind_p != NULL) ? ind_p->type : NULL, -1, prefix, ind_prefix, arrsize, type->struct_sizeof, (ind_p != NULL) ? ind_type->struct_sizeof : NULL);
