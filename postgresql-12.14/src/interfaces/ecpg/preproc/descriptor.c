...
output_set_descr(char *desc_name, char *index)
{
  ...
  for (results = assignments; results != NULL; results = results->next) {
    ...
    case ECPGd_ret_length:
    case ECPGd_ret_octet:
      mmfatal(PARSE_ERROR, "descriptor item \"%s\" cannot be set", descriptor_item_name(results->value));
      break;
...
descriptor_variable(const char *name, int input)
{
  ...
  static struct variable varspace[2] = {{descriptor_names[0], &descriptor_type, 0, NULL}, {descriptor_names[1], &descriptor_type, 0, NULL}};
