...
static int
dttofmtasc_replace(timestamp *ts, date dDate, int dow, struct tm *tm, char *output, int *pstr_len, const char *fmtstr)
{
  ...
  while (*p) {
    if (*p == '%') {
      ...
      case 'D':
         ...
         *
         * pstr_len is a pointer to the remaining size of output,* i.e. the size of q
         */
