...
main(int argc, char **argv)
{
  ...
  printfPQExpBuffer(&sql, "%s",
      "SELECT c.relname, (SELECT nspname FROM pg_catalog.pg_namespace n WHERE n.oid = c.relnamespace) AS nspname, a.attname FROM pg_catalog.pg_class c, pg_catalog.pg_attribute a WHERE a.attnum > 0 AND c.relkind = " CppAsString2(RELKIND_RELATION) " AND a.attrelid = c.oid AND a.atttypid IN ('pg_catalog.oid'::regtype,  'pg_catalog.regclass'::regtype,  'pg_catalog.regoper'::regtype,  'pg_catalog.regoperator'::regtype,  'pg_catalog.regproc'::regtype,  'pg_catalog.regprocedure'::regtype,  'pg_catalog.regtype'::regtype,  'pg_catalog.regconfig'::regtype,  'pg_catalog.regdictionary'::regtype) ORDER BY nspname, c.relname, a.attnum");
  ...
  for (fk = 0; fk < PQntuples(fkrel_res); fk++) {
    ...
    for (pk = 0; pk < PQntuples(pkrel_res); pk++) {
      ...
      printfPQExpBuffer(&sql,
          "SELECT	1 FROM \"%s\".\"%s\" t1, \"%s\".\"%s\" t2 WHERE t1.\"%s\"::pg_catalog.oid = t2.oid LIMIT 1",
  ...
  for (fk = 0; fk < PQntuples(fkrel_res); fk++) {
    ...
    for (pk = 0; pk < PQntuples(pkrel_res); pk++) {
      ...
      printfPQExpBuffer(&sql,
          "SELECT	1 FROM \"%s\".\"%s\" t1, \"%s\".\"%s\" t2 WHERE t2.oid = ANY(t1.\"%s\")LIMIT 1",
          fk_nspname, fk_relname, pk_nspname, pk_relname, fk_attname);
