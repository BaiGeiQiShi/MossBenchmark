...
static const unicodeStyleFormat unicode_style = {
    ...
    {
        /* └│┌─┐┘ */
        {"\342\224\224", "\342\224\202", "\342\224\214", "\342\224\200",
         "\342\224\220", "\342\224\230"},
        /* ╚║╔═╗╝ */
        {"\342\225\232", "\342\225\221", "\342\225\224", "\342\225\220",
         "\342\225\227", "\342\225\235"},
    },
    " ",
    "\342\206\265", /* ↵ */
    " ",
...
/* Local functions */
static int strlen_max_width(unsigned char *str, int *target_width,
                            int encoding);
...
static printTableFooter *footers_with_default(const printTableContent *cont) {
  if (cont->footers == NULL && cont->opt->default_footer) {
    ...
    snprintf(default_footer, sizeof(default_footer),
             ngettext("(%lu row)", "(%lu rows)", total_records), total_records);
  ...
  for (i = 0; i < ncolumns; i++) {
    ...
    if (i < ncolumns - 1) {
      if (border == 0) {
        ...
        fprintf(fout, "%s%s%s", lformat->hrule, lformat->midvrule,
                lformat->hrule);
      }
    }
  }
...
static void print_aligned_text(const printTableContent *cont, FILE *fout,
  ...
  for (i = 0; i < col_count; i++) {
    ...
    pg_wcssize((const unsigned char *)cont->headers[i],
               strlen(cont->headers[i]), encoding, &width, &nl_lines,
               &bytes_required);
  ...
  for (i = 0, ptr = cont->cells; *ptr; ptr++, i++, cell_count++) {
    ...
    pg_wcssize((const unsigned char *)*ptr, strlen(*ptr), encoding, &width,
  ...
  for (i = 0; i < col_count; i++) {
    ...
    col_lineptrs[i] =
        pg_malloc0((max_nl_lines[i] + 1) * sizeof(**col_lineptrs));
      ...
      while (width_total > output_columns) {
        ...
        for (i = 0; i < col_count; i++) {
          if (width_average[i] && width_wrap[i] > width_header[i]) {
            ...
            ratio =
                (double)width_wrap[i] / width_average[i] + max_width[i] * 0.01;
            if (ratio > max_ratio) {
              max_ratio = ratio;
              worst_col = i;
            }
    ...
    if (!opt_tuples_only) {
      ...
      for (i = 0; i < col_count; i++) {
        pg_wcsformat((const unsigned char *)cont->headers[i],
                     strlen(cont->headers[i]), encoding, col_lineptrs[i],
                     max_nl_lines[i]);
      }
      ...
      while (more_col_wrapping) {
        ...
        for (i = 0; i < cont->ncolumns; i++) {
          ...
          if (!header_done[i]) {
            ...
            /* centered */
            fprintf(fout, "%-*s%s%-*s", nbspace / 2, "", this_line->ptr,
                    (nbspace + 1) / 2, "");
  ...
  for (i = 0, ptr = cont->cells; *ptr; i += col_count, ptr += col_count) {
    ...
    do {
      ...
      for (j = 0; j < col_count; j++) {
        ...
        bool finalspaces =
            (opt_border == 2 || (col_count > 0 && j < col_count - 1));
        ...
        if (!this_line->ptr) {
          ...
          bytes_to_output = strlen_max_width(this_line->ptr + bytes_output[j],
...
static void print_aligned_vertical_line(const printTextFormat *format,
                                        ...
                                        unsigned int hwidth,
                                        unsigned int dwidth, printTextRule pos,
                                        FILE *fout) {
  const printTextLineFormat *lformat = &format->lrule[pos];
  unsigned int i;
  int reclen = 0;
...
static void print_aligned_vertical(const printTableContent *cont, FILE *fout,
  ...
  const char *const *ptr;
  unsigned int i, hwidth = 0, dwidth = 0, hheight = 1, dheight = 1,
  ...
  for (i = 0; i < cont->ncolumns; i++) {
    ...
    pg_wcssize((const unsigned char *)cont->headers[i],
               strlen(cont->headers[i]), encoding, &width, &height, &fs);
    if (width > hwidth) {
      hwidth = width;
    }
    if (height > hheight) {
  ...
  if (cont->opt->format == PRINT_WRAPPED) {
    ...
    for (;;) {
       ...
       */
      if (newdwidth < dwidth && !dmultiline && opt_border < 2 &&
  ...
  for (i = 0, ptr = cont->cells; *ptr; i++, ptr++) {
    ...
    if (i % cont->ncolumns == 0) {
      ...
      if (!opt_tuples_only) {
        print_aligned_vertical_line(format, opt_border, record++, lhwidth,
                                    dwidth, pos, fout);
      } else if (i != 0 || !cont->opt->start_table || opt_border == 2) {
        print_aligned_vertical_line(format, opt_border, 0, lhwidth, dwidth, pos,
                                    fout);
      }
    }
    ...
    pg_wcsformat((const unsigned char *)cont->headers[i % cont->ncolumns],
                 strlen(cont->headers[i % cont->ncolumns]), encoding, hlineptr,
                 hheight);
    /* Format the data */
    pg_wcsformat((const unsigned char *)*ptr, strlen(*ptr), encoding, dlineptr,
                 dheight);
...
static void print_html_text(const printTableContent *cont, FILE *fout) {
  ...
  for (i = 0, ptr = cont->cells; *ptr; i++, ptr++) {
    ...
    fprintf(fout, "    <td align=\"%s\">",
            cont->aligns[(i) % cont->ncolumns] == 'r' ? "right" : "left");
...
static void print_asciidoc_text(const printTableContent *cont, FILE *fout) {
  ...
  if (cont->opt->start_table) {
    ...
    for (i = 0; i < cont->ncolumns; i++) {
      ...
      fprintf(fout, "%s",
              cont->aligns[(i) % cont->ncolumns] == 'r' ? ">l" : "<l");
    }
    fputs("\"", fout);
    switch (opt_border) {
    case 0:
...
#define LONGTABLE_WHITESPACE " \t\n"
        ...
        next_opt_table_attr_char +=
            strspn(next_opt_table_attr_char, LONGTABLE_WHITESPACE);
        /* We have a value? */
        if (next_opt_table_attr_char[0] != '\0') {
          fputs("p{", fout);
          fwrite(next_opt_table_attr_char,
                 strcspn(next_opt_table_attr_char, LONGTABLE_WHITESPACE), 1,
                 fout);
          last_opt_table_attr_char = next_opt_table_attr_char;
          next_opt_table_attr_char +=
              strcspn(next_opt_table_attr_char, LONGTABLE_WHITESPACE);
          fputs("\\textwidth}", fout);
        }
        /* use previous value */
        else if (last_opt_table_attr_char != NULL) {
          fputs("p{", fout);
          fwrite(last_opt_table_attr_char,
                 strcspn(last_opt_table_attr_char, LONGTABLE_WHITESPACE), 1,
                 fout);
          fputs("\\textwidth}", fout);
        } else {
          fputc('l', fout);
 ...
 */
void set_sigpipe_trap_state(bool ignore) { always_ignore_sigpipe = ignore; }
...
FILE *PageOutput(int lines, const printTableOpt *topt) {
  ...
  if (topt && topt->pager && isatty(fileno(stdin)) && isatty(fileno(stdout))) {
    ...
    if (result == -1 || (lines >= screen_size.ws_row && lines >= min_lines) ||
        pager > 1)
#endif
    {
...
void printTableAddHeader(printTableContent *const content, char *header,
  ...
  *content->header =
      (char *)mbvalidate((unsigned char *)header, content->opt->encoding);
#ifdef ENABLE_NLS
  if (translate) {
 ...
 */
void printTableAddCell(printTableContent *const content, char *cell,
  ...
  *content->cell =
      (char *)mbvalidate((unsigned char *)cell, content->opt->encoding);
 ...
 */
void printTable(const printTableContent *cont, FILE *fout, bool is_pager,
                FILE *flog) {
  bool is_local_pager = false;
  ...
  if (!is_pager && cont->opt->format != PRINT_ALIGNED &&
...
void printQuery(const PGresult *result, const printQueryOpt *opt, FILE *fout,
  ...
  printTableInit(&cont, &opt->topt, opt->title, PQnfields(result),
                 PQntuples(result));
  ...
  for (i = 0; i < cont.ncolumns; i++) {
    printTableAddHeader(&cont, PQfname(result, i), opt->translate_header,
...
void refresh_utf8format(const printTableOpt *opt) {
  ...
  popt->lrule[PRINT_RULE_TOP].midvrule =
      column->down_and_horizontal[opt->unicode_border_linestyle];
  popt->lrule[PRINT_RULE_TOP].rightvrule = border->down_and_left;

  popt->lrule[PRINT_RULE_MIDDLE].hrule = header->horizontal;
  popt->lrule[PRINT_RULE_MIDDLE].leftvrule =
      header->vertical_and_right[opt->unicode_border_linestyle];
  popt->lrule[PRINT_RULE_MIDDLE].midvrule =
      column->vertical_and_horizontal[opt->unicode_header_linestyle];
  popt->lrule[PRINT_RULE_MIDDLE].rightvrule =
      header->vertical_and_left[opt->unicode_border_linestyle];

  popt->lrule[PRINT_RULE_BOTTOM].hrule = border->horizontal;
  popt->lrule[PRINT_RULE_BOTTOM].leftvrule = border->up_and_right;
  popt->lrule[PRINT_RULE_BOTTOM].midvrule =
      column->up_and_horizontal[opt->unicode_border_linestyle];
