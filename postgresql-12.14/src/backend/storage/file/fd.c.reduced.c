...
static void
RemovePgTempFilesInDir(const char *tmpdirname, bool missing_ok,
                       bool unlink_all);
static void
  ...
  if (rename(oldfile, newfile) < 0) {
    ereport(elevel, (errcode_for_file_access(),
                     ...
                     (AmCheckpointerProcess()
                          ? errhint("This is known to fail occasionally during "
                                    "archive recovery, where it is harmless.")
                          : 0)));
    return -1;
  }
#endif
...
LruInsert(File file)
{
  ...
  if (FileIsNotOpen(file)) {
     ...
     */
    vfdP->fd =
        BasicOpenFilePerm(vfdP->fileName, vfdP->fileFlags, vfdP->fileMode);
    if (vfdP->fd < 0) {
...
FreeVfd(File file)
{
  ...
  DO_DB(elog(LOG, "FreeVfd: %d (%s)", file,
...
PathNameOpenFilePerm(const char *fileName, int fileFlags, mode_t fileMode)
{
  ...
  DO_DB(elog(LOG, "PathNameOpenFilePerm: %s %x %o", fileName, fileFlags,
             fileMode));
...
FileWriteback(File file, off_t offset, off_t nbytes, uint32 wait_event_info)
{
  ...
  DO_DB(elog(LOG, "FileWriteback: %d (%s) " INT64_FORMAT " " INT64_FORMAT, file,
             VfdCache[file].fileName, (int64)offset, (int64)nbytes));
         ...
         uint32 wait_event_info)
{
  ...
  DO_DB(elog(LOG, "FileRead: %d (%s) " INT64_FORMAT " %d %p", file,
             VfdCache[file].fileName, (int64)offset, amount, buffer));
          ...
          uint32 wait_event_info)
{
  ...
  DO_DB(elog(LOG, "FileWrite: %d (%s) " INT64_FORMAT " %d %p", file,
             VfdCache[file].fileName, (int64)offset, amount, buffer));
...
OpenTransientFilePerm(const char *fileName, int fileFlags, mode_t fileMode)
{
  ...
  DO_DB(elog(LOG, "OpenTransientFile: Allocated %d (%s)", numAllocatedDescs,
             fileName));
...
OpenPipeStream(const char *command, const char *mode)
{
  ...
  DO_DB(elog(LOG, "OpenPipeStream: Allocated %d (%s)", numAllocatedDescs,
             command));
...
CloseTransientFile(int fd)
{
  ...
  /* Only get here if someone passes us a file not in allocatedDescs */
  elog(WARNING, "fd passed to CloseTransientFile was not obtained from "
...
AllocateDir(const char *dirname)
{
  ...
  DO_DB(
      elog(LOG, "AllocateDir: Allocated %d (%s)", numAllocatedDescs, dirname));
...
RemovePgTempRelationFiles(const char *tsdirname)
{
  ...
  while ((de = ReadDirExtended(ts_dir, tsdirname, LOG)) != NULL) {
    ...
    snprintf(dbspace_path, sizeof(dbspace_path), "%s/%s", tsdirname,
             de->d_name);
