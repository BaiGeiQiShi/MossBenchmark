...
ExecInitTableFunctionResult(Expr *expr, ExprContext *econtext, PlanState *parent)
{
  ...
  if (IsA(expr, FuncExpr))
  {
    ...
    init_sexpr(func->funcid, func->inputcollid, expr, state, parent, econtext->ecxt_per_query_memory, func->funcretset, false);
  }
  else
  {
...
ExecMakeTableFunctionResult(SetExprState *setexpr, ExprContext *econtext, MemoryContext argContext, TupleDesc expectedDesc, bool randomAccess)
{
  ...
  if (!setexpr->elidedFuncState)
  {
    ...
    InitFunctionCallInfoData(*fcinfo, &(setexpr->func), list_length(setexpr->args), setexpr->fcinfo->fncollation, NULL, (Node *)&rsinfo);
  ...
  for (;;)
  {
    ...
    {
      ereport(ERROR, (errcode(ERRCODE_E_R_I_E_SRF_PROTOCOL_VIOLATED), errmsg("unrecognized table-function returnMode: %d", (int)rsinfo.returnMode)));
    }
...
ExecInitFunctionResultSet(Expr *expr, ExprContext *econtext, PlanState *parent)
{
  ...
  if (IsA(expr, FuncExpr))
  {
    ...
    state->args = ExecInitExprList(func->args, parent);
    init_sexpr(func->funcid, func->inputcollid, expr, state, parent, econtext->ecxt_per_query_memory, true, true);
  }
  else if (IsA(expr, OpExpr))
  {
    ...
    init_sexpr(op->opfuncid, op->inputcollid, expr, state, parent, econtext->ecxt_per_query_memory, true, true);
  }
  else
  {
  ...
  if (rsinfo.returnMode == SFRM_ValuePerCall)
  {
    if (*isDone != ExprEndResult)
    {
      ...
      if (*isDone == ExprMultipleResult)
      {
        ...
        {
          RegisterExprContextCallback(econtext, ShutdownSetExpr, PointerGetDatum(fcache));
...
init_sexpr(Oid foid, Oid input_collation, Expr *node, SetExprState *sexpr, PlanState *parent, MemoryContext sexprCxt, bool allowSRF, bool needDescForSRF)
{
  ...
  InitFunctionCallInfoData(*sexpr->fcinfo, &(sexpr->func), numargs, input_collation, NULL, NULL);
...
tupledesc_match(TupleDesc dst_tupdesc, TupleDesc src_tupdesc)
{
  ...
  {
    ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH), errmsg("function return row and query-specified return row do not match"), errdetail_plural("Returned row contains %d attribute, but query expects %d.", "Returned row contains %d attributes, but query expects %d.", src_tupdesc->natts, src_tupdesc->natts, dst_tupdesc->natts)));
  }
  ...
  for (i = 0; i < dst_tupdesc->natts; i++)
  {
    ...
    {
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH), errmsg("function return row and query-specified return row do not match"), errdetail("Returned type %s at ordinal position %d, but query expects %s.", format_type_be(sattr->atttypid), i + 1, format_type_be(dattr->atttypid))));
    }
