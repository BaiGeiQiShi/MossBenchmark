...
CheckOpSlotCompatibility(ExprEvalStep *op, TupleTableSlot *slot)
{
  ...
  if (slot->tts_ops == &TTSOpsHeapTuple && op->d.fetch.kind == &TTSOpsBufferHeapTuple)
  {
    return;
  }
...
ExecInitInterpreter(void)
{
  ...
  if (dispatch_table == NULL)
  {
    ...
    qsort(reverse_dispatch_table, EEOP_LAST /* nmembers */, sizeof(ExprEvalOpLookup), dispatch_compare_ptr);
  }
#endif
}
...
ExecEvalStepOp(ExprState *state, ExprEvalStep *op)
{
  ...
  if (state->flags & EEO_FLAG_DIRECT_THREADED)
  {
    ...
    res = bsearch(&key, reverse_dispatch_table, EEOP_LAST /* nmembers */, sizeof(ExprEvalOpLookup), dispatch_compare_ptr);
...
ExecEvalSubscriptingRefFetch(ExprState *state, ExprEvalStep *op)
{
  ...
  {
    /* Scalar case */
    *op->resvalue = array_get_element(*op->resvalue, sbsrefstate->numupper, sbsrefstate->upperindex, sbsrefstate->refattrlength, sbsrefstate->refelemlength, sbsrefstate->refelembyval, sbsrefstate->refelemalign, op->resnull);
  }
  else
  {
    /* Slice case */
    *op->resvalue = array_get_slice(*op->resvalue, sbsrefstate->numupper, sbsrefstate->upperindex, sbsrefstate->lowerindex, sbsrefstate->upperprovided, sbsrefstate->lowerprovided, sbsrefstate->refattrlength, sbsrefstate->refelemlength, sbsrefstate->refelembyval, sbsrefstate->refelemalign);
  }
}
...
ExecEvalSubscriptingRefOld(ExprState *state, ExprEvalStep *op)
{
  ...
  {
    /* Scalar case */
    sbsrefstate->prevvalue = array_get_element(*op->resvalue, sbsrefstate->numupper, sbsrefstate->upperindex, sbsrefstate->refattrlength, sbsrefstate->refelemlength, sbsrefstate->refelembyval, sbsrefstate->refelemalign, &sbsrefstate->prevnull);
  }
  else
  {
    /* Slice case */
...
ExecEvalSubscriptingRefAssign(ExprState *state, ExprEvalStep *op)
{
  ...
  {
    /* Scalar case */
    *op->resvalue = array_set_element(*op->resvalue, sbsrefstate->numupper, sbsrefstate->upperindex, sbsrefstate->replacevalue, sbsrefstate->replacenull, sbsrefstate->refattrlength, sbsrefstate->refelemlength, sbsrefstate->refelembyval, sbsrefstate->refelemalign);
  }
  else
  {
    /* Slice case */
    *op->resvalue = array_set_slice(*op->resvalue, sbsrefstate->numupper, sbsrefstate->upperindex, sbsrefstate->lowerindex, sbsrefstate->upperprovided, sbsrefstate->lowerprovided, sbsrefstate->replacevalue, sbsrefstate->replacenull, sbsrefstate->refattrlength, sbsrefstate->refelemlength, sbsrefstate->refelembyval, sbsrefstate->refelemalign);
  }
}
...
ExecAggTransReparent(AggState *aggstate, AggStatePerTrans pertrans, Datum newValue, bool newValueIsNull, Datum oldValue, bool oldValueIsNull)
{
  ...
  if (!newValueIsNull)
  {
    ...
    if (DatumIsReadWriteExpandedObject(newValue, false, pertrans->transtypeLen) && MemoryContextGetParent(DatumGetEOHP(newValue)->eoh_context) == CurrentMemoryContext)
      /* do nothing */;
    else
    {
