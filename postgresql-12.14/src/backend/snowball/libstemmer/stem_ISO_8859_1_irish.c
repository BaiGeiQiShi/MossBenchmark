...
#endif
  extern int
  irish_ISO_8859_1_stem(struct SN_env *z);
#ifdef __cplusplus
}
#endif
static int
r_verb_sfx(struct SN_env *z);
static int
r_deriv(struct SN_env *z);
static int
r_noun_sfx(struct SN_env *z);
static int
r_mark_regions(struct SN_env *z);
static int
r_initial_morph(struct SN_env *z);
static int
...
extern "C"
{
  ...
  irish_ISO_8859_1_create_env(void);
  extern void
  irish_ISO_8859_1_close_env(struct SN_env *z);
...
{ /* backwardmode */
  if (!(z->I[0] <= z->c))
  {
    return 0; /* $(<integer expression> <= <integer expression>), line 99 */
  }
  return 1;
}
...
{ /* backwardmode */
  if (!(z->I[2] <= z->c))
  {
    return 0; /* $(<integer expression> <= <integer expression>), line 101 */
  }
  return 1;
}
...
{ /* backwardmode */
  int among_var;
  z->ket = z->c; /* [, line 130 */
  if (z->c - 2 <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((282896 >> (z->p[z->c - 1] & 0x1f)) & 1))
  {
    return 0; /* substring, line 130 */
  }
