...
gistFindPath(Relation r, BlockNumber child, OffsetNumber *downlinkoffnum)
{
  ...
  while (fifo != NIL)
  {
    ...
    if (top->parent && top->parent->lsn < GistPageGetNSN(page) && GistPageGetOpaque(page)->rightlink != InvalidBlockNumber /* sanity check */)
    {
      /*
...
gistfinishsplit(GISTInsertState *state, GISTInsertStack *stack, GISTSTATE *giststate, List *splitinfo, bool unlockbuf)
{
  ...
  gistFindCorrectParent(state->r, stack);
  if (gistinserttuples(state, stack->parent, giststate, tuples, 2, stack->downlinkoffnum, left->buf, right->buf, true, /* Unlock parent */
