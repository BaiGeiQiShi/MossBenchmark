...
static bool gistindex_keytest(IndexScanDesc scan, IndexTuple tuple, Page page, OffsetNumber offset, bool *recheck_p, bool *recheck_distances_p)
{
  ...
  *recheck_distances_p = false;
  ...
  while (keySize > 0) {
    ...
    datum =
        index_getattr(tuple, key->sk_attno, giststate->leafTupdesc, &isNull);
    ...
    if (key->sk_flags & SK_ISNULL) {
      ...
      gistdentryinit(giststate, key->sk_attno - 1, &de, datum, r, page, offset,
                     false, isNull);
      ...
      test = FunctionCall5Coll(
          &key->sk_func, key->sk_collation, PointerGetDatum(&de),
          key->sk_argument, Int16GetDatum(key->sk_strategy),
          ObjectIdGetDatum(key->sk_subtype), PointerGetDatum(&recheck));
      ...
      if (so->curPageData < so->nPageData) {
        if (scan->kill_prior_tuple && so->curPageData > 0) {
          ...
          if (so->killedItems == NULL) {
            ...
            so->killedItems = (OffsetNumber *)palloc(MaxIndexTuplesPerPage *
                                                     sizeof(OffsetNumber));
