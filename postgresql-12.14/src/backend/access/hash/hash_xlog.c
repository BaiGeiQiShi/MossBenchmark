...
hash_xlog_init_meta_page(XLogReaderState *record)
{
  ...
  _hash_init_metabuffer(metabuf, xlrec->num_tuples, xlrec->procid, xlrec->ffactor, true);
  ...
  if (action == BLK_NEEDS_REDO)
  {
    ...
    if (xldata->ntups > 0)
    {
      ...
      while (data - begin < datalen)
      {
        ...
        {
          elog(ERROR, "hash_xlog_move_page_contents: failed to add item to hash index page, size %d bytes", (int)itemsz);
        }
...
hash_xlog_squeeze_page(XLogReaderState *record)
{
  ...
  if (action == BLK_NEEDS_REDO)
  {
    ...
    if (xldata->ntups > 0)
    {
      ...
      while (data - begin < datalen)
      {
        ...
        if (l == InvalidOffsetNumber)
        {
          elog(ERROR, "hash_xlog_squeeze_page: failed to add item to hash index page, size %d bytes", (int)itemsz);
        }
  ...
  if (!xldata->is_prev_bucket_same_wrt && XLogReadBufferForRedo(record, 3, &prevbuf) == BLK_NEEDS_REDO)
  {
