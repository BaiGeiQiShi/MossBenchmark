 ...
 *			Try to make a given tuple fit into one page by
 *compressing or moving off attributes
 *
 *		toast_delete -
 ...
 * toast_datum_size
 *
 *	Return the physical storage size (possibly compressed) of a varlena
 *datum
 * ----------
 */
Size
 ...
 *	If we "untoasted" them here, they'd just get changed back to
 *short-header format anyway within heap_fill_tuple.
 * ----------
 */
Datum
 ...
 *
 *	Test whether a toast value with the given ID exists in the toast
 *relation. For safety, we consider a value to exist if there are either live or
 *dead toast rows with that ID; see notes for GetNewOidWithIndex().
 * ----------
 */
static bool
toastrel_valueid_exists(Relation toastrel, Oid valueid)
  ...
  while ((ttup = systable_getnext_ordered(toastscan, ForwardScanDirection)) != NULL) {
    ...
    if (residx < numchunks - 1) {
      if (chunksize != TOAST_MAX_CHUNK_SIZE) {
            ...
            chunksize, (int)TOAST_MAX_CHUNK_SIZE, residx, numchunks, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
      }
    } else if (residx == numchunks - 1) {
      if ((residx * TOAST_MAX_CHUNK_SIZE + chunksize) != ressize) {

            "unexpected chunk size %d (expected %d) in final chunk %d for toast value %u in %s",
            chunksize, (int)(ressize - residx * TOAST_MAX_CHUNK_SIZE), residx, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
      }
    } else {

          "unexpected chunk number %d (out of range %d..%d) for toast value %u in %s",
          residx, 0, numchunks - 1, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
    }
  ...
  while ((ttup = systable_getnext_ordered(toastscan, ForwardScanDirection)) != NULL) {
    ...
    if (residx < totalchunks - 1) {
      if (chunksize != TOAST_MAX_CHUNK_SIZE) {
            ...
            chunksize, (int)TOAST_MAX_CHUNK_SIZE, residx, totalchunks, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
      }
    } else if (residx == totalchunks - 1) {
      if ((residx * TOAST_MAX_CHUNK_SIZE + chunksize) != attrsize) {

            "unexpected chunk size %d (expected %d) in final chunk %d for toast value %u in %s when fetching slice",
            chunksize, (int)(attrsize - residx * TOAST_MAX_CHUNK_SIZE), residx, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
      }
    } else {

          "unexpected chunk number %d (out of range %d..%d) for toast value %u in %s",
          residx, 0, totalchunks - 1, toast_pointer.va_valueid, RelationGetRelationName(toastrel));
    }
 ...
 *	be called for a set of indexes opened previously with
 *toast_open_indexes.
 */
static void
toast_close_indexes(Relation *toastidxs, int num_indexes, LOCKMODE lock)
 ...
 *	just use the oldest one.  This is safe: at worst, we will get a
 *"snapshot too old" error that might have been avoided otherwise.
 */
static void
init_toast_snapshot(Snapshot toast_snapshot)
{
