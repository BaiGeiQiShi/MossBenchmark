...
heap_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  else if (info == XLOG_HEAP_UPDATE)
  {
    ...
    appendStringInfo(buf, "off %u xmax %u flags 0x%02X ", xlrec->old_offnum, xlrec->old_xmax, xlrec->flags);
    out_infobits(buf, xlrec->old_infobits_set);
    appendStringInfo(buf, "; new off %u xmax %u", xlrec->new_offnum, xlrec->new_xmax);
  }
  ...
  else if (info == XLOG_HEAP_LOCK)
  {
    ...
    appendStringInfo(buf, "off %u: xid %u: flags 0x%02X ", xlrec->offnum, xlrec->locking_xid, xlrec->flags);
    out_infobits(buf, xlrec->infobits_set);
  }
...
heap2_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  else if (info == XLOG_HEAP2_FREEZE_PAGE)
  {
    ...
    appendStringInfo(buf, "cutoff xid %u ntuples %u", xlrec->cutoff_xid, xlrec->ntuples);
  }
  ...
  else if (info == XLOG_HEAP2_VISIBLE)
  {
    ...
    appendStringInfo(buf, "cutoff xid %u flags 0x%02X", xlrec->cutoff_xid, xlrec->flags);
  }
  else if (info == XLOG_HEAP2_MULTI_INSERT)
  {
    xl_heap_multi_insert *xlrec = (xl_heap_multi_insert *)rec;

    appendStringInfo(buf, "%d tuples flags 0x%02X", xlrec->ntuples, xlrec->flags);
  }
  else if (info == XLOG_HEAP2_LOCK_UPDATED)
  {
    xl_heap_lock_updated *xlrec = (xl_heap_lock_updated *)rec;

    appendStringInfo(buf, "off %u: xmax %u: flags 0x%02X ", xlrec->offnum, xlrec->xmax, xlrec->flags);
    out_infobits(buf, xlrec->infobits_set);
  }
  else if (info == XLOG_HEAP2_NEW_CID)
  {
    ...
    appendStringInfo(buf, "rel %u/%u/%u; tid %u/%u", xlrec->target_node.spcNode, xlrec->target_node.dbNode, xlrec->target_node.relNode, ItemPointerGetBlockNumber(&(xlrec->target_tid)), ItemPointerGetOffsetNumber(&(xlrec->target_tid)));
