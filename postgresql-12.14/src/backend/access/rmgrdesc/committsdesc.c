...
commit_ts_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  else if (info == COMMIT_TS_TRUNCATE)
  {
    ...
    appendStringInfo(buf, "pageno %d, oldestXid %u", trunc->pageno, trunc->oldestXid);
  }
  else if (info == COMMIT_TS_SETTS)
  {
    ...
    appendStringInfo(buf, "set %s/%d for: %u", timestamptz_to_str(xlrec->timestamp), xlrec->nodeid, xlrec->mainxid);
