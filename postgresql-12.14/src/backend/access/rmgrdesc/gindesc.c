...
gin_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  case XLOG_GIN_INSERT:
  {
    ...
    appendStringInfo(buf, "isdata: %c isleaf: %c", (xlrec->flags & GIN_INSERT_ISDATA) ? 'T' : 'F', (xlrec->flags & GIN_INSERT_ISLEAF) ? 'T' : 'F');
    ...
    else
    {
      ...
      else
      {
        ...
        appendStringInfo(buf, " pitem: %u-%u/%u", PostingItemGetBlockNumber(&insertData->newitem), ItemPointerGetBlockNumber(&insertData->newitem.key), ItemPointerGetOffsetNumber(&insertData->newitem.key));
      }
    }
  }
  break;
  ...
  case XLOG_GIN_DELETE_LISTPAGE:
    appendStringInfo(buf, "ndeleted: %d", ((ginxlogDeleteListPages *)rec)->ndeleted);
    break;
  }
}
