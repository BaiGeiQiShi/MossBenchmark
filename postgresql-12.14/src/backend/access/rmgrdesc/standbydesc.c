...
standby_desc_running_xacts(StringInfo buf, xl_running_xacts *xlrec)
{
  ...
  appendStringInfo(buf, "nextXid %u latestCompletedXid %u oldestRunningXid %u", xlrec->nextXid, xlrec->latestCompletedXid, xlrec->oldestRunningXid);
  if (xlrec->xcnt > 0)
  {
...
standby_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  if (info == XLOG_STANDBY_LOCK)
  {
    ...
    {
      appendStringInfo(buf, "xid %u db %u rel %u ", xlrec->locks[i].xid, xlrec->locks[i].dbOid, xlrec->locks[i].relOid);
    }
  }
