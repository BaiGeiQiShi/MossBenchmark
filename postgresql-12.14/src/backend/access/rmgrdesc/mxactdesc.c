...
multixact_desc(StringInfo buf, XLogReaderState *record)
{
  ...
  else if (info == XLOG_MULTIXACT_CREATE_ID)
  {
    ...
    appendStringInfo(buf, "%u offset %u nmembers %d: ", xlrec->mid, xlrec->moff, xlrec->nmembers);
  ...
  else if (info == XLOG_MULTIXACT_TRUNCATE_ID)
  {
    ...
    appendStringInfo(buf, "offsets [%u, %u), members [%u, %u)", xlrec->startTruncOff, xlrec->endTruncOff, xlrec->startTruncMemb, xlrec->endTruncMemb);
  }
}
