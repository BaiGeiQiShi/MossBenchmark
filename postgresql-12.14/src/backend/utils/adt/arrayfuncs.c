    ...
    while (!itemdone)
    {
      ...
      case '}':
        if (!in_quotes)
        {
          ...
          {
            ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION), errmsg("malformed array literal: \"%s\"", str),
...
array_set_element_expanded(Datum arraydatum, int nSubscripts, int *indx, Datum dataValue, bool isNull, int arraytyplen, int elmlen, bool elmbyval, char elmalign)
{
  ...
  if (dim[0] > eah->dvalueslen)
  {
    ...
    eah->dvalues = dvalues = (Datum *)repalloc(dvalues, newlen * sizeof(Datum));
    if (dnulls)
    {
      eah->dnulls = dnulls = (bool *)repalloc(dnulls, newlen * sizeof(bool));
    }
  ...
  {
    eah->dnulls = dnulls = (bool *)MemoryContextAllocZero(eah->hdr.eoh_context, eah->dvalueslen * sizeof(bool));
  }
...
array_set_slice(Datum arraydatum, int nSubscripts, int *upperIndx, int *lowerIndx, bool *upperProvided, bool *lowerProvided, Datum srcArrayDatum, bool isNull, int arraytyplen, int elmlen, bool elmbyval, char elmalign)
{
  ...
  if (ndim == 0)
  {
    ...
    for (i = 0; i < nSubscripts; i++)
    {
      ...
      {
        ereport(ERROR, (errcode(ERRCODE_ARRAY_SUBSCRIPT_ERROR), errmsg("array slice subscript must provide both boundaries"),
  ...
  *nelemsp = nelems;
  ...
  for (i = 0; i < nelems; i++)
  {
    ...
    if (bitmap && (*bitmap & bitmask) == 0)
    {
      ...
      {
        ereport(ERROR, (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED), errmsg("null array element not allowed in this context")));
      }
    }
    else
    {
...
hash_array(PG_FUNCTION_ARGS)
{
   ...
   */
  InitFunctionCallInfoData(*locfcinfo, &typentry->hash_proc_finfo, 1, PG_GET_COLLATION(), NULL, NULL);
...
hash_array_extended(PG_FUNCTION_ARGS)
{
  ...
  InitFunctionCallInfoData(*locfcinfo, &typentry->hash_extended_proc_finfo, 2, PG_GET_COLLATION(), NULL, NULL);
...
array_contain_compare(AnyArrayType *array1, AnyArrayType *array2, Oid collation, bool matchall, void **fn_extra)
{
  ...
  {
    deconstruct_array((ArrayType *)array2, element_type, typlen, typbyval, typalign, &values2, &nulls2, &nelems2);
  }
...
initArrayResultAny(Oid input_type, MemoryContext rcontext, bool subcontext)
{
  ...
  if (OidIsValid(element_type))
  {
    ...
    astate = (ArrayBuildStateAny *)MemoryContextAlloc(arraystate->mcontext, sizeof(ArrayBuildStateAny));
  ...
  else
  {
    ...
    astate = (ArrayBuildStateAny *)MemoryContextAlloc(scalarstate->mcontext, sizeof(ArrayBuildStateAny));
...
accumArrayResultAny(ArrayBuildStateAny *astate, Datum dvalue, bool disnull, Oid input_type, MemoryContext rcontext)
{
  ...
  {
    (void)accumArrayResultArr(astate->arraystate, dvalue, disnull, input_type, rcontext);
  }
