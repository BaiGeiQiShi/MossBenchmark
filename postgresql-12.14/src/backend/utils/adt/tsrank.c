  ...
  *nitem = 0;
  ...
  if (item->prefix)
  {
    ...
    *nitem = 0;
    ...
    while (StopMiddle < (WordEntry *)STRPTR(t) && WordECompareQueryItem(STRPTR(t), GETOPERAND(q), StopMiddle, item, true) == 0)
    {
      (*nitem)++;
      StopMiddle++;
    }
...
Cover(DocRepresentation *doc, int len, QueryRepresentation *qr, CoverExt *ext)
{
  ...
  while (ptr - doc < len)
  {
    ...
    if (TS_execute(GETQUERY(qr->query), (void *)qr, TS_EXEC_EMPTY, checkcondition_QueryOperand))
    {
