...
fetch_array_arg_replace_nulls(FunctionCallInfo fcinfo, int argno)
{
  ...
  if (my_extra == NULL)
  {
    my_extra = (ArrayMetaState *)MemoryContextAlloc(fcinfo->flinfo->fn_mcxt, sizeof(ArrayMetaState));
...
array_positions(PG_FUNCTION_ARGS)
{
  ...
  while (array_iterate(array_iterator, &value, &isnull))
  {
    ...
    if (isnull || null_search)
    {
      if (isnull && null_search)
      {
        astate = accumArrayResult(astate, Int32GetDatum(position), false, INT4OID, CurrentMemoryContext);
      }
