  ...
  else
  {
    ...
    case JSONBTYPE_JSONB:
    {
      ...
      else
      {
        ...
        {
          if (type == WJB_END_ARRAY || type == WJB_END_OBJECT || type == WJB_BEGIN_ARRAY || type == WJB_BEGIN_OBJECT)
          {
...
array_dim_to_jsonb(JsonbInState *result, int dim, int ndims, int *dims, Datum *vals, bool *nulls, int *valcount, JsonbTypeCategory tcategory, Oid outfuncoid)
{
  ...
  for (i = 1; i <= dims[dim]; i++)
  {
    if (dim + 1 == ndims)
    {
      datum_to_jsonb(vals[*valcount], nulls[*valcount], result, tcategory, outfuncoid, false);
      (*valcount)++;
    }
    else
    {
...
array_to_jsonb_internal(Datum array, JsonbInState *result)
{
  ...
  jsonb_categorize_type(element_type, &tcategory, &outfuncoid);

  deconstruct_array(v, element_type, typlen, typbyval, typalign, &elements, &nulls, &nitems);

  array_dim_to_jsonb(result, 0, ndim, dim, elements, nulls, &count, tcategory, outfuncoid);
...
jsonb_agg_transfn(PG_FUNCTION_ARGS)
{
  ...
  datum_to_jsonb(val, PG_ARGISNULL(1), &elem, state->val_category, state->val_output_func, false);
...
jsonb_object_agg_transfn(PG_FUNCTION_ARGS)
{
  ...
  datum_to_jsonb(val, false, &elem, state->key_category, state->key_output_func, true);
  ...
  datum_to_jsonb(val, PG_ARGISNULL(2), &elem, state->val_category, state->val_output_func, false);
...
cannotCastJsonbValue(enum jbvType type, const char *sqltype)
{
  static const struct
  {
    enum jbvType type;
    const char *msg;
  } messages[] = {{jbvNull, gettext_noop("cannot cast jsonb null to type %s")}, {jbvString, gettext_noop("cannot cast jsonb string to type %s")}, {jbvNumeric, gettext_noop("cannot cast jsonb numeric to type %s")}, {jbvBool, gettext_noop("cannot cast jsonb boolean to type %s")}, {jbvArray, gettext_noop("cannot cast jsonb array to type %s")}, {jbvObject, gettext_noop("cannot cast jsonb object to type %s")}, {jbvBinary, gettext_noop("cannot cast jsonb array or object to type %s")}};
  int i;
...
jsonb_int4(PG_FUNCTION_ARGS)
{
  ...
  retValue = DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric));
...
jsonb_float8(PG_FUNCTION_ARGS)
{
  ...
  retValue = DirectFunctionCall1(numeric_float8, NumericGetDatum(v.val.numeric));
