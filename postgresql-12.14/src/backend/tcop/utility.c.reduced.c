...
static void
ProcessUtilitySlow(ParseState *pstate, PlannedStmt *pstmt, const char *queryString, ProcessUtilityContext context, ParamListInfo params, QueryEnvironment *queryEnv, DestReceiver *dest, char *completionTag);
static void
...
ProcessUtility(PlannedStmt *pstmt, const char *queryString, ProcessUtilityContext context, ParamListInfo params, QueryEnvironment *queryEnv, DestReceiver *dest, char *completionTag)
{
  ...
  if (ProcessUtility_hook) {
    (*ProcessUtility_hook)(pstmt, queryString, context, params, queryEnv, dest, completionTag);
  } else {
...
standard_ProcessUtility(PlannedStmt *pstmt, const char *queryString, ProcessUtilityContext context, ParamListInfo params, QueryEnvironment *queryEnv, DestReceiver *dest, char *completionTag)
{
  ...
  case T_DeclareCursorStmt:;
    PerformCursorOpen((DeclareCursorStmt *)parsetree, params, queryString, isTopLevel);
    break;
  ...
  case T_CopyStmt: {;
    ...
    DoCopy(pstate, (CopyStmt *)parsetree, pstmt->stmt_location, pstmt->stmt_len, &processed);
    if (completionTag) {
  ...
  case T_VacuumStmt: {;
    ...
    /* we choose to allow this during "read only" transactions */
    PreventCommandDuringRecovery(stmt->is_vacuumcmd ? "VACUUM" : "ANALYZE");
    /* forbidden in parallel mode due to CommandIsReadOnly */
    ExecVacuum(pstate, stmt, isTopLevel);
  } break;

  case T_ExplainStmt:;
    ExplainQuery(pstate, (ExplainStmt *)parsetree, queryString, params, queryEnv, dest);
    break;
...
ProcessUtilitySlow(ParseState *pstate, PlannedStmt *pstmt, const char *queryString, ProcessUtilityContext context, ParamListInfo params, QueryEnvironment *queryEnv, DestReceiver *dest, char *completionTag)
{
  ...
  PG_TRY();
  {
    ...
    switch (nodeTag(parsetree)) {
       ...
       */
    case T_CreateSchemaStmt:;
      CreateSchemaCommand((CreateSchemaStmt *)parsetree, queryString, pstmt->stmt_location, pstmt->stmt_len);
    ...
    case T_CreateForeignTableStmt: {;
      ...
      while (stmts != NIL) {
        ...
        if (IsA(stmt, CreateStmt)) {
          ...
          /* Create the table itself */
          address = DefineRelation(&cstmt->base, RELKIND_FOREIGN_TABLE, InvalidOid, NULL, queryString);
          ...
          ProcessUtility(wrapper, queryString, PROCESS_UTILITY_SUBCOMMAND, params, NULL, None_Receiver, NULL);
        }
    ...
    case T_AlterTableStmt: {;
      ...
      if (OidIsValid(relid)) {
        /* Run parse analysis ... */
        stmts = transformAlterTableStmt(relid, atstmt, queryString);
    ...
    case T_AlterDomainStmt: {;
      ...
      case 'C': ;/* ADD CONSTRAINT */
        address = AlterDomainAddConstraint(stmt->typeName, stmt->def, &secondaryObject);
        break;
      case 'X': ;/* DROP CONSTRAINT */
        address = AlterDomainDropConstraint(stmt->typeName, stmt->name, stmt->behavior, stmt->missing_ok);
        break;
    ...
    case T_DefineStmt: {;
      ...
      case OBJECT_AGGREGATE:;
        address = DefineAggregate(pstate, stmt->defnames, stmt->args, stmt->oldstyle, stmt->definition, stmt->replace);
        break;
      ...
      case OBJECT_TSCONFIGURATION:;
        Assert(stmt->args == NIL);
        address = DefineTSConfiguration(stmt->defnames, stmt->definition, &secondaryObject);
        break;
    ...
    case T_IndexStmt: ;/* CREATE INDEX */
    {
      ...
      if (stmt->concurrent) {
        PreventInTransactionBlock(isTopLevel, "CREATE INDEX CONCURRENTLY");
      }
      ...
      if (stmt->relation->inh && get_rel_relkind(relid) == RELKIND_PARTITIONED_TABLE) {
        ...
        foreach (lc, inheritors) {
          ...
          if (relkind != RELKIND_RELATION && relkind != RELKIND_MATVIEW && relkind != RELKIND_PARTITIONED_TABLE && relkind != RELKIND_FOREIGN_TABLE) {
          ...
          if (relkind == RELKIND_FOREIGN_TABLE && (stmt->unique || stmt->primary)) {
            ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE), errmsg("cannot create unique index on partitioned table \"%s\"", stmt->relation->relname),errdetail("Table \"%s\" contains partitions that are foreign tables.",       stmt->relation->relname)));
          }
        }
      ...
      address = DefineIndex(relid, /* OID of heap relation */
          stmt, InvalidOid,        /* no predefined OID */
          InvalidOid,              /* no parent index */
          InvalidOid,              /* no parent constraint */
          is_alter_table, true,    /* check_rights */
    ...
    case T_AlterExtensionContentsStmt:;
      address = ExecAlterExtensionContentsStmt((AlterExtensionContentsStmt *)parsetree, &secondaryObject);
      break;
    ...
    case T_CreateTableAsStmt:;
      address = ExecCreateTableAs((CreateTableAsStmt *)parsetree, queryString, params, queryEnv, completionTag);
      break;
    ...
    case T_RefreshMatViewStmt:;
      ...
      PG_TRY();
      {
        address = ExecRefreshMatView((RefreshMatViewStmt *)parsetree, queryString, params, completionTag);
      }
      PG_CATCH();
      {
    ...
    case T_CreateTrigStmt:;
      address = CreateTrigger((CreateTrigStmt *)parsetree, queryString, InvalidOid, InvalidOid, InvalidOid, InvalidOid, InvalidOid, InvalidOid, NULL, false, false);
    ...
    case T_AlterObjectDependsStmt:;
      address = ExecAlterObjectDependsStmt((AlterObjectDependsStmt *)parsetree, &secondaryObject);
      break;

    case T_AlterObjectSchemaStmt:;
      address = ExecAlterObjectSchemaStmt((AlterObjectSchemaStmt *)parsetree, &secondaryObject);
      break;
    ...
    case T_CreateSubscriptionStmt:;
      address = CreateSubscription((CreateSubscriptionStmt *)parsetree, isTopLevel);
      break;
