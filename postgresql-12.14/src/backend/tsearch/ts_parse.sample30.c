...
LexizeExec(LexizeData *ld, ParsedLex **correspondLexem)
{
  ...
  if (ld->curDictId == InvalidOid)
  {
    ...
    while (ld->towork.head)
    {
      ...
      for (i = ld->posDict; i < map->len; i++)
      {
        ...
        res = (TSLexeme *)DatumGetPointer(FunctionCall4(&(dict->lexize), PointerGetDatum(dict->dictData), PointerGetDatum(curValLemm), Int32GetDatum(curValLenLemm), PointerGetDatum(&ld->dictState)));
  ...
  { /* curDictId is valid */
    ...
    while (ld->curSub)
    {
      ...
      res = (TSLexeme *)DatumGetPointer(FunctionCall4(&(dict->lexize), PointerGetDatum(dict->dictData), PointerGetDatum(curVal->lemm), Int32GetDatum(curVal->lenlemm), PointerGetDatum(&ld->dictState)));
...
parsetext(Oid cfgId, ParsedText *prs, char *buf, int buflen)
{
  ...
  prsdata = (void *)DatumGetPointer(FunctionCall2(&prsobj->prsstart, PointerGetDatum(buf), Int32GetDatum(buflen)));

  LexizeInit(&ldata, cfg);

  do
  {
    type = DatumGetInt32(FunctionCall3(&(prsobj->prstoken), PointerGetDatum(prsdata), PointerGetDatum(&lemm), PointerGetDatum(&lenlemm)));
...
hlfinditem(HeadlineParsedText *prs, TSQuery query, int32 pos, char *buf, int buflen)
{
  ...
  {
    if (item->type == QI_VAL && tsCompareString(GETOPERAND(query) + item->qoperand.distance, item->qoperand.length, buf, buflen, item->qoperand.prefix) == 0)
    {
      if (word->item)
      {
