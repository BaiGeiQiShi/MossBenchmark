...
{
  int i, nnw = 0, tnm = 16;
  TheLexeme *newwrds = (TheLexeme *)palloc(sizeof(TheLexeme) * tnm), *ptrwrds;
  ...
  for (i = 0; i < d->nwrds; i++)
  {
    ...
    {
      ptr = (TSLexeme *)DatumGetPointer(FunctionCall4(&(d->subdict->lexize), PointerGetDatum(d->subdict->dictData), PointerGetDatum(d->wrds[i].lexeme), Int32GetDatum(strlen(d->wrds[i].lexeme)), PointerGetDatum(NULL)));
...
compileTheSubstitute(DictThesaurus *d)
{
  ...
  for (i = 0; i < d->nsubst; i++)
  {
    ...
    while (inptr && inptr->lexeme)
    {
      ...
      {
        lexized = (TSLexeme *)DatumGetPointer(FunctionCall4(&(d->subdict->lexize), PointerGetDatum(d->subdict->dictData), PointerGetDatum(inptr->lexeme), Int32GetDatum(strlen(inptr->lexeme)), PointerGetDatum(NULL)));
      }
