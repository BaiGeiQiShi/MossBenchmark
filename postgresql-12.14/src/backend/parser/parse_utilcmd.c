...
transformTableLikeClause(CreateStmtContext *cxt, TableLikeClause *table_like_clause)
{
  ...
  for (parent_attno = 1; parent_attno <= tupleDesc->natts; parent_attno++)
  {
     ...
     */
    if (attribute->atthasdef && attribute->attgenerated && (table_like_clause->options & CREATE_TABLE_LIKE_GENERATED))
    {
   ...
   */
  if (table_like_clause->options & (CREATE_TABLE_LIKE_DEFAULTS | CREATE_TABLE_LIKE_GENERATED | CREATE_TABLE_LIKE_CONSTRAINTS | CREATE_TABLE_LIKE_INDEXES))
  {
...
expandTableLikeClause(RangeVar *heapRel, TableLikeClause *table_like_clause)
{
   ...
   */
  attmap = convert_tuples_by_name_map(RelationGetDescr(childrel), tupleDesc, gettext_noop("could not convert row type"));

  /*
   * Process defaults, if required.
   */
  if ((table_like_clause->options & (CREATE_TABLE_LIKE_DEFAULTS | CREATE_TABLE_LIKE_GENERATED)) && constr != NULL)
  {
    ...
    for (parent_attno = 1; parent_attno <= tupleDesc->natts; parent_attno++)
    {
       ...
       * defaults.
       */
      if (attribute->atthasdef && (attribute->attgenerated ? (table_like_clause->options & CREATE_TABLE_LIKE_GENERATED) : (table_like_clause->options & CREATE_TABLE_LIKE_DEFAULTS)))
      {
        ...
        atsubcmd->def = map_variable_attnos(this_default, 1, 0, attmap, tupleDesc->natts, InvalidOid, &found_whole_row);
        ...
        if (found_whole_row)
        {
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("cannot convert whole-row table reference"), errdetail("Generation expression for column \"%s\" contains a whole-row reference to table \"%s\".", NameStr(attribute->attname), RelationGetRelationName(relation))));
        }
  ...
  if ((table_like_clause->options & CREATE_TABLE_LIKE_CONSTRAINTS) && constr != NULL)
  {
    ...
    for (ccnum = 0; ccnum < constr->num_check; ccnum++)
    {
      ...
      if ((table_like_clause->options & CREATE_TABLE_LIKE_COMMENTS) && (comment = GetComment(get_relation_constraint_oid(RelationGetRelid(relation), n->conname, false), ConstraintRelationId, 0)) != NULL)
      {
        ...
        stmt->object = (Node *)list_make3(makeString(heapRel->schemaname), makeString(heapRel->relname), makeString(n->conname));
        stmt->comment = comment;
  ...
  if ((table_like_clause->options & CREATE_TABLE_LIKE_INDEXES) && relation->rd_rel->relhasindex)
  {
    ...
    foreach (l, parent_indexes)
    {
      ...
      index_stmt = generateClonedIndexStmt(heapRel, parent_index, attmap, tupleDesc->natts, NULL);
    ...
    foreach (lc, constraint->keys)
    {
      ...
      foreach (columns, index->indexParams)
      {
        ...
        if (iparam->name && strcmp(key, iparam->name) == 0)
        {
          ...
          {
            ereport(ERROR, (errcode(ERRCODE_DUPLICATE_COLUMN), errmsg("column \"%s\" appears twice in unique constraint", key), parser_errposition(cxt->pstate, constraint->location)));
          }
        }
      }
...
transformAlterTableStmt(Oid relid, AlterTableStmt *stmt, const char *queryString)
{
  ...
  foreach (lcmd, stmt->cmds)
  {
    ...
    case AT_AlterColumnType:
    {
       ...
       * later
       */
      if (attnum > 0 && TupleDescAttr(tupdesc, attnum - 1)->attidentity)
      {
...
#define SUPPORTS_ATTRS(node) ((node) != NULL && ((node)->contype == CONSTR_PRIMARY || (node)->contype == CONSTR_UNIQUE || (node)->contype == CONSTR_EXCLUSION || (node)->contype == CONSTR_FOREIGN))
...
setSchemaName(char *context_schema, char **stmt_schema_name)
{
  ...
  {
    ereport(ERROR, (errcode(ERRCODE_INVALID_SCHEMA_DEFINITION), errmsg("CREATE specifies a schema (%s) "
...
transformPartitionBound(ParseState *pstate, Relation parent, PartitionBoundSpec *spec)
{
  ...
  if (strategy == PARTITION_STRATEGY_HASH)
  {
    ...
    if (spec->modulus <= 0)
    {
      ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION), errmsg("modulus for hash partition must be an integer value greater than zero")));
    }
  ...
  else if (strategy == PARTITION_STRATEGY_LIST)
  {
    ...
    {
      colname = deparse_expression((Node *)linitial(partexprs), deparse_context_for(RelationGetRelationName(parent), RelationGetRelid(parent)), false, false);
    }
...
transformPartitionBoundValue(ParseState *pstate, Node *val, const char *colName, Oid colType, int32 colTypmod, Oid partCollation)
{
  ...
  if (IsA(value, CollateExpr))
  {
    ...
    if (type_is_collatable(colType))
    {
      ...
      {
        ereport(ERROR, (errcode(ERRCODE_INDETERMINATE_COLLATION), errmsg("could not determine which collation to use for partition bound expression"), errhint("Use the COLLATE clause to set the collation explicitly.")));
      }
    }
    else
    {
      if (OidIsValid(exprCollOid))
      {
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH), errmsg("collations are not supported by type %s", format_type_be(colType))));
      }
    }
