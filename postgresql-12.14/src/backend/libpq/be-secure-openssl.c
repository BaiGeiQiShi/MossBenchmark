...
aloop:
  ...
  if (r <= 0) {
    ...
    case SSL_ERROR_SYSCALL:
      ...
      break;
    case SSL_ERROR_SSL:
      ereport(COMMERROR, (errcode(ERRCODE_PROTOCOL_VIOLATION), errmsg("could not accept SSL connection: %s", SSLerrmessage(ecode))));
      break;
...
be_tls_read(Port *port, void *ptr, size_t len, int *waitfor)
{
  ...
  case SSL_ERROR_SYSCALL:
    ...
    break;
  case SSL_ERROR_SSL:
    ereport(COMMERROR, (errcode(ERRCODE_PROTOCOL_VIOLATION), errmsg("SSL error: %s", SSLerrmessage(ecode))));
    errno = ECONNRESET;
    n = -1;
    break;
  case SSL_ERROR_ZERO_RETURN:
...
ssl_protocol_version_to_openssl(int v, const char *guc_name, int loglevel)
{
  ...
  ereport(loglevel, (errmsg("%s setting %s not supported by this build", guc_name, GetConfigOption(guc_name, false, false))));
  return -1;
}
