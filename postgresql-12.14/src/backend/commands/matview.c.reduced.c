...
refresh_by_match_merge(Oid matviewOid, Oid tempOid, Oid relowner, int save_sec_context)
{
  ...
  appendStringInfo(&querybuf,
      "SELECT newdata.*::%s FROM %s newdata WHERE newdata.* IS NOT NULL AND EXISTS (SELECT 1 FROM %s newdata2 WHERE newdata2.* IS NOT NULL AND newdata2.* OPERATOR(pg_catalog.*=) newdata.* AND newdata2.ctid OPERATOR(pg_catalog.<>) newdata.ctid)",
  ...
  resetStringInfo(&querybuf);
  appendStringInfo(&querybuf,
      "CREATE TEMP TABLE %s AS SELECT mv.ctid AS tid, newdata.*::%s AS newdata FROM %s mv FULL JOIN %s newdata ON (",
  ...
  resetStringInfo(&querybuf);
  appendStringInfo(&querybuf,
      "DELETE FROM %s mv WHERE ctid OPERATOR(pg_catalog.=) ANY (SELECT diff.tid FROM %s diff WHERE diff.tid IS NOT NULL AND diff.newdata IS NULL)",
