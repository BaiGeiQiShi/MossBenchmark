...
parse_extension_control_file(ExtensionControlFile *control, const char *version)
{
  ...
  for (item = head; item != NULL; item = item->next)
  {
    ...
    {
      ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR), errmsg("unrecognized parameter \"%s\" in file \"%s\"", item->name, filename)));
    }
  }
...
get_required_extension(char *reqExtensionName, char *extensionName, char *origSchemaName, bool cascade, List *parents, bool is_create)
{
  ...
  if (!OidIsValid(reqExtensionOid))
  {
    ...
    {
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT), errmsg("required extension \"%s\" is not installed", reqExtensionName), is_create ? errhint("Use CREATE EXTENSION ... CASCADE to install required extensions too.") : 0));
    }
  }
...
pg_extension_config_dump(PG_FUNCTION_ARGS)
{
  ...
  if (!creating_extension)
  {
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("%s can only be called from an SQL script executed by CREATE EXTENSION", "pg_extension_config_dump()")));
  }
  ...
  ScanKeyInit(&key[0], Anum_pg_extension_oid, BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(CurrentExtensionObject));
  ...
  else
  {
    ...
    a = array_set(a, 1, &arrayIndex, elementDatum, false, -1 /* varlena array */, sizeof(Oid) /* OID's typlen */, true /* OID's typbyval */, 'i' /* OID's typalign */);
  }
  ...
  else
  {
    ...
    a = array_set(a, 1, &arrayIndex, elementDatum, false, -1 /* varlena array */, -1 /* TEXT's typlen */, false /* TEXT's typbyval */, 'i' /* TEXT's typalign */);
  }
...
extension_config_remove(Oid extensionoid, Oid tableoid)
{
  ...
  ScanKeyInit(&key[0], Anum_pg_extension_oid, BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(extensionoid));
  ...
  else
  {
    ...
    deconstruct_array(a, OIDOID, sizeof(Oid), true, 'i', &dvalues, NULL, &nelems);
  ...
  else
  {
    ...
    /* We already checked there are no nulls */
    deconstruct_array(a, TEXTOID, -1, false, 'i', &dvalues, NULL, &nelems);
...
ExecAlterExtensionContentsStmt(AlterExtensionContentsStmt *stmt, ObjectAddress *objAddr)
{
   ...
   */
  extension = get_object_address(OBJECT_EXTENSION, (Node *)makeString(stmt->extname), &relation, AccessShareLock, false);
