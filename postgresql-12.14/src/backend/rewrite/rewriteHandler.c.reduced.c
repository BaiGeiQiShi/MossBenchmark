  ...
  for (attrno = 1; attrno <= numattrs; attrno++)
  {
    ...
    else if (apply_default)
    {
      ...
      if (!new_expr)
      {
        ...
        {
          new_expr = (Node *)makeConst(att_tup->atttypid, -1, att_tup->attcollation, att_tup->attlen, (Datum)0, true, /* isnull */
              att_tup->attbyval);
...
process_matched_tle(TargetEntry *src_tle, TargetEntry *prior_tle, const char *attrName)
{
   ...
   *							  FieldStore(placeholder,
   *subfld1, x)), fld2, FieldStore(placeholder, subfld2, y)) Note the leftmost
   *expression goes on the inside so that the assignments appear to occur
   *left-to-right.
   *
