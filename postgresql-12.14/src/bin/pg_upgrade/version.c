...
new_9_0_populate_pg_largeobject_metadata(ClusterInfo *cluster, bool check_mode)
{
  ...
  for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
    ...
    res = executeQueryOrDie(conn, "SELECT count(*) FROM	pg_catalog.pg_largeobject ");
 ...
 *
 * base_query should be a SELECT yielding a single column named "oid",* containing the pg_type OIDs of one or more types that are known to have
...
check_for_data_types_usage(ClusterInfo *cluster, const char *base_query, const char *output_path)
{
  ...
  for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
    ...
    /*
     * The type(s) of interest might be wrapped in a domain, array,* composite, or range, and these container types can be nested (to
    ...
    appendPQExpBuffer(&querybuf,"WITH RECURSIVE oids AS ( "
        /* start with the type(s) returned by base_query */
        "	%s 	UNION ALL 	SELECT * FROM ( "
        /* inner WITH because we can only reference the CTE once */
        "		WITH x AS (SELECT oid FROM oids) "
        /* domains on any type selected so far */
        "			SELECT t.oid FROM pg_catalog.pg_type t, x WHERE typbasetype = x.oid AND typtype = 'd' 			UNION ALL "
        /* arrays over any type selected so far */
        "			SELECT t.oid FROM pg_catalog.pg_type t, x WHERE typelem = x.oid AND typtype = 'b' 			UNION ALL "
        /* composite types containing any type selected so far */
        "			SELECT t.oid FROM pg_catalog.pg_type t, pg_catalog.pg_class c, pg_catalog.pg_attribute a, x 			WHERE t.typtype = 'c' AND 				  t.oid = c.reltype AND 				  c.oid = a.attrelid AND 				  NOT a.attisdropped AND 				  a.atttypid = x.oid ",base_query);

    /* Ranges came in in 9.2 */
    if (GET_MAJOR_VERSION(cluster->major_version) >= 902) {
      appendPQExpBuffer(&querybuf, "			UNION ALL "
                                   /* ranges containing any type selected so far */
                                   "			SELECT t.oid FROM pg_catalog.pg_type t, pg_catalog.pg_range r, x 			WHERE t.typtype = 'r' AND r.rngtypid = t.oid AND r.rngsubtype = x.oid");
    }

    appendPQExpBuffer(&querybuf, "	) foo ) "
                                 /* now look for stored columns of any such type */
                                 "SELECT n.nspname, c.relname, a.attname FROM	pg_catalog.pg_class c, 		pg_catalog.pg_namespace n, 		pg_catalog.pg_attribute a WHERE	c.oid = a.attrelid AND 		NOT a.attisdropped AND 		a.atttypid IN (SELECT oid FROM oids) AND 		c.relkind IN (" CppAsString2(RELKIND_RELATION) ", " CppAsString2(RELKIND_MATVIEW) ", " CppAsString2(RELKIND_INDEX) ") AND 		c.relnamespace = n.oid AND "
                                                                                                                                                                   /* exclude
                                                                                                                                                                    * possible
                                                                                                                                                                    * orphaned
                                                                                                                                                                    * temp
...
old_9_6_invalidate_hash_indexes(ClusterInfo *cluster, bool check_mode)
{
  ...
  for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
    ...
    /* find hash indexes */
    res = executeQueryOrDie(conn, "SELECT n.nspname, c.relname FROM	pg_catalog.pg_class c, 		pg_catalog.pg_index i, 		pg_catalog.pg_am a, 		pg_catalog.pg_namespace n WHERE	i.indexrelid = c.oid AND 		c.relam = a.oid AND 		c.relnamespace = n.oid AND 		a.amname = 'hash'");
    ...
    if (!check_mode && db_used) {
      ...
      PQclear(executeQueryOrDie(conn, "UPDATE pg_catalog.pg_index i SET	indisvalid = false FROM	pg_catalog.pg_class c, 		pg_catalog.pg_am a, 		pg_catalog.pg_namespace n WHERE	i.indexrelid = c.oid AND 		c.relam = a.oid AND 		c.relnamespace = n.oid AND 		a.amname = 'hash'"));
    }
 ...
 *	11 -> 12
 *	In 12, the sql_identifier data type was switched from name to varchar,*	which does affect the storage (name is by-ref, but not varlena). This
...
report_extension_updates(ClusterInfo *cluster)
{
  ...
  for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
    ...
    res = executeQueryOrDie(conn, "SELECT name FROM pg_available_extensions WHERE installed_version != default_version");
