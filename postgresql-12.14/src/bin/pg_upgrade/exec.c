...
/*
 * exec_prog()
 *		Execute an external program with stdout/stderr redirected, and
 *report errors
 *
 * Formats a command from the given argument list, logs it to the log file,* and attempts to execute that command.  If the command executes
...
#define MAXCMDLEN (2 * MAXPGPATH)
...
#ifdef WIN32
  {
     ...
     * holding its stdout and -l log file descriptors open.  Therefore,* try to open the log file a few more times.
     */
    int iter;
