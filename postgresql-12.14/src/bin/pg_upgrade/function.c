 ...
 * same length, then database array index.  This is to ensure that, eg,* "hstore_plpython" sorts after both "hstore" and "plpython"; otherwise
...
get_loadable_libraries(void)
{
  ...
  for (dbnum = 0; dbnum < old_cluster.dbarr.ndbs; dbnum++) {
     ...
     */
    ress[dbnum] = executeQueryOrDie(conn,"SELECT DISTINCT probin FROM pg_catalog.pg_proc WHERE prolang = %u AND probin IS NOT NULL AND oid >= %u;",ClanguageId, FirstNormalObjectId);
    ...
    if (GET_MAJOR_VERSION(old_cluster.major_version) <= 900) {
      ...
      res = executeQueryOrDie(conn,"SELECT 1 FROM pg_catalog.pg_proc p     JOIN pg_catalog.pg_namespace n     ON pronamespace = n.oid WHERE proname = 'plpython_call_handler' AND nspname = 'public' AND prolang = %u AND probin = '$libdir/plpython' AND p.oid >= %u;",ClanguageId, FirstNormalObjectId);
      if (PQntuples(res) > 0) {
