...
static void
transfer_relfile(FileNameMap *map, const char *suffix, bool vm_must_add_frozenbit);
 ...
 */
void
transfer_all_new_tablespaces(DbInfoArr *old_db_arr, DbInfoArr *new_db_arr, char *old_pgdata, char *new_pgdata)
{
   ...
   * NULL tablespace path, which matches all tablespaces.  In parallel mode,* we pass the default tablespace and all user-created tablespaces and let
  ...
  if (user_opts.jobs <= 1) {
    ...
    for (tblnum = 0; tblnum < os_info.num_old_tablespaces; tblnum++) {
      parallel_transfer_all_new_dbs(old_db_arr, new_db_arr, old_pgdata, new_pgdata, os_info.old_tablespaces[tblnum]);
    }
    /* reap all children */
 ...
 */
void
transfer_all_new_dbs(DbInfoArr *old_db_arr, DbInfoArr *new_db_arr, char *old_pgdata, char *new_pgdata, char *old_tablespace)
{
...
transfer_single_new_db(FileNameMap *maps, int size, char *old_tablespace)
{
   ...
   */
  if (old_cluster.controldata.cat_ver < VISIBILITY_MAP_CRASHSAFE_CAT_VER && new_cluster.controldata.cat_ver >= VISIBILITY_MAP_CRASHSAFE_CAT_VER) {
   ...
   */
  if (old_cluster.controldata.cat_ver < VISIBILITY_MAP_FROZEN_BIT_CAT_VER && new_cluster.controldata.cat_ver >= VISIBILITY_MAP_FROZEN_BIT_CAT_VER) {
    vm_must_add_frozenbit = true;
  }

  for (mapnum = 0; mapnum < size; mapnum++) {
    if (old_tablespace == NULL || strcmp(maps[mapnum].old_tablespace, old_tablespace) == 0) {
...
transfer_relfile(FileNameMap *map, const char *type_suffix, bool vm_must_add_frozenbit)
{
  ...
  for (segno = 0;; segno++) {
    ...
    snprintf(old_file, sizeof(old_file), "%s%s/%u/%u%s%s", map->old_tablespace, map->old_tablespace_suffix, map->old_db_oid, map->old_relfilenode, type_suffix, extent_suffix);
    snprintf(new_file, sizeof(new_file), "%s%s/%u/%u%s%s", map->new_tablespace, map->new_tablespace_suffix, map->new_db_oid, map->new_relfilenode, type_suffix, extent_suffix);
    ...
    if (type_suffix[0] != '\0' || segno != 0) {
      ...
      if (stat(old_file, &statbuf) != 0) {
        ...
        if (errno == ENOENT) {
          ...
          pg_fatal("error while checking for file existence \"%s.%s\" (\"%s\" to \"%s\"): %s\n",map->nspname, map->relname, old_file, new_file, strerror(errno));
        }
      }
