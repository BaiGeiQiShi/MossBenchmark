 ...
 *	this, old/new pg_class.relfilenode values will not match if CLUSTER,*	REINDEX, or VACUUM FULL have been performed in the old cluster.
 *
 ...
 *via SQL commands), that might change at some point in the future.
 */
...
create_new_objects(void)
{
  ...
  /*
   * We cannot process the template1 database concurrently with others,* because when it's transiently dropped, connection attempts would fail.
  ...
  for (dbnum = 0; dbnum < old_cluster.dbarr.ndbs; dbnum++) {
    ...
    exec_prog(log_file_name, NULL, true, true,"\"%s/pg_restore\" %s %s --exit-on-error --verbose --dbname postgres \"%s\"",new_cluster.bindir, cluster_conn_opts(&new_cluster), create_opts, sql_file_name);
  ...
  for (dbnum = 0; dbnum < old_cluster.dbarr.ndbs; dbnum++) {
    ...
    parallel_exec_prog(log_file_name, NULL,"\"%s/pg_restore\" %s %s --exit-on-error --verbose --dbname template1 \"%s\"",new_cluster.bindir, cluster_conn_opts(&new_cluster), create_opts, sql_file_name);
  }
  ...
  /*
   * We don't have minmxids for databases or relations in pre-9.3 clusters,* so set those after we have restored the schema.
   */
...
copy_xact_xlog_xid(void)
{
  ...
  exec_prog(UTILITY_LOG_FILE, NULL, true, true,/* use timeline 1 to match controldata and no WAL history file */
...
set_frozenxids(bool minmxid_only)
{
  ...
  if (!minmxid_only) {
    ...
    PQclear(executeQueryOrDie(conn_template1,"UPDATE pg_catalog.pg_database SET	datfrozenxid = '%u'",old_cluster.controldata.chkpnt_nxtxid));
  }
  ...
  for (dbnum = 0; dbnum < ntups; dbnum++) {
     ...
     * template0, because autovacuum increments their datfrozenxids,* relfrozenxids, and relminmxid even if autovacuum is turned off, and
     * even though all the data rows are already frozen.  To enable this,* we temporarily change datallowconn.
     */
    ...
    if (!minmxid_only) {
      ...
      PQclear(executeQueryOrDie(conn,"UPDATE	pg_catalog.pg_class SET	relfrozenxid = '%u' "
          /* only heap, materialized view, and TOAST are
             vacuumed */
          "WHERE	relkind IN (" CppAsString2(RELKIND_RELATION) ", " CppAsString2(RELKIND_MATVIEW) ", " CppAsString2(RELKIND_TOASTVALUE) ")",old_cluster.controldata.chkpnt_nxtxid));
    }

    /* set pg_class.relminmxid */
    PQclear(executeQueryOrDie(conn,"UPDATE	pg_catalog.pg_class SET	relminmxid = '%u' "
        /* only heap, materialized view, and TOAST are vacuumed */
        "WHERE	relkind IN (" CppAsString2(RELKIND_RELATION) ", " CppAsString2(RELKIND_MATVIEW) ", " CppAsString2(RELKIND_TOASTVALUE) ")",old_cluster.controldata.chkpnt_nxtmulti));
    PQfinish(conn);
