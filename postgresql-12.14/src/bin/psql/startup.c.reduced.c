...
#define PARAMS_ARRAY_SIZE 8
    ...
    values[4] = (options.list_dbs && options.dbname == NULL) ? "postgres"
                                                             : options.dbname;
    ...
    if (PQstatus(pset.db) == CONNECTION_BAD &&
        PQconnectionNeedsPassword(pset.db) && !have_password &&
  ...
  if (options.actions.head != NULL) {
    ...
    for (cell = options.actions.head; cell; cell = cell->next) {
      if (cell->action == ACT_SINGLE_QUERY) {
        ...
        psql_scan_setup(scan_state, cell->val, strlen(cell->val), pset.encoding,
                        standard_strings());
        cond_stack = conditional_stack_create();
        psql_scan_set_passthrough(scan_state, (void *)cond_stack);

        successResult = HandleSlashCmds(scan_state, cond_stack, NULL, NULL) !=
                                PSQL_CMD_ERROR
                            ? EXIT_SUCCESS
                            : EXIT_FAILURE;
...
static void parse_psql_options(int argc, char *argv[],
  ...
  while ((c = getopt_long(argc, argv,
    ...
    case 'c':
      if (optarg[0] == '\\') {
        simple_action_list_append(&options->actions, ACT_SINGLE_SLASH,
                                  optarg + 1);
      } else {
  ...
  if (pg_strcasecmp(newval, "interactive") == 0) {
    ...
    if (ParseVariableBool(newval, NULL, &on_off)) {
      pset.on_error_rollback =
          on_off ? PSQL_ERROR_ROLLBACK_ON : PSQL_ERROR_ROLLBACK_OFF;
    } else {
...
static void EstablishVariableSpace(void) {
  ...
  SetVariableHooks(pset.vars, "AUTOCOMMIT", bool_substitute_hook,
                   autocommit_hook);
  SetVariableHooks(pset.vars, "ON_ERROR_STOP", bool_substitute_hook,
                   on_error_stop_hook);
  SetVariableHooks(pset.vars, "QUIET", bool_substitute_hook, quiet_hook);
  SetVariableHooks(pset.vars, "SINGLELINE", bool_substitute_hook,
                   singleline_hook);
  SetVariableHooks(pset.vars, "SINGLESTEP", bool_substitute_hook,
                   singlestep_hook);
  SetVariableHooks(pset.vars, "FETCH_COUNT", fetch_count_substitute_hook,
                   fetch_count_hook);
  SetVariableHooks(pset.vars, "HISTFILE", NULL, histfile_hook);
  SetVariableHooks(pset.vars, "HISTSIZE", histsize_substitute_hook,
                   histsize_hook);
  SetVariableHooks(pset.vars, "IGNOREEOF", ignoreeof_substitute_hook,
                   ignoreeof_hook);
  ...
  SetVariableHooks(pset.vars, "ECHO_HIDDEN", bool_substitute_hook,
                   echo_hidden_hook);
  SetVariableHooks(pset.vars, "ON_ERROR_ROLLBACK", bool_substitute_hook,
                   on_error_rollback_hook);
  SetVariableHooks(pset.vars, "COMP_KEYWORD_CASE",
                   comp_keyword_case_substitute_hook, comp_keyword_case_hook);
  SetVariableHooks(pset.vars, "HISTCONTROL", histcontrol_substitute_hook,
                   histcontrol_hook);
  ...
  SetVariableHooks(pset.vars, "VERBOSITY", verbosity_substitute_hook,
                   verbosity_hook);
  SetVariableHooks(pset.vars, "SHOW_CONTEXT", show_context_substitute_hook,
                   show_context_hook);
  SetVariableHooks(pset.vars, "HIDE_TABLEAM", bool_substitute_hook,
                   hide_tableam_hook);
}
