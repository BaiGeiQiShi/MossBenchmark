...
static struct copy_options *parse_slash_copy(const char *args) {
  ...
  token =
      strtokx(args, whitespace, ".,()", "\"", 0, false, false, pset.encoding);
  if (!token) {
    goto error;
  }
  ...
  if (pg_strcasecmp(token, "binary") == 0) {
    ...
    token =
        strtokx(NULL, whitespace, ".,()", "\"", 0, false, false, pset.encoding);
    if (!token) {
      goto error;
    }
  }
  ...
  if (token[0] == '(') {
    ...
    while (parens > 0) {
      ...
      xstrcat(&result->before_tofrom, token);
      token = strtokx(NULL, whitespace, "()", "\"'", nonstd_backslash, true,
                      false, pset.encoding);
      if (!token) {
        goto error;
      }
...
bool handleCopyOut(PGconn *conn, FILE *copystream, PGresult **res) {
  ...
  for (;;) {
    ...
    if (ret < 0) {
      break;
    } /* done or server/connection error */
...
bool handleCopyIn(PGconn *conn, FILE *copystream, bool isbinary,
  ...
  if (sigsetjmp(sigint_interrupt_jmp, 1) != 0) {
    ...
    PQputCopyEnd(conn,
                 (PQprotocolVersion(conn) < 3) ? NULL : _("canceled by user"));
   ...
   * protocol version 2.
   */
  if (PQputCopyEnd(conn, (OK || PQprotocolVersion(conn) < 3)
                             ? NULL
...
copyin_cleanup:
  ...
  while (*res = PQgetResult(conn), PQresultStatus(*res) == PGRES_COPY_IN) {
    ...
    PQputCopyEnd(conn, (PQprotocolVersion(conn) < 3)
                           ? NULL
