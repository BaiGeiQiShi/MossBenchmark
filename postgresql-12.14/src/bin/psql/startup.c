...
static void
simple_action_list_append(SimpleActionList *list, enum _actions action, const char *val);
static void
...
#define PARAMS_ARRAY_SIZE 8
    ...
    keywords[4] = "dbname";                       
    values[4] = (options.list_dbs && options.dbname == NULL) ? "postgres" : options.dbname;
  ...
  if (options.actions.head != NULL)
  {
    ...
    for (cell = options.actions.head; cell; cell = cell->next)
    {
      ...
      else if (cell->action == ACT_SINGLE_SLASH)
      {
        ...
        successResult = HandleSlashCmds(scan_state, cond_stack, NULL, NULL) != PSQL_CMD_ERROR ? EXIT_SUCCESS : EXIT_FAILURE;
