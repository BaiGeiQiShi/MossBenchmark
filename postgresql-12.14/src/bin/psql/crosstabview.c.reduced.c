...
static bool printCrosstab(const PGresult *results, int num_columns,
                          pivot_field *piv_columns, int field_for_columns,
                          int num_rows, pivot_field *piv_rows,
                          int field_for_rows, int field_for_data);
static void avlInit(avl_tree *tree);
static void avlMergeValue(avl_tree *tree, char *name, char *sort_value);
static int avlCollectFields(avl_tree *tree, avl_node *node, pivot_field *fields,
                            int idx);
...
bool PrintResultsInCrosstab(const PGresult *res) {
  ...
  if (field_for_columns == field_for_rows) {
    pg_log_error("\\crosstabview: vertical and horizontal headers must be "
                 "different columns");
    goto error_return;
  }
  ...
  if (pset.ctv_args[2] == NULL) {
     ...
     */
    if (PQnfields(res) != 3) {
      pg_log_error("\\crosstabview: data column must be specified when query "
                   "returns more than three columns");
      goto error_return;
    }
  ...
  for (rn = 0; rn < PQntuples(res); rn++) {
    ...
    /* horizontal */
    val = PQgetisnull(res, rn, field_for_columns)
              ? NULL
    ...
    /* vertical */
    val = PQgetisnull(res, rn, field_for_rows)
              ? NULL
   ...
   */
  retval = printCrosstab(res, num_columns, array_columns, field_for_columns,
                         num_rows, array_rows, field_for_rows, field_for_data);
...
static bool printCrosstab(const PGresult *results, int num_columns,
  ...
  printTableAddHeader(&cont, PQfname(results, field_for_rows), false,
  ...
  for (i = 0; i < num_rows; i++) {
    ...
    cont.cells[k * (num_columns + 1)] =
        piv_rows[i].name ? piv_rows[i].name
                         : (popt.nullPrint ? popt.nullPrint : "");
  }
  ...
  for (rn = 0; rn < PQntuples(results); rn++) {
    ...
    rp = (pivot_field *)bsearch(&elt, piv_rows, num_rows, sizeof(pivot_field),
                                pivotFieldCompare);
    Assert(rp != NULL);
    row_number = rp->rank;
    ...
    cp = (pivot_field *)bsearch(&elt, piv_columns, num_columns,
    ...
    if (col_number >= 0 && row_number >= 0) {
      ...
      if (cont.cells[idx] != NULL) {
        pg_log_error(
            "\\crosstabview: query result contains multiple data values for "
            "row \"%s\", column \"%s\"",
...
static void avlInsertNode(avl_tree *tree, avl_node **node, pivot_field field) {
  ...
  if (current == tree->end) {
    ...
    *node = new_node;
    ...
    if (cmp != 0) {
      avlInsertNode(
          tree, cmp > 0 ? &current->children[1] : &current->children[0], field);
