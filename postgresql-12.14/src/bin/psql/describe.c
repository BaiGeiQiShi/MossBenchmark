...
static bool
describeOneTableDetails(const char *schemaname, const char *relationname, const char *oid, bool verbose);
static void
add_tablespace_footer(printTableContent *const cont, char relkind, Oid tablespace, const bool newline);
static void
...
static bool
describeOneTSConfig(const char *oid, const char *nspname, const char *cfgname, const char *pnspname, const char *prsname);
static void
...
describeAggregates(const char *pattern, bool verbose, bool showSystem)
{
  ...
  printfPQExpBuffer(&buf,"SELECT n.nspname as \"%s\",\n  p.proname AS \"%s\",\n  pg_catalog.format_type(p.prorettype, NULL) AS \"%s\",\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Result data type"));

  if (pset.sversion >= 80400) {
    appendPQExpBuffer(&buf,"  CASE WHEN p.pronargs = 0\n    THEN CAST('*' AS pg_catalog.text)\n    ELSE pg_catalog.pg_get_function_arguments(p.oid)\n  END AS \"%s\",\n",gettext_noop("Argument data types"));
  } else if (pset.sversion >= 80200) {
    appendPQExpBuffer(&buf,"  CASE WHEN p.pronargs = 0\n    THEN CAST('*' AS pg_catalog.text)\n    ELSE\n    pg_catalog.array_to_string(ARRAY(\n      SELECT\n        pg_catalog.format_type(p.proargtypes[s.i], NULL)\n      FROM\n        pg_catalog.generate_series(0, pg_catalog.array_upper(p.proargtypes, 1)) AS s(i)\n    ), ', ')\n  END AS \"%s\",\n",gettext_noop("Argument data types"));
  } else {
    appendPQExpBuffer(&buf, "  pg_catalog.format_type(p.proargtypes[0], NULL) AS \"%s\",\n", gettext_noop("Argument data types"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "p.proname", NULL, "pg_catalog.pg_function_is_visible(p.oid)");
...
describeAccessMethods(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 90600) {
    ...
    pg_log_error("The server (version %s) does not support access methods.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT amname AS \"%s\",\n  CASE amtype WHEN 'i' THEN '%s' WHEN 't' THEN '%s' END AS \"%s\"",gettext_noop("Name"), gettext_noop("Index"), gettext_noop("Table"), gettext_noop("Type"));

  if (verbose) {
    appendPQExpBuffer(&buf,",\n  amhandler AS \"%s\",\n  pg_catalog.obj_description(oid, 'pg_am') AS \"%s\"",gettext_noop("Handler"), gettext_noop("Description"));
  }

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_am\n");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "amname", NULL, NULL);
...
describeTablespaces(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 80000) {
    ...
    pg_log_info("The server (version %s) does not support tablespaces.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  if (pset.sversion >= 90200) {
    printfPQExpBuffer(&buf,"SELECT spcname AS \"%s\",\n  pg_catalog.pg_get_userbyid(spcowner) AS \"%s\",\n  pg_catalog.pg_tablespace_location(oid) AS \"%s\"",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Location"));
  } else {
    printfPQExpBuffer(&buf,"SELECT spcname AS \"%s\",\n  pg_catalog.pg_get_userbyid(spcowner) AS \"%s\",\n  spclocation AS \"%s\"",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Location"));
  }
  ...
  if (verbose && pset.sversion >= 90200) {
    appendPQExpBuffer(&buf,",\n  pg_catalog.pg_size_pretty(pg_catalog.pg_tablespace_size(oid)) AS \"%s\"",gettext_noop("Size"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "spcname", NULL, NULL);
...
describeFunctions(const char *functypes, const char *pattern, bool verbose, bool showSystem)
{
  ...
  if (showProcedure && pset.sversion < 110000) {
    ...
    pg_log_error("\\df does not take a \"%c\" option with server version %s", 'p', formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  if (showWindow && pset.sversion < 80400) {
    ...
    pg_log_error("\\df does not take a \"%c\" option with server version %s", 'w', formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  if (pset.sversion >= 110000) {
    appendPQExpBuffer(&buf,"  pg_catalog.pg_get_function_result(p.oid) as \"%s\",\n  pg_catalog.pg_get_function_arguments(p.oid) as \"%s\",\n CASE p.prokind\n  WHEN 'a' THEN '%s'\n  WHEN 'w' THEN '%s'\n  WHEN 'p' THEN '%s'\n  ELSE '%s'\n END as \"%s\"",gettext_noop("Result data type"), gettext_noop("Argument data types"),/* translator: "agg" is short for "aggregate" */
        gettext_noop("agg"), gettext_noop("window"), gettext_noop("proc"), gettext_noop("func"), gettext_noop("Type"));
  } else if (pset.sversion >= 80400) {
    appendPQExpBuffer(&buf,"  pg_catalog.pg_get_function_result(p.oid) as \"%s\",\n  pg_catalog.pg_get_function_arguments(p.oid) as \"%s\",\n CASE\n  WHEN p.proisagg THEN '%s'\n  WHEN p.proiswindow THEN '%s'\n  WHEN p.prorettype = 'pg_catalog.trigger'::pg_catalog.regtype THEN '%s'\n  ELSE '%s'\n END as \"%s\"",gettext_noop("Result data type"), gettext_noop("Argument data types"),/* translator: "agg" is short for "aggregate" */
        gettext_noop("agg"), gettext_noop("window"), gettext_noop("trigger"), gettext_noop("func"), gettext_noop("Type"));
  } else if (pset.sversion >= 80100) {
    appendPQExpBuffer(&buf,"  CASE WHEN p.proretset THEN 'SETOF ' ELSE '' END ||\n  pg_catalog.format_type(p.prorettype, NULL) as \"%s\",\n  CASE WHEN proallargtypes IS NOT NULL THEN\n    pg_catalog.array_to_string(ARRAY(\n      SELECT\n        CASE\n          WHEN p.proargmodes[s.i] = 'i' THEN ''\n          WHEN p.proargmodes[s.i] = 'o' THEN 'OUT '\n          WHEN p.proargmodes[s.i] = 'b' THEN 'INOUT '\n          WHEN p.proargmodes[s.i] = 'v' THEN 'VARIADIC '\n        END ||\n        CASE\n          WHEN COALESCE(p.proargnames[s.i], '') = '' THEN ''\n          ELSE p.proargnames[s.i] || ' '\n        END ||\n        pg_catalog.format_type(p.proallargtypes[s.i], NULL)\n      FROM\n        pg_catalog.generate_series(1, pg_catalog.array_upper(p.proallargtypes, 1)) AS s(i)\n    ), ', ')\n  ELSE\n    pg_catalog.array_to_string(ARRAY(\n      SELECT\n        CASE\n          WHEN COALESCE(p.proargnames[s.i+1], '') = '' THEN ''\n          ELSE p.proargnames[s.i+1] || ' '\n          END ||\n        pg_catalog.format_type(p.proargtypes[s.i], NULL)\n      FROM\n        pg_catalog.generate_series(0, pg_catalog.array_upper(p.proargtypes, 1)) AS s(i)\n    ), ', ')\n  END AS \"%s\",\n  CASE\n    WHEN p.proisagg THEN '%s'\n    WHEN p.prorettype = 'pg_catalog.trigger'::pg_catalog.regtype THEN '%s'\n    ELSE '%s'\n  END AS \"%s\"",gettext_noop("Result data type"), gettext_noop("Argument data types"),/* translator: "agg" is short for "aggregate" */
        gettext_noop("agg"), gettext_noop("trigger"), gettext_noop("func"), gettext_noop("Type"));
  } else {
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,",\n CASE\n  WHEN p.provolatile = 'i' THEN '%s'\n  WHEN p.provolatile = 's' THEN '%s'\n  WHEN p.provolatile = 'v' THEN '%s'\n END as \"%s\"",gettext_noop("immutable"), gettext_noop("stable"), gettext_noop("volatile"), gettext_noop("Volatility"));
    if (pset.sversion >= 90600) {
      appendPQExpBuffer(&buf,",\n CASE\n  WHEN p.proparallel = 'r' THEN '%s'\n  WHEN p.proparallel = 's' THEN '%s'\n  WHEN p.proparallel = 'u' THEN '%s'\n END as \"%s\"",gettext_noop("restricted"), gettext_noop("safe"), gettext_noop("unsafe"), gettext_noop("Parallel"));
    }
    appendPQExpBuffer(&buf,",\n pg_catalog.pg_get_userbyid(p.proowner) as \"%s\",\n CASE WHEN prosecdef THEN '%s' ELSE '%s' END AS \"%s\"",gettext_noop("Owner"), gettext_noop("definer"), gettext_noop("invoker"), gettext_noop("Security"));
    appendPQExpBufferStr(&buf, ",\n ");
    printACLColumn(&buf, "p.proacl");
    appendPQExpBuffer(&buf,",\n l.lanname as \"%s\",\n p.prosrc as \"%s\",\n pg_catalog.obj_description(p.oid, 'pg_proc') as \"%s\"",gettext_noop("Language"), gettext_noop("Source code"), gettext_noop("Description"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, have_where, false, "n.nspname", "p.proname", NULL, "pg_catalog.pg_function_is_visible(p.oid)");
...
describeTypes(const char *pattern, bool verbose, bool showSystem)
{
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,"  t.typname AS \"%s\",\n  CASE WHEN t.typrelid != 0\n      THEN CAST('tuple' AS pg_catalog.text)\n    WHEN t.typlen < 0\n      THEN CAST('var' AS pg_catalog.text)\n    ELSE CAST(t.typlen AS pg_catalog.text)\n  END AS \"%s\",\n",gettext_noop("Internal name"), gettext_noop("Size"));
  }
  if (verbose && pset.sversion >= 80300) {
    appendPQExpBufferStr(&buf, "  pg_catalog.array_to_string(\n      ARRAY(\n          SELECT e.enumlabel\n          FROM pg_catalog.pg_enum e\n          WHERE e.enumtypid = t.oid\n");

    if (pset.sversion >= 90100) {
      appendPQExpBufferStr(&buf, "          ORDER BY e.enumsortorder\n");
    } else {
      appendPQExpBufferStr(&buf, "          ORDER BY e.oid\n");
    }

    appendPQExpBuffer(&buf,"      ),\n      E'\\n'\n  ) AS \"%s\",\n",gettext_noop("Elements"));
  }
  if (verbose) {
    appendPQExpBuffer(&buf, "  pg_catalog.pg_get_userbyid(t.typowner) AS \"%s\",\n", gettext_noop("Owner"));
  }
  if (verbose && pset.sversion >= 90200) {
  ...
  appendPQExpBufferStr(&buf, "OR (SELECT c.relkind = " CppAsString2(RELKIND_COMPOSITE_TYPE) " FROM pg_catalog.pg_class c WHERE c.oid = t.typrelid))\n");
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "t.typname", "pg_catalog.format_type(t.oid, NULL)", "pg_catalog.pg_type_is_visible(t.oid)");
...
describeOperators(const char *pattern, bool verbose, bool showSystem)
{
  ...
  printfPQExpBuffer(&buf,"SELECT n.nspname as \"%s\",\n  o.oprname AS \"%s\",\n  CASE WHEN o.oprkind='l' THEN NULL ELSE pg_catalog.format_type(o.oprleft, NULL) END AS \"%s\",\n  CASE WHEN o.oprkind='r' THEN NULL ELSE pg_catalog.format_type(o.oprright, NULL) END AS \"%s\",\n  pg_catalog.format_type(o.oprresult, NULL) AS \"%s\",\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Left arg type"), gettext_noop("Right arg type"), gettext_noop("Result type"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, true, "n.nspname", "o.oprname", NULL, "pg_catalog.pg_operator_is_visible(o.oid)");
...
listAllDbs(const char *pattern, bool verbose)
{
  ...
  printfPQExpBuffer(&buf,"SELECT d.datname as \"%s\",\n       pg_catalog.pg_get_userbyid(d.datdba) as \"%s\",\n       pg_catalog.pg_encoding_to_char(d.encoding) as \"%s\",\n",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Encoding"));
  if (pset.sversion >= 80400) {
    appendPQExpBuffer(&buf,"       d.datcollate as \"%s\",\n       d.datctype as \"%s\",\n",gettext_noop("Collate"), gettext_noop("Ctype"));
  }
  appendPQExpBufferStr(&buf, "       ");
  printACLColumn(&buf, "d.datacl");
  if (verbose && pset.sversion >= 80200) {
    appendPQExpBuffer(&buf,",\n       CASE WHEN pg_catalog.has_database_privilege(d.datname, 'CONNECT')\n            THEN pg_catalog.pg_size_pretty(pg_catalog.pg_database_size(d.datname))\n            ELSE 'No Access'\n       END as \"%s\"",gettext_noop("Size"));
  }
  if (verbose && pset.sversion >= 80000) {
    appendPQExpBuffer(&buf, ",\n       t.spcname as \"%s\"", gettext_noop("Tablespace"));
  }
  if (verbose && pset.sversion >= 80200) {
    appendPQExpBuffer(&buf,",\n       pg_catalog.shobj_description(d.oid, 'pg_database') as \"%s\"",gettext_noop("Description"));
  }
  ...
  if (pattern) {
    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "d.datname", NULL, NULL);
  }
...
permissionsList(const char *pattern)
{
   ...
   * pg_catalog.*
   */
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "c.relname", NULL, "n.nspname !~ '^pg_' AND pg_catalog.pg_table_is_visible(c.oid)");
...
listDefaultACLs(const char *pattern)
{
  ...
  if (pset.sversion < 90000) {
    ...
    pg_log_error("The server (version %s) does not support altering default privileges.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT pg_catalog.pg_get_userbyid(d.defaclrole) AS \"%s\",\n  n.nspname AS \"%s\",\n  CASE d.defaclobjtype WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' END AS \"%s\",\n  ",gettext_noop("Owner"), gettext_noop("Schema"), DEFACLOBJ_RELATION, gettext_noop("table"), DEFACLOBJ_SEQUENCE, gettext_noop("sequence"), DEFACLOBJ_FUNCTION, gettext_noop("function"), DEFACLOBJ_TYPE, gettext_noop("type"), DEFACLOBJ_NAMESPACE, gettext_noop("schema"), gettext_noop("Type"));

  printACLColumn(&buf, "d.defaclacl");

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_default_acl d\n     LEFT JOIN pg_catalog.pg_namespace n ON n.oid = d.defaclnamespace\n");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "n.nspname", "pg_catalog.pg_get_userbyid(d.defaclrole)", NULL);
 ...
 * types of objects will be displayed: constraint, operator class,* operator family, rule, and trigger.
 *
 */
bool
objectDescription(const char *pattern, bool showSystem)
{
  ...
  processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, false, "n.nspname", "pgc.conname", NULL, "pg_catalog.pg_table_is_visible(c.oid)");
  ...
  processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, false, "n.nspname", "pgc.conname", NULL, "pg_catalog.pg_type_is_visible(t.oid)");
  ...
  if (pset.sversion >= 80300) {
    ...
    processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "o.opcname", NULL, "pg_catalog.pg_opclass_is_visible(o.oid)");
  }

  /*
   * although operator family comments have been around since 8.3,* pg_opfamily_is_visible is only available in 9.2+
   */
  if (pset.sversion >= 90200) {
    ...
    processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "opf.opfname", NULL, "pg_catalog.pg_opfamily_is_visible(opf.oid)");
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "r.rulename", NULL, "pg_catalog.pg_table_is_visible(c.oid)");
  ...
  processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, false, "n.nspname", "t.tgname", NULL, "pg_catalog.pg_table_is_visible(c.oid)");

  appendPQExpBufferStr(&buf, ") AS tt\n  JOIN pg_catalog.pg_description d ON (tt.oid = d.objoid AND tt.tableoid = d.classoid AND d.objsubid = 0)\n");
...
describeTableDetails(const char *pattern, bool verbose, bool showSystem)
{
  ...
  printfPQExpBuffer(&buf, "SELECT c.oid,\n  n.nspname,\n  c.relname\nFROM pg_catalog.pg_class c\n     LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace\n");

  if (!showSystem && !pattern) {
    appendPQExpBufferStr(&buf, "WHERE n.nspname <> 'pg_catalog'\n      AND n.nspname <> 'information_schema'\n");
  }

  processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, false, "n.nspname", "c.relname", NULL, "pg_catalog.pg_table_is_visible(c.oid)");
...
describeOneTableDetails(const char *schemaname, const char *relationname, const char *oid, bool verbose)
{
  ...
  int attname_col = -1, /* column indexes in "res" */
      atttype_col = -1, attrdef_col = -1, attnotnull_col = -1, attcoll_col = -1, attidentity_col = -1, attgenerated_col = -1, isindexkey_col = -1, indexdef_col = -1, fdwopts_col = -1, attstorage_col = -1, attstattarget_col = -1, attdescr_col = -1;
  int numrows;
  struct {
    int16 checks;
    char relkind;
  ...
  if (pset.sversion >= 120000) {
    ...
    printfPQExpBuffer(&buf,"SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules, c.relhastriggers, c.relrowsecurity, c.relforcerowsecurity, c.relhasoids, c.relispartition, %s, c.reltablespace, CASE WHEN c.reloftype = 0 THEN '' ELSE c.reloftype::pg_catalog.regtype::pg_catalog.text END, c.relpersistence, c.relreplident\nFROM pg_catalog.pg_class c\n LEFT JOIN pg_catalog.pg_class tc ON (c.reltoastrelid = tc.oid)\nWHERE c.oid = '%s';",(verbose ? "pg_catalog.array_to_string(c.reloptions || array(select 'toast.' || x from pg_catalog.unnest(tc.reloptions) x), ', ')\n"
                 : "''"),oid);
    ...
    printfPQExpBuffer(&buf,"SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules, c.relhastriggers, false, false, c.relhasoids, false as relispartition, %s, c.reltablespace, CASE WHEN c.reloftype = 0 THEN '' ELSE c.reloftype::pg_catalog.regtype::pg_catalog.text END, c.relpersistence, c.relreplident\nFROM pg_catalog.pg_class c\n LEFT JOIN pg_catalog.pg_class tc ON (c.reltoastrelid = tc.oid)\nWHERE c.oid = '%s';",(verbose ? "pg_catalog.array_to_string(c.reloptions || array(select 'toast.' || x from pg_catalog.unnest(tc.reloptions) x), ', ')\n"
                 : "''"),oid);
  } else if (pset.sversion >= 90100) {
    printfPQExpBuffer(&buf,"SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules, c.relhastriggers, false, false, c.relhasoids, false as relispartition, %s, c.reltablespace, CASE WHEN c.reloftype = 0 THEN '' ELSE c.reloftype::pg_catalog.regtype::pg_catalog.text END, c.relpersistence\nFROM pg_catalog.pg_class c\n LEFT JOIN pg_catalog.pg_class tc ON (c.reltoastrelid = tc.oid)\nWHERE c.oid = '%s';",(verbose ? "pg_catalog.array_to_string(c.reloptions || array(select 'toast.' || x from pg_catalog.unnest(tc.reloptions) x), ', ')\n"
                 : "''"),oid);
  } else if (pset.sversion >= 90000) {
    printfPQExpBuffer(&buf,"SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules, c.relhastriggers, false, false, c.relhasoids, false as relispartition, %s, c.reltablespace, CASE WHEN c.reloftype = 0 THEN '' ELSE c.reloftype::pg_catalog.regtype::pg_catalog.text END\nFROM pg_catalog.pg_class c\n LEFT JOIN pg_catalog.pg_class tc ON (c.reltoastrelid = tc.oid)\nWHERE c.oid = '%s';",(verbose ? "pg_catalog.array_to_string(c.reloptions || array(select 'toast.' || x from pg_catalog.unnest(tc.reloptions) x), ', ')\n"
                 : "''"),oid);
  } else if (pset.sversion >= 80400) {
    printfPQExpBuffer(&buf,"SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules, c.relhastriggers, false, false, c.relhasoids, false as relispartition, %s, c.reltablespace\nFROM pg_catalog.pg_class c\n LEFT JOIN pg_catalog.pg_class tc ON (c.reltoastrelid = tc.oid)\nWHERE c.oid = '%s';",(verbose ? "pg_catalog.array_to_string(c.reloptions || array(select 'toast.' || x from pg_catalog.unnest(tc.reloptions) x), ', ')\n"
                 : "''"),oid);
  } else if (pset.sversion >= 80200) {
    printfPQExpBuffer(&buf,"SELECT relchecks, relkind, relhasindex, relhasrules, reltriggers <> 0, false, false, relhasoids, false as relispartition, %s, reltablespace\nFROM pg_catalog.pg_class WHERE oid = '%s';",(verbose ? "pg_catalog.array_to_string(reloptions, E', ')" : "''"), oid);
  } else if (pset.sversion >= 80000) {
    printfPQExpBuffer(&buf,"SELECT relchecks, relkind, relhasindex, relhasrules, reltriggers <> 0, false, false, relhasoids, false as relispartition, '', reltablespace\nFROM pg_catalog.pg_class WHERE oid = '%s';",oid);
  } else {
    printfPQExpBuffer(&buf,"SELECT relchecks, relkind, relhasindex, relhasrules, reltriggers <> 0, false, false, relhasoids, false as relispartition, '', ''\nFROM pg_catalog.pg_class WHERE oid = '%s';",oid);
  }
  ...
  tableinfo.reloptions = (pset.sversion >= 80200) ? pg_strdup(PQgetvalue(res, 0, 9)) : NULL;
  tableinfo.tablespace = (pset.sversion >= 80000) ? atooid(PQgetvalue(res, 0, 10)) : 0;
  tableinfo.reloftype = (pset.sversion >= 90000 && strcmp(PQgetvalue(res, 0, 11), "") != 0) ? pg_strdup(PQgetvalue(res, 0, 11)) : NULL;
  tableinfo.relpersistence = (pset.sversion >= 90100) ? *(PQgetvalue(res, 0, 12)) : 0;
  tableinfo.relreplident = (pset.sversion >= 90400) ? *(PQgetvalue(res, 0, 13)) : 'd';
  if (pset.sversion >= 120000) {
    tableinfo.relam = PQgetisnull(res, 0, 14) ? (char *)NULL : pg_strdup(PQgetvalue(res, 0, 14));
  } else {
  ...
  if (tableinfo.relkind == RELKIND_SEQUENCE) {
    ...
    if (pset.sversion >= 100000) {
      printfPQExpBuffer(&buf,"SELECT pg_catalog.format_type(seqtypid, NULL) AS \"%s\",\n       seqstart AS \"%s\",\n       seqmin AS \"%s\",\n       seqmax AS \"%s\",\n       seqincrement AS \"%s\",\n       CASE WHEN seqcycle THEN '%s' ELSE '%s' END AS \"%s\",\n       seqcache AS \"%s\"\n",gettext_noop("Type"), gettext_noop("Start"), gettext_noop("Minimum"), gettext_noop("Maximum"), gettext_noop("Increment"), gettext_noop("yes"), gettext_noop("no"), gettext_noop("Cycles?"), gettext_noop("Cache"));
    ...
    if (!result) {
      ...
      case 'a':
        footers[0] = psprintf(_("Owned by: %s"), PQgetvalue(result, 0, 0));
        break;
      case 'i':
        footers[0] = psprintf(_("Sequence for identity column: %s"), PQgetvalue(result, 0, 0));
        break;
      }
    }
    PQclear(result);
  ...
  if (tableinfo.relkind == RELKIND_INDEX || tableinfo.relkind == RELKIND_PARTITIONED_INDEX) {
    if (pset.sversion >= 110000) {
      appendPQExpBuffer(&buf,",\n  CASE WHEN a.attnum <= (SELECT i.indnkeyatts FROM pg_catalog.pg_index i WHERE i.indexrelid = '%s') THEN '%s' ELSE '%s' END AS is_key",oid, gettext_noop("yes"), gettext_noop("no"));
      isindexkey_col = cols++;
    }
  ...
  case RELKIND_PARTITIONED_INDEX:
    if (tableinfo.relpersistence == 'u') {
      printfPQExpBuffer(&title, _("Unlogged partitioned index \"%s.%s\""), schemaname, relationname);
    } else {
      printfPQExpBuffer(&title, _("Partitioned index \"%s.%s\""), schemaname, relationname);
    }
    break;
  case 's':
  ...
  case RELKIND_PARTITIONED_TABLE:
    if (tableinfo.relpersistence == 'u') {
      printfPQExpBuffer(&title, _("Unlogged partitioned table \"%s.%s\""), schemaname, relationname);
    } else {
      printfPQExpBuffer(&title, _("Partitioned table \"%s.%s\""), schemaname, relationname);
    }
    break;
  default:;
  ...
  for (i = 0; i < numrows; i++) {
    ...
    if (attstorage_col >= 0) {
      ...
      printTableAddCell(&cont, (storage[0] == 'p' ? "plain" : (storage[0] == 'm' ? "main" : (storage[0] == 'x' ? "extended" : (storage[0] == 'e' ? "external" : "???")))), false, false);
    }
  ...
  if (tableinfo.ispartition) {
    ...
    if (PQntuples(result) > 0) {
      ...
      printfPQExpBuffer(&tmpbuf, _("Partition of: %s %s"), parent_name, partdef);
      ...
      if (verbose) {
        ...
        if (partconstraintdef == NULL || partconstraintdef[0] == '\0') {
          ...
          printfPQExpBuffer(&tmpbuf, _("Partition constraint: %s"), partconstraintdef);
        }
        printTableAddFooter(&cont, tmpbuf.data);
      }
    }
  ...
  if (tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
    ...
    printfPQExpBuffer(&buf, "SELECT pg_catalog.pg_get_partkeydef('%s'::pg_catalog.oid);", oid);
    result = PSQLexec(buf.data);
    if (!result) {
      goto error_return;
    }
  ...
  if (tableinfo.relkind == RELKIND_INDEX || tableinfo.relkind == RELKIND_PARTITIONED_INDEX) {
    ...
    if (pset.sversion >= 90000) {
      appendPQExpBufferStr(&buf, "  (NOT i.indimmediate) AND EXISTS (SELECT 1 FROM pg_catalog.pg_constraint WHERE conrelid = i.indrelid AND conindid = i.indexrelid AND contype IN ('p','u','x') AND condeferrable) AS condeferrable,\n  (NOT i.indimmediate) AND EXISTS (SELECT 1 FROM pg_catalog.pg_constraint WHERE conrelid = i.indrelid AND conindid = i.indexrelid AND contype IN ('p','u','x') AND condeferred) AS condeferred,\n");
    } else {
      appendPQExpBufferStr(&buf, "  false AS condeferrable, false AS condeferred,\n");
    }
    ...
    PQclear(result);
  } else if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_MATVIEW || tableinfo.relkind == RELKIND_FOREIGN_TABLE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
    ...
    if (tableinfo.hasindex) {
      ...
      if (pset.sversion >= 90000) {
        appendPQExpBufferStr(&buf, "pg_catalog.pg_get_constraintdef(con.oid, true), contype, condeferrable, condeferred");
      } else {
        appendPQExpBufferStr(&buf, "null AS constraintdef, null AS contype, false AS condeferrable, false AS condeferred");
      }
      if (pset.sversion >= 90400) {
      ...
      if (pset.sversion >= 90000) {
        appendPQExpBufferStr(&buf, "  LEFT JOIN pg_catalog.pg_constraint con ON (conrelid = i.indrelid AND conindid = i.indexrelid AND contype IN ('p','u','x'))\n");
      }
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          if (pset.sversion >= 80000) {
            add_tablespace_footer(&cont, RELKIND_INDEX, atooid(PQgetvalue(result, i, 11)), false);
          }
        }
      }
      PQclear(result);
    ...
    /*
     * Print foreign-key constraints (there are none if no triggers,* except if the table is partitioned, in which case the triggers
     * appear in the partitions)
     */
    if (tableinfo.hastriggers || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      if (pset.sversion >= 120000 && (tableinfo.ispartition || tableinfo.relkind == RELKIND_PARTITIONED_TABLE)) {
        /*
         * Put the constraints defined in this table first, followed
         * by the constraints defined in ancestor partitioned tables.
         */
        printfPQExpBuffer(&buf,"SELECT conrelid = '%s'::pg_catalog.regclass AS sametable,\n       conname,\n       pg_catalog.pg_get_constraintdef(oid, true) AS condef,\n       conrelid::pg_catalog.regclass AS ontable\n  FROM pg_catalog.pg_constraint,\n       pg_catalog.pg_partition_ancestors('%s')\n WHERE conrelid = relid AND contype = 'f' AND conparentid = 0\nORDER BY sametable DESC, conname;",oid, oid);
      } else {
        printfPQExpBuffer(&buf,"SELECT true as sametable, conname,\n  pg_catalog.pg_get_constraintdef(r.oid, true) as condef,\n  conrelid::pg_catalog.regclass AS ontable\nFROM pg_catalog.pg_constraint r\nWHERE r.conrelid = '%s' AND r.contype = 'f'\n",oid);
      ...
      if (tuples > 0) {
        int i_sametable = PQfnumber(result, "sametable"), i_conname = PQfnumber(result, "conname"), i_condef = PQfnumber(result, "condef"), i_ontable = PQfnumber(result, "ontable");
        ...
        for (i = 0; i < tuples; i++) {
          ...
          if (strcmp(PQgetvalue(result, i, i_sametable), "f") == 0) {
            printfPQExpBuffer(&buf, "    TABLE \"%s\" CONSTRAINT \"%s\" %s", PQgetvalue(result, i, i_ontable), PQgetvalue(result, i, i_conname), PQgetvalue(result, i, i_condef));
          } else {
            printfPQExpBuffer(&buf, "    \"%s\" %s", PQgetvalue(result, i, i_conname), PQgetvalue(result, i, i_condef));
          }
    ...
    if (tableinfo.hastriggers || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      ...
      if (tuples > 0) {
        int i_conname = PQfnumber(result, "conname"), i_ontable = PQfnumber(result, "ontable"), i_condef = PQfnumber(result, "condef");
    ...
    if (pset.sversion >= 90500) {
      ...
      for (i = 0; i < tuples; i++) {
        ...
        if (!PQgetisnull(result, i, 5)) {
          appendPQExpBuffer(&buf, " FOR %s", PQgetvalue(result, i, 5));
        }
        ...
        if (!PQgetisnull(result, i, 4)) {
          appendPQExpBuffer(&buf, "\n      WITH CHECK (%s)", PQgetvalue(result, i, 4));
        }
    ...
    if (pset.sversion >= 100000) {
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          appendPQExpBuffer(&buf, "\"%s\".\"%s\" (", PQgetvalue(result, i, 2), PQgetvalue(result, i, 3));
          ...
          appendPQExpBuffer(&buf, ") ON %s FROM %s", PQgetvalue(result, i, 4), PQgetvalue(result, i, 1));
    ...
    if (tableinfo.hasrules && tableinfo.relkind != RELKIND_MATVIEW) {
      if (pset.sversion >= 80300) {
        printfPQExpBuffer(&buf,"SELECT r.rulename, trim(trailing ';' from pg_catalog.pg_get_ruledef(r.oid, true)), ev_enabled\nFROM pg_catalog.pg_rewrite r\nWHERE r.ev_class = '%s' ORDER BY 1;",oid);
      } else {
        printfPQExpBuffer(&buf,"SELECT r.rulename, trim(trailing ';' from pg_catalog.pg_get_ruledef(r.oid, true)), 'O' AS ev_enabled\nFROM pg_catalog.pg_rewrite r\nWHERE r.ev_class = '%s' ORDER BY 1;",oid);
      }
      result = PSQLexec(buf.data);
      if (!result) {
        goto error_return;
    ...
    if (pset.sversion >= 100000) {
      printfPQExpBuffer(&buf,"SELECT pubname\nFROM pg_catalog.pg_publication p\nJOIN pg_catalog.pg_publication_rel pr ON p.oid = pr.prpubid\nWHERE pr.prrelid = '%s'\nUNION ALL\nSELECT pubname\nFROM pg_catalog.pg_publication p\nWHERE p.puballtables AND pg_catalog.pg_relation_is_publishable('%s')\nORDER BY 1;",oid, oid);
  ...
  if (tableinfo.hastriggers) {
    ...
    printfPQExpBuffer(&buf,"SELECT t.tgname, pg_catalog.pg_get_triggerdef(t.oid%s), t.tgenabled, %s\nFROM pg_catalog.pg_trigger t\nWHERE t.tgrelid = '%s' AND ",(pset.sversion >= 90000 ? ", true" : ""),(pset.sversion >= 90000      ? "t.tgisinternal"
            : pset.sversion >= 80300 ? "t.tgconstraint <> 0 AS tgisinternal"
    ...
    if (pset.sversion >= 110000) {
      ...
      appendPQExpBufferStr(&buf, "(NOT tgisconstraint  OR NOT EXISTS  (SELECT 1 FROM pg_catalog.pg_depend d    JOIN pg_catalog.pg_constraint c ON (d.refclassid = c.tableoid AND d.refobjid = c.oid)    WHERE d.classid = t.tableoid AND d.objid = t.oid AND d.deptype = 'i' AND c.contype = 'f'))");
    }
    appendPQExpBufferStr(&buf, "\nORDER BY 1;");
    ...
    if (tuples > 0) {
      ...
      /*
       * split the output into 4 different categories. Enabled triggers,* disabled triggers and the two special ALWAYS and REPLICA
       * configurations.
       */
      for (category = 0; category <= 4; category++) {
        have_heading = false;
   ...
   */
  if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_MATVIEW || tableinfo.relkind == RELKIND_FOREIGN_TABLE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
    PGresult *result;
    int tuples;
    ...
    printfPQExpBuffer(&buf,"SELECT c.oid::pg_catalog.regclass FROM pg_catalog.pg_class c, pg_catalog.pg_inherits i WHERE c.oid=i.inhparent AND i.inhrelid = '%s' AND c.relkind != " CppAsString2(RELKIND_PARTITIONED_TABLE) " ORDER BY inhseqno;",oid);
    ...
    if (pset.sversion >= 100000) {
      printfPQExpBuffer(&buf,"SELECT c.oid::pg_catalog.regclass,       pg_catalog.pg_get_expr(c.relpartbound, c.oid),       c.relkind FROM pg_catalog.pg_class c, pg_catalog.pg_inherits i WHERE c.oid=i.inhrelid AND i.inhparent = '%s' ORDER BY pg_catalog.pg_get_expr(c.relpartbound, c.oid) = 'DEFAULT',          c.oid::pg_catalog.regclass::pg_catalog.text;",oid);
    } else if (pset.sversion >= 80300) {
    ...
    if (tableinfo.relkind == RELKIND_PARTITIONED_TABLE && tuples == 0) {
      ...
      const char *ct = (tableinfo.relkind != RELKIND_PARTITIONED_TABLE) ? _("Child tables") : _("Partitions");
      ...
      for (i = 0; i < tuples; i++) {
        if (tableinfo.relkind != RELKIND_PARTITIONED_TABLE) {
          ...
          if (i == 0) {
            printfPQExpBuffer(&buf, "%s: %s %s%s", ct, PQgetvalue(result, i, 0), PQgetvalue(result, i, 1), partitioned_note);
          } else {
            printfPQExpBuffer(&buf, "%*s  %s %s%s", ctw, "", PQgetvalue(result, i, 0), PQgetvalue(result, i, 1), partitioned_note);
          }
        }
        if (i < tuples - 1) {
          appendPQExpBufferChar(&buf, ',');
    ...
    if (verbose && (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_MATVIEW) &&
      ...
      printfPQExpBuffer(&buf, "%s: %s", s, tableinfo.relreplident == 'f' ? "FULL" : tableinfo.relreplident == 'n' ? "NOTHING" : "???");
...
add_tablespace_footer(printTableContent *const cont, char relkind, Oid tablespace, const bool newline)
{
  ...
  if (relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW || relkind == RELKIND_INDEX || relkind == RELKIND_PARTITIONED_TABLE || relkind == RELKIND_PARTITIONED_INDEX) {
    /*
...
describeRoles(const char *pattern, bool verbose, bool showSystem)
{
  ...
  if (pset.sversion >= 80100) {
    printfPQExpBuffer(&buf, "SELECT r.rolname, r.rolsuper, r.rolinherit,\n  r.rolcreaterole, r.rolcreatedb, r.rolcanlogin,\n  r.rolconnlimit, r.rolvaliduntil,\n  ARRAY(SELECT b.rolname\n        FROM pg_catalog.pg_auth_members m\n        JOIN pg_catalog.pg_roles b ON (m.roleid = b.oid)\n        WHERE m.member = r.oid) as memberof");
    ...
    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "r.rolname", NULL, NULL);
  } else {
    printfPQExpBuffer(&buf, "SELECT u.usename AS rolname,\n  u.usesuper AS rolsuper,\n  true AS rolinherit, false AS rolcreaterole,\n  u.usecreatedb AS rolcreatedb, true AS rolcanlogin,\n  -1 AS rolconnlimit,  u.valuntil as rolvaliduntil,\n  ARRAY(SELECT g.groname FROM pg_catalog.pg_group g WHERE u.usesysid = ANY(g.grolist)) as memberof\nFROM pg_catalog.pg_user u\n");

    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "u.usename", NULL, NULL);
  }
...
listDbRoleSettings(const char *pattern, const char *pattern2)
{
  ...
  if (pset.sversion < 90000) {
    ...
    pg_log_error("The server (version %s) does not support per-database role settings.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT rolname AS \"%s\", datname AS \"%s\",\npg_catalog.array_to_string(setconfig, E'\\n') AS \"%s\"\nFROM pg_catalog.pg_db_role_setting s\nLEFT JOIN pg_catalog.pg_database d ON d.oid = setdatabase\nLEFT JOIN pg_catalog.pg_roles r ON r.oid = setrole\n",gettext_noop("Role"), gettext_noop("Database"), gettext_noop("Settings"));
  havewhere = processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "r.rolname", NULL, NULL);
  processSQLNamePattern(pset.db, &buf, pattern2, havewhere, false, NULL, "d.datname", NULL, NULL);
  appendPQExpBufferStr(&buf, "ORDER BY 1, 2;");
...
listTables(const char *tabtypes, const char *pattern, bool verbose, bool showSystem)
{
   ...
   */
  printfPQExpBuffer(&buf,"SELECT n.nspname as \"%s\",\n  c.relname as \"%s\",\n  CASE c.relkind WHEN " CppAsString2(RELKIND_RELATION) " THEN '%s' WHEN " CppAsString2(RELKIND_VIEW) " THEN '%s' WHEN " CppAsString2(RELKIND_MATVIEW) " THEN '%s' WHEN " CppAsString2(RELKIND_INDEX) " THEN '%s' WHEN " CppAsString2(RELKIND_SEQUENCE) " THEN '%s' WHEN 's' THEN '%s' WHEN " CppAsString2(RELKIND_FOREIGN_TABLE) " THEN '%s' WHEN " CppAsString2(RELKIND_PARTITIONED_TABLE) " THEN '%s' WHEN " CppAsString2(RELKIND_PARTITIONED_INDEX) " THEN '%s' END as \"%s\",\n  pg_catalog.pg_get_userbyid(c.relowner) as \"%s\"",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("table"), gettext_noop("view"), gettext_noop("materialized view"), gettext_noop("index"), gettext_noop("sequence"), gettext_noop("special"), gettext_noop("foreign table"), gettext_noop("partitioned table"), gettext_noop("partitioned index"), gettext_noop("Type"), gettext_noop("Owner"));
  ...
  if (verbose) {
     ...
     */
    if (pset.sversion >= 90000) {
      appendPQExpBuffer(&buf,",\n  pg_catalog.pg_size_pretty(pg_catalog.pg_table_size(c.oid)) as \"%s\"",gettext_noop("Size"));
    } else if (pset.sversion >= 80100) {
      appendPQExpBuffer(&buf,",\n  pg_catalog.pg_size_pretty(pg_catalog.pg_relation_size(c.oid)) as \"%s\"",gettext_noop("Size"));
    }
  ...
  appendPQExpBufferStr(&buf, "\nWHERE c.relkind IN (");
  if (showTables) {
    appendPQExpBufferStr(&buf, CppAsString2(RELKIND_RELATION) "," CppAsString2(RELKIND_PARTITIONED_TABLE) ",");
  }
  if (showViews) {
  ...
  if (showIndexes) {
    appendPQExpBufferStr(&buf, CppAsString2(RELKIND_INDEX) "," CppAsString2(RELKIND_PARTITIONED_INDEX) ",");
  }
  if (showSeq) {
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "c.relname", NULL, "pg_catalog.pg_table_is_visible(c.oid)");
...
listPartitionedTables(const char *reltypes, const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 100000) {
    ...
    pg_log_error("The server (version %s) does not support declarative table partitioning.",formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  printfPQExpBuffer(&buf,"SELECT n.nspname as \"%s\",\n  c.relname as \"%s\",\n  pg_catalog.pg_get_userbyid(c.relowner) as \"%s\"",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Owner"));

  if (mixed_output) {
    appendPQExpBuffer(&buf,",\n  CASE c.relkind WHEN " CppAsString2(RELKIND_PARTITIONED_TABLE) " THEN '%s' WHEN " CppAsString2(RELKIND_PARTITIONED_INDEX) " THEN '%s' END as \"%s\"",gettext_noop("partitioned table"), gettext_noop("partitioned index"), gettext_noop("Type"));

    translate_columns[3] = true;
  }

  if (showNested || pattern) {
    appendPQExpBuffer(&buf, ",\n  inh.inhparent::pg_catalog.regclass as \"%s\"", gettext_noop("Parent name"));
  }

  if (showIndexes) {
    appendPQExpBuffer(&buf, ",\n c2.oid::pg_catalog.regclass as \"%s\"", gettext_noop("Table"));
  }

  if (verbose) {
    if (showNested) {
      appendPQExpBuffer(&buf, ",\n  s.dps as \"%s\"", gettext_noop("Leaf partition size"));
      appendPQExpBuffer(&buf, ",\n  s.tps as \"%s\"", gettext_noop("Total size"));
    } else {
  ...
  appendPQExpBuffer(&buf, "ORDER BY \"Schema\", %s%s\"Name\";", mixed_output ? "\"Type\" DESC, " : "", showNested || pattern ? "\"Parent name\" NULLS FIRST, " : "");
...
listLanguages(const char *pattern, bool verbose, bool showSystem)
{
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,",\n       NOT l.lanispl AS \"%s\",\n       l.lanplcallfoid::pg_catalog.regprocedure AS \"%s\",\n       l.lanvalidator::pg_catalog.regprocedure AS \"%s\",\n       ",gettext_noop("Internal language"), gettext_noop("Call handler"), gettext_noop("Validator"));
    if (pset.sversion >= 90000) {
  ...
  if (pattern) {
    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "l.lanname", NULL, NULL);
  }
...
listDomains(const char *pattern, bool verbose, bool showSystem)
{
  ...
  printfPQExpBuffer(&buf,"SELECT n.nspname as \"%s\",\n       t.typname as \"%s\",\n       pg_catalog.format_type(t.typbasetype, t.typtypmod) as \"%s\",\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Type"));
  ...
  appendPQExpBuffer(&buf,"       CASE WHEN t.typnotnull THEN 'not null' END as \"%s\",\n       t.typdefault as \"%s\",\n       pg_catalog.array_to_string(ARRAY(\n         SELECT pg_catalog.pg_get_constraintdef(r.oid, true) FROM pg_catalog.pg_constraint r WHERE t.oid = r.contypid\n       ), ' ') as \"%s\"",gettext_noop("Nullable"), gettext_noop("Default"), gettext_noop("Check"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "t.typname", NULL, "pg_catalog.pg_type_is_visible(t.oid)");
...
listConversions(const char *pattern, bool verbose, bool showSystem)
{
  ...
  printfPQExpBuffer(&buf,"SELECT n.nspname AS \"%s\",\n       c.conname AS \"%s\",\n       pg_catalog.pg_encoding_to_char(c.conforencoding) AS \"%s\",\n       pg_catalog.pg_encoding_to_char(c.contoencoding) AS \"%s\",\n       CASE WHEN c.condefault THEN '%s'\n       ELSE '%s' END AS \"%s\"",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Source"), gettext_noop("Destination"), gettext_noop("yes"), gettext_noop("no"), gettext_noop("Default?"));
  ...
  if (verbose) {
    appendPQExpBufferStr(&buf, "LEFT JOIN pg_catalog.pg_description d ON d.classoid = c.tableoid\n          AND d.objoid = c.oid AND d.objsubid = 0\n");
  }
...
listEventTriggers(const char *pattern, bool verbose)
{
  ...
  PGresult *res;
  printQueryOpt myopt = pset.popt;
  static const bool translate_columns[] = {false, false, false, true, false, false, false};

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT evtname as \"%s\", evtevent as \"%s\", pg_catalog.pg_get_userbyid(e.evtowner) as \"%s\",\n case evtenabled when 'O' then '%s'  when 'R' then '%s'  when 'A' then '%s'  when 'D' then '%s' end as \"%s\",\n e.evtfoid::pg_catalog.regproc as \"%s\", pg_catalog.array_to_string(array(select x from pg_catalog.unnest(evttags) as t(x)), ', ') as \"%s\"",gettext_noop("Name"), gettext_noop("Event"), gettext_noop("Owner"), gettext_noop("enabled"), gettext_noop("replica"), gettext_noop("always"), gettext_noop("disabled"), gettext_noop("Enabled"), gettext_noop("Function"), gettext_noop("Tags"));
  if (verbose) {
    appendPQExpBuffer(&buf, ",\npg_catalog.obj_description(e.oid, 'pg_event_trigger') as \"%s\"", gettext_noop("Description"));
  }
  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_event_trigger e ");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "evtname", NULL, NULL);
...
listCasts(const char *pattern, bool verbose)
{
  ...
  printfPQExpBuffer(&buf,"SELECT pg_catalog.format_type(castsource, NULL) AS \"%s\",\n       pg_catalog.format_type(casttarget, NULL) AS \"%s\",\n",gettext_noop("Source type"), gettext_noop("Target type"));

  /*
   * We don't attempt to localize '(binary coercible)' or '(with inout)',* because there's too much risk of gettext translating a function name
   * that happens to match some string in the PO database.
   */
  if (pset.sversion >= 80400) {
    appendPQExpBuffer(&buf,"       CASE WHEN c.castmethod = '%c' THEN '(binary coercible)'\n            WHEN c.castmethod = '%c' THEN '(with inout)'\n            ELSE p.proname\n       END AS \"%s\",\n",COERCION_METHOD_BINARY, COERCION_METHOD_INOUT, gettext_noop("Function"));
  } else {
    appendPQExpBuffer(&buf,"       CASE WHEN c.castfunc = 0 THEN '(binary coercible)'\n            ELSE p.proname\n       END AS \"%s\",\n",gettext_noop("Function"));
  }

  appendPQExpBuffer(&buf,"       CASE WHEN c.castcontext = '%c' THEN '%s'\n            WHEN c.castcontext = '%c' THEN '%s'\n            ELSE '%s'\n       END AS \"%s\"",COERCION_CODE_EXPLICIT, gettext_noop("no"), COERCION_CODE_ASSIGNMENT, gettext_noop("in assignment"), gettext_noop("yes"), gettext_noop("Implicit?"));
   ...
   */
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "ns.nspname", "ts.typname", "pg_catalog.format_type(ts.oid, NULL)", "pg_catalog.pg_type_is_visible(ts.oid)");

  appendPQExpBufferStr(&buf, ") OR (true");

  processSQLNamePattern(pset.db, &buf, pattern, true, false, "nt.nspname", "tt.typname", "pg_catalog.format_type(tt.oid, NULL)", "pg_catalog.pg_type_is_visible(tt.oid)");
...
listCollations(const char *pattern, bool verbose, bool showSystem)
{
  ...
  if (pset.sversion < 90100) {
    ...
    pg_log_error("The server (version %s) does not support collations.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT n.nspname AS \"%s\",\n       c.collname AS \"%s\",\n       c.collcollate AS \"%s\",\n       c.collctype AS \"%s\"",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Collate"), gettext_noop("Ctype"));

  if (pset.sversion >= 100000) {
    appendPQExpBuffer(&buf,",\n       CASE c.collprovider WHEN 'd' THEN 'default' WHEN 'c' THEN 'libc' WHEN 'i' THEN 'icu' END AS \"%s\"",gettext_noop("Provider"));
  } else {
    appendPQExpBuffer(&buf, ",\n       'libc' AS \"%s\"", gettext_noop("Provider"));
  }

  if (pset.sversion >= 120000) {
    appendPQExpBuffer(&buf,",\n       CASE WHEN c.collisdeterministic THEN '%s' ELSE '%s' END AS \"%s\"",gettext_noop("yes"), gettext_noop("no"), gettext_noop("Deterministic?"));
  } else {
    appendPQExpBuffer(&buf, ",\n       '%s' AS \"%s\"", gettext_noop("yes"), gettext_noop("Deterministic?"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "c.collname", NULL, "pg_catalog.pg_collation_is_visible(c.oid)");
...
listSchemas(const char *pattern, bool verbose, bool showSystem)
{
  ...
  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT n.nspname AS \"%s\",\n  pg_catalog.pg_get_userbyid(n.nspowner) AS \"%s\"",gettext_noop("Name"), gettext_noop("Owner"));
  ...
  if (!showSystem && !pattern) {
    appendPQExpBufferStr(&buf, "WHERE n.nspname !~ '^pg_' AND n.nspname <> 'information_schema'\n");
  }
...
listTSParsers(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 80300) {
    ...
    pg_log_error("The server (version %s) does not support full text search.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  printfPQExpBuffer(&buf,"SELECT\n  n.nspname as \"%s\",\n  p.prsname as \"%s\",\n  pg_catalog.obj_description(p.oid, 'pg_ts_parser') as \"%s\"\nFROM pg_catalog.pg_ts_parser p\nLEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.prsnamespace\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Description"));

  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname", "p.prsname", NULL, "pg_catalog.pg_ts_parser_is_visible(p.oid)");
...
listTSParsersVerbose(const char *pattern)
{
  ...
  printfPQExpBuffer(&buf, "SELECT p.oid,\n  n.nspname,\n  p.prsname\nFROM pg_catalog.pg_ts_parser p\nLEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.prsnamespace\n");
...
describeOneTSParser(const char *oid, const char *nspname, const char *prsname)
{
  ...
  printfPQExpBuffer(&buf,"SELECT '%s' AS \"%s\",\n   p.prsstart::pg_catalog.regproc AS \"%s\",\n   pg_catalog.obj_description(p.prsstart, 'pg_proc') as \"%s\"\n FROM pg_catalog.pg_ts_parser p\n WHERE p.oid = '%s'\nUNION ALL\nSELECT '%s',\n   p.prstoken::pg_catalog.regproc,\n   pg_catalog.obj_description(p.prstoken, 'pg_proc')\n FROM pg_catalog.pg_ts_parser p\n WHERE p.oid = '%s'\nUNION ALL\nSELECT '%s',\n   p.prsend::pg_catalog.regproc,\n   pg_catalog.obj_description(p.prsend, 'pg_proc')\n FROM pg_catalog.pg_ts_parser p\n WHERE p.oid = '%s'\nUNION ALL\nSELECT '%s',\n   p.prsheadline::pg_catalog.regproc,\n   pg_catalog.obj_description(p.prsheadline, 'pg_proc')\n FROM pg_catalog.pg_ts_parser p\n WHERE p.oid = '%s'\nUNION ALL\nSELECT '%s',\n   p.prslextype::pg_catalog.regproc,\n   pg_catalog.obj_description(p.prslextype, 'pg_proc')\n FROM pg_catalog.pg_ts_parser p\n WHERE p.oid = '%s';",gettext_noop("Start parse"), gettext_noop("Method"), gettext_noop("Function"), gettext_noop("Description"), oid, gettext_noop("Get next token"), oid, gettext_noop("End parse"), oid, gettext_noop("Get headline"), oid, gettext_noop("Get token types"), oid);
  ...
  initPQExpBuffer(&title);
  if (nspname) {
    printfPQExpBuffer(&title, _("Text search parser \"%s.%s\""), nspname, prsname);
  } else {
  ...
  printfPQExpBuffer(&buf,"SELECT t.alias as \"%s\",\n  t.description as \"%s\"\nFROM pg_catalog.ts_token_type( '%s'::pg_catalog.oid ) as t\nORDER BY 1;",gettext_noop("Token name"), gettext_noop("Description"), oid);
  ...
  myopt.nullPrint = NULL;
  if (nspname) {
    printfPQExpBuffer(&title, _("Token types for parser \"%s.%s\""), nspname, prsname);
  } else {
...
listTSDictionaries(const char *pattern, bool verbose)
{
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,"  ( SELECT COALESCE(nt.nspname, '(null)')::pg_catalog.text || '.' || t.tmplname FROM\n    pg_catalog.pg_ts_template t\n    LEFT JOIN pg_catalog.pg_namespace nt ON nt.oid = t.tmplnamespace\n    WHERE d.dicttemplate = t.oid ) AS  \"%s\",\n  d.dictinitoption as \"%s\",\n",gettext_noop("Template"), gettext_noop("Init options"));
  }

  appendPQExpBuffer(&buf, "  pg_catalog.obj_description(d.oid, 'pg_ts_dict') as \"%s\"\n", gettext_noop("Description"));

  appendPQExpBufferStr(&buf, "FROM pg_catalog.pg_ts_dict d\nLEFT JOIN pg_catalog.pg_namespace n ON n.oid = d.dictnamespace\n");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname", "d.dictname", NULL, "pg_catalog.pg_ts_dict_is_visible(d.oid)");
...
listTSTemplates(const char *pattern, bool verbose)
{
  ...
  if (verbose) {
    printfPQExpBuffer(&buf,"SELECT\n  n.nspname AS \"%s\",\n  t.tmplname AS \"%s\",\n  t.tmplinit::pg_catalog.regproc AS \"%s\",\n  t.tmpllexize::pg_catalog.regproc AS \"%s\",\n  pg_catalog.obj_description(t.oid, 'pg_ts_template') AS \"%s\"\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Init"), gettext_noop("Lexize"), gettext_noop("Description"));
  } else {
    printfPQExpBuffer(&buf,"SELECT\n  n.nspname AS \"%s\",\n  t.tmplname AS \"%s\",\n  pg_catalog.obj_description(t.oid, 'pg_ts_template') AS \"%s\"\n",gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Description"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname", "t.tmplname", NULL, "pg_catalog.pg_ts_template_is_visible(t.oid)");
...
listTSConfigs(const char *pattern, bool verbose)
{
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname", "c.cfgname", NULL, "pg_catalog.pg_ts_config_is_visible(c.oid)");
...
listTSConfigsVerbose(const char *pattern)
{
  ...
  printfPQExpBuffer(&buf, "SELECT c.oid, c.cfgname,\n   n.nspname,\n   p.prsname,\n   np.nspname as pnspname\nFROM pg_catalog.pg_ts_config c\n   LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.cfgnamespace,\n pg_catalog.pg_ts_parser p\n   LEFT JOIN pg_catalog.pg_namespace np ON np.oid = p.prsnamespace\nWHERE  p.oid = c.cfgparser\n");

  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname", "c.cfgname", NULL, "pg_catalog.pg_ts_config_is_visible(c.oid)");
...
listForeignDataWrappers(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 80400) {
    ...
    pg_log_error("The server (version %s) does not support foreign-data wrappers.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT fdw.fdwname AS \"%s\",\n  pg_catalog.pg_get_userbyid(fdw.fdwowner) AS \"%s\",\n",gettext_noop("Name"), gettext_noop("Owner"));
  if (pset.sversion >= 90100) {
  ...
  if (verbose) {
    ...
    appendPQExpBuffer(&buf,",\n CASE WHEN fdwoptions IS NULL THEN '' ELSE   '(' || pg_catalog.array_to_string(ARRAY(SELECT   pg_catalog.quote_ident(option_name) ||  ' ' ||   pg_catalog.quote_literal(option_value)  FROM   pg_catalog.pg_options_to_table(fdwoptions)),  ', ') || ')'   END AS \"%s\"",gettext_noop("FDW options"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "fdwname", NULL, NULL);
...
listForeignServers(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 80400) {
    ...
    pg_log_error("The server (version %s) does not support foreign servers.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT s.srvname AS \"%s\",\n  pg_catalog.pg_get_userbyid(s.srvowner) AS \"%s\",\n  f.fdwname AS \"%s\"",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Foreign-data wrapper"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "s.srvname", NULL, NULL);
...
listUserMappings(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 80400) {
    ...
    pg_log_error("The server (version %s) does not support user mappings.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT um.srvname AS \"%s\",\n  um.usename AS \"%s\"",gettext_noop("Server"), gettext_noop("User name"));

  if (verbose) {
    appendPQExpBuffer(&buf,",\n CASE WHEN umoptions IS NULL THEN '' ELSE   '(' || pg_catalog.array_to_string(ARRAY(SELECT   pg_catalog.quote_ident(option_name) ||  ' ' ||   pg_catalog.quote_literal(option_value)  FROM   pg_catalog.pg_options_to_table(umoptions)),  ', ') || ')'   END AS \"%s\"",gettext_noop("FDW options"));
  }

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_user_mappings um\n");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "um.srvname", "um.usename", NULL);
...
listForeignTables(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 90100) {
    ...
    pg_log_error("The server (version %s) does not support foreign tables.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT n.nspname AS \"%s\",\n  c.relname AS \"%s\",\n  s.srvname AS \"%s\"",gettext_noop("Schema"), gettext_noop("Table"), gettext_noop("Server"));

  if (verbose) {
    appendPQExpBuffer(&buf,",\n CASE WHEN ftoptions IS NULL THEN '' ELSE   '(' || pg_catalog.array_to_string(ARRAY(SELECT   pg_catalog.quote_ident(option_name) ||  ' ' ||   pg_catalog.quote_literal(option_value)  FROM   pg_catalog.pg_options_to_table(ftoptions)),  ', ') || ')'   END AS \"%s\",\n  d.description AS \"%s\"",gettext_noop("FDW options"), gettext_noop("Description"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname", "c.relname", NULL, "pg_catalog.pg_table_is_visible(c.oid)");
...
listExtensions(const char *pattern)
{
  ...
  if (pset.sversion < 90100) {
    ...
    pg_log_error("The server (version %s) does not support extensions.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT e.extname AS \"%s\", e.extversion AS \"%s\", n.nspname AS \"%s\", c.description AS \"%s\"\nFROM pg_catalog.pg_extension e LEFT JOIN pg_catalog.pg_namespace n ON n.oid = e.extnamespace LEFT JOIN pg_catalog.pg_description c ON c.objoid = e.oid AND c.classoid = 'pg_catalog.pg_extension'::pg_catalog.regclass\n",gettext_noop("Name"), gettext_noop("Version"), gettext_noop("Schema"), gettext_noop("Description"));

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "e.extname", NULL, NULL);
...
listExtensionContents(const char *pattern)
{
  ...
  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf, "SELECT e.extname, e.oid\nFROM pg_catalog.pg_extension e\n");
...
listOneExtensionContents(const char *extname, const char *oid)
{
  ...
  initPQExpBuffer(&buf);
  printfPQExpBuffer(&buf,"SELECT pg_catalog.pg_describe_object(classid, objid, 0) AS \"%s\"\nFROM pg_catalog.pg_depend\nWHERE refclassid = 'pg_catalog.pg_extension'::pg_catalog.regclass AND refobjid = '%s' AND deptype = 'e'\nORDER BY 1;",gettext_noop("Object description"), oid);
...
listPublications(const char *pattern)
{
  ...
  if (pset.sversion < 100000) {
    ...
    pg_log_error("The server (version %s) does not support publications.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT pubname AS \"%s\",\n  pg_catalog.pg_get_userbyid(pubowner) AS \"%s\",\n  puballtables AS \"%s\",\n  pubinsert AS \"%s\",\n  pubupdate AS \"%s\",\n  pubdelete AS \"%s\"",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("All tables"), gettext_noop("Inserts"), gettext_noop("Updates"), gettext_noop("Deletes"));
  if (pset.sversion >= 110000) {
    appendPQExpBuffer(&buf, ",\n  pubtruncate AS \"%s\"", gettext_noop("Truncates"));
  }

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_publication\n");

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "pubname", NULL, NULL);
...
describePublications(const char *pattern)
{
  ...
  if (has_pubtruncate) {
    appendPQExpBuffer(&buf, ", pubtruncate");
  }
  appendPQExpBuffer(&buf, "\nFROM pg_catalog.pg_publication\n");
  ...
  for (i = 0; i < PQntuples(res); i++) {
    ...
    if (!puballtables) {
      printfPQExpBuffer(&buf,"SELECT n.nspname, c.relname\nFROM pg_catalog.pg_class c,\n     pg_catalog.pg_namespace n,\n     pg_catalog.pg_publication_rel pr\nWHERE c.relnamespace = n.oid\n  AND c.oid = pr.prrelid\n  AND pr.prpubid = '%s'\nORDER BY 1,2",pubid);
      ...
      for (j = 0; j < tables; j++) {
        printfPQExpBuffer(&buf, "    \"%s.%s\"", PQgetvalue(tabres, j, 0), PQgetvalue(tabres, j, 1));
...
describeSubscriptions(const char *pattern, bool verbose)
{
  ...
  if (pset.sversion < 100000) {
    ...
    pg_log_error("The server (version %s) does not support subscriptions.", formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  initPQExpBuffer(&buf);

  printfPQExpBuffer(&buf,"SELECT subname AS \"%s\"\n,  pg_catalog.pg_get_userbyid(subowner) AS \"%s\"\n,  subenabled AS \"%s\"\n,  subpublications AS \"%s\"\n",gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Enabled"), gettext_noop("Publication"));

  if (verbose) {
    appendPQExpBuffer(&buf,",  subsynccommit AS \"%s\"\n,  subconninfo AS \"%s\"\n",gettext_noop("Synchronous commit"), gettext_noop("Conninfo"));
  }

  /* Only display subscriptions in current database. */
  appendPQExpBufferStr(&buf, "FROM pg_catalog.pg_subscription\nWHERE subdbid = (SELECT oid\n                 FROM pg_catalog.pg_database\n                 WHERE datname = pg_catalog.current_database())");

  processSQLNamePattern(pset.db, &buf, pattern, true, false, NULL, "subname", NULL, NULL);
...
{
  if (pset.sversion >= 80100) {
    appendPQExpBuffer(buf, "pg_catalog.array_to_string(%s, E'\\n') AS \"%s\"", colname, gettext_noop("Access privileges"));
  } else {
    appendPQExpBuffer(buf, "pg_catalog.array_to_string(%s, '\\n') AS \"%s\"", colname, gettext_noop("Access privileges"));
  }
}
