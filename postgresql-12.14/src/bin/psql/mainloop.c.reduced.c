...
int MainLoop(FILE *source) {
  ...
  if (PQExpBufferBroken(query_buf) || PQExpBufferBroken(previous_buf) ||
  ...
  while (successResult == EXIT_SUCCESS) {
    ...
    if (pset.lineno == 1 && pset.encoding == PG_UTF8 &&
        strncmp(line, "\xef\xbb\xbf", 3) == 0) {
    ...
    if (pset.cur_cmd_interactive) {
      ...
      if (found_help) {
        if (query_buf->len != 0)
#ifndef WIN32
          puts(_("Use \\? for help or press control-C to clear the input "
                 "buffer."));
#else
        {
          puts(_("Use \\? for help."));
       ...
       */
      if (found_q && query_buf->len != 0 && prompt_status != PROMPT_READY &&
          prompt_status != PROMPT_CONTINUE && prompt_status != PROMPT_PAREN)
#ifndef WIN32
        puts(_("Use control-D to quit."));
#else
      {
     ...
     */
    psql_scan_setup(scan_state, line, strlen(line), pset.encoding,
                    standard_strings());
    success = true;
    ...
    pg_log_error("query ignored;
        ...
        slashCmdStatus =
            HandleSlashCmds(scan_state, cond_stack, query_buf, previous_buf);
   ...
   */
  if (slashCmdStatus != PSQL_CMD_TERMINATE && successResult != EXIT_USER &&
