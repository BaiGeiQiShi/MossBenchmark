...
bool ParseVariableNum(const char *value, const char *name, int *result) {
  ...
  if (errno == 0 && *end == '\0' && end != value && numval == (int)numval) {
    *result = (int)numval;
    ...
    /* string is not recognized; don't clobber *result */
    if (name) {
      pg_log_error("invalid value \"%s\" for \"%s\": integer expected", value,
                   name);
    }
    return false;
  }
  ...
  for (previous = space, current = space->next; current;
    ...
    if (cmp == 0) {
      ...
      if (confirmed) {
         ...
         */
        if (new_value == NULL && current->substitute_hook == NULL &&
...
void SetVariableHooks(VariableSpace space, const char *name,
                      VariableSubstituteHook shook, VariableAssignHook ahook) {
  struct _variable *current, *previous;
 ...
 */
void PsqlVarEnumError(const char *name, const char *value,
