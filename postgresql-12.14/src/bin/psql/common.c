 ...
 *
 * fname == NULL selects stdout, else an initial '|' selects a pipe,* else plain file.
 *
...
psql_get_variable(const char *varname, PsqlScanQuoteType quote, void *passthrough)
{
  ...
  case PQUOTE_SHELL_ARG: {
     ...
     * is too.  In any case, the only special character is "'",* which is not known to appear in valid multibyte characters.
     */
    PQExpBufferData buf;
...
/* CheckConnection
 *
 * Verify that we still have a good connection to the backend, and if not,* see if it can be restored.
 *
 ...
 *
 * Note: current policy is to apply this to the results of all queries,* including "back door" queries, for debugging's sake.  It's OK to use
 ...
 *
 * Returns 1 if the query executed successfully, 0 if it cannot be repeated,* e.g., because of the interrupt, -1 on error.
 */
int
 ...
 *
 * When our command string contained a COPY FROM STDIN or COPY TO STDOUT,* PQexec() has stopped at the PGresult associated with the first such
 ...
 *
 * Changes its argument to point to the last PGresult of the command string,* or NULL if that result was for a COPY TO STDOUT.  (Returning NULL prevents
...
ProcessResult(PGresult **results)
{
  ...
  for (;;) {
    ...
    if (is_copy) {
       ...
       *
       * For COPY OUT, direct the output to pset.copyStream if it's set,* otherwise to pset.gfname if it's set, otherwise to queryFout.
       * For COPY IN, use pset.copyStream as data source if it's set,* otherwise cur_cmd_source.
       */
      FILE *copystream;
      ...
      if (result_status == PGRES_COPY_OUT) {
        ...
        success = handleCopyOut(pset.db, copystream, &copy_result) && success && (copystream != NULL);
...
SendQuery(const char *query)
{
  ...
  if (transaction_status == PQTRANS_INTRANS && pset.on_error_rollback != PSQL_ERROR_ROLLBACK_OFF && (pset.cur_cmd_interactive || pset.on_error_rollback == PSQL_ERROR_ROLLBACK_ON)) {
    if (on_error_rollback_warning == false && pset.sversion < 80000) {
      ...
      pg_log_warning("The server (version %s) does not support savepoints for ON_ERROR_ROLLBACK.",formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
  ...
  if (on_error_rollback_savepoint) {
    ...
    case PQTRANS_INTRANS:
       ...
       */
      if (results && (strcmp(PQcmdStatus(results), "COMMIT") == 0 || strcmp(PQcmdStatus(results), "SAVEPOINT") == 0 || strcmp(PQcmdStatus(results), "RELEASE") == 0 || strcmp(PQcmdStatus(results), "ROLLBACK") == 0)) {
        svptcmd = NULL;
      } else {
  ...
  *elapsed_msec = 0;
  ...
  if (OK && results) {
    if (PQnfields(results) > 0) {
      ...
      fprintf(pset.queryFout, _("The command has no result, or the result has no columns.\n"));
    }
  }
...
ExecQueryUsingCursor(const char *query, double *elapsed_msec)
{
  ...
  *elapsed_msec = 0;
  ...
  if (OK) {
     ...
     * the right row count, so fake SetResultVariables().  In error cases,* we already set the result variables above.
     */
    char buf[32];
 ...
 *
 * Note: With a pre-protocol-3.0 connection this will always say "false",* which should be the right answer.
 */
bool
