...
static bool describeOneTableDetails(const char *schemaname,
                                    const char *relationname, const char *oid,
                                    bool verbose);
...
static bool describeOneTSConfig(const char *oid, const char *nspname,
                                const char *cfgname, const char *pnspname,
                                const char *prsname);
...
bool describeAggregates(const char *pattern, bool verbose, bool showSystem) {
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname",
                        "p.proname", NULL,
...
bool describeAccessMethods(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    " WHEN 't' THEN '%s'"
                    " END AS \"%s\"",
                    gettext_noop("Name"), gettext_noop("Index"),
                    gettext_noop("Table"), gettext_noop("Type"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "amname",
                        NULL, NULL);
...
bool describeTablespaces(const char *pattern, bool verbose) {
  ...
  if (verbose && pset.sversion >= 90200) {
    appendPQExpBuffer(
        &buf,
        ",\n  pg_catalog.pg_size_pretty(pg_catalog.pg_tablespace_size(oid)) AS "
        "\"%s\"",
        gettext_noop("Size"));
  }
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "spcname",
                        NULL, NULL);
 ...
 */
bool describeFunctions(const char *functypes, const char *pattern, bool verbose,
                       bool showSystem) {
  ...
  static const bool translate_columns_pre_96[] = {false, false, false, false,
  ...
  if (showProcedure && pset.sversion < 110000) {
    ...
    pg_log_error(
        "\\df does not take a \"%c\" option with server version %s", 'p',
        formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  if (showWindow && pset.sversion < 80400) {
    ...
    pg_log_error(
        "\\df does not take a \"%c\" option with server version %s", 'w',
        formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }

  if (!showAggregate && !showNormal && !showProcedure && !showTrigger &&
      !showWindow) {
  ...
  if (pset.sversion >= 110000) {
    appendPQExpBuffer(
        ...
        gettext_noop("agg"), gettext_noop("window"), gettext_noop("proc"),
        gettext_noop("func"), gettext_noop("Type"));
    ...
    appendPQExpBuffer(
        ...
        gettext_noop("Result data type"), gettext_noop("Argument data types"),
        /* translator: "agg" is short for "aggregate" */
        gettext_noop("agg"), gettext_noop("window"), gettext_noop("trigger"),
        gettext_noop("func"), gettext_noop("Type"));
    ...
    appendPQExpBuffer(
        ...
        gettext_noop("Result data type"), gettext_noop("Argument data types"),
        /* translator: "agg" is short for "aggregate" */
        gettext_noop("agg"), gettext_noop("trigger"), gettext_noop("func"),
        gettext_noop("Type"));
  } else {
    appendPQExpBuffer(
        &buf,
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,
                      ...
                      " END as \"%s\"",
                      gettext_noop("immutable"), gettext_noop("stable"),
                      gettext_noop("volatile"), gettext_noop("Volatility"));
    if (pset.sversion >= 90600) {
      appendPQExpBuffer(&buf,
                        ",\n CASE\n"
                        ...
                        " END as \"%s\"",
                        gettext_noop("restricted"), gettext_noop("safe"),
                        gettext_noop("unsafe"), gettext_noop("Parallel"));
    }
    appendPQExpBuffer(
        &buf,
        ",\n pg_catalog.pg_get_userbyid(p.proowner) as \"%s\""
        ",\n CASE WHEN prosecdef THEN '%s' ELSE '%s' END AS \"%s\"",
        gettext_noop("Owner"), gettext_noop("definer"), gettext_noop("invoker"),
    ...
    appendPQExpBuffer(
        ...
        ",\n p.prosrc as \"%s\""
        ",\n pg_catalog.obj_description(p.oid, 'pg_proc') as \"%s\"",
        gettext_noop("Language"), gettext_noop("Source code"),
        gettext_noop("Description"));
  }
  ...
  if (verbose) {
    appendPQExpBufferStr(
        &buf, "     LEFT JOIN pg_catalog.pg_language l ON l.oid = p.prolang\n");
  }

  have_where = false;

  /* filter by function type, if requested */
  if (showNormal && showAggregate && showProcedure && showTrigger &&
      showWindow) {
    /* Do nothing */;
  } else if (showNormal) {
    if (!showAggregate) {
    ...
    if (!showTrigger) {
      ...
      appendPQExpBufferStr(
          &buf, "p.prorettype <> 'pg_catalog.trigger'::pg_catalog.regtype\n");
    }
    if (!showWindow && pset.sversion >= 80400) {
      if (have_where) {
        appendPQExpBufferStr(&buf, "      AND ");
  ...
  processSQLNamePattern(pset.db, &buf, pattern, have_where, false, "n.nspname",
                        "p.proname", NULL,
                        "pg_catalog.pg_function_is_visible(p.oid)");
...
bool describeTypes(const char *pattern, bool verbose, bool showSystem) {
  ...
  appendPQExpBuffer(
      &buf, "  pg_catalog.obj_description(t.oid, 'pg_type') as \"%s\"\n",
      gettext_noop("Description"));
  ...
  appendPQExpBufferStr(
      &buf, "OR (SELECT c.relkind = " CppAsString2(
                RELKIND_COMPOSITE_TYPE) " FROM pg_catalog.pg_class c "
  ...
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "n.nspname",
                        "t.typname", "pg_catalog.format_type(t.oid, NULL)",
...
bool describeOperators(const char *pattern, bool verbose, bool showSystem) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "pg_catalog.format_type(o.oprleft, NULL) END AS \"%s\",\n"
                    "  CASE WHEN o.oprkind='r' THEN NULL ELSE "
                    "pg_catalog.format_type(o.oprright, NULL) END AS \"%s\",\n"
...
bool listAllDbs(const char *pattern, bool verbose) {
  ...
  if (verbose && pset.sversion >= 80200) {
    appendPQExpBuffer(&buf,
                      ",\n       pg_catalog.shobj_description(d.oid, "
                      "'pg_database') as \"%s\"",
                      gettext_noop("Description"));
  }
  ...
  if (verbose && pset.sversion >= 80000) {
    appendPQExpBufferStr(
        &buf, "  JOIN pg_catalog.pg_tablespace t on d.dattablespace = t.oid\n");
  }

  if (pattern) {
    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL,
                          "d.datname", NULL, NULL);
  }
...
bool permissionsList(const char *pattern) {
  ...
  PGresult *res;
  printQueryOpt myopt = pset.popt;
  static const bool translate_columns[] = {false, false, true,
                                           false, false, false};
  ...
  printfPQExpBuffer(
      ...
      "  CASE c.relkind"
      " WHEN " CppAsString2(
          RELKIND_RELATION) " THEN '%s'"
                            " WHEN " CppAsString2(
                                RELKIND_VIEW) " THEN '%s'"
                                              " WHEN " CppAsString2(
                                                  RELKIND_MATVIEW) " THEN '%s'"
                                                                   " WHEN"
                                                                   " " CppAsString2(
                                                                       RELKIND_SEQUENCE) " THEN '%s'"
                                                                                         " WHEN " CppAsString2(
                                                                                             RELKIND_FOREIGN_TABLE) " THEN '%s'"
                                                                                                                    " WHEN " CppAsString2(
                                                                                                                        RELKIND_PARTITIONED_TABLE) " THEN '%s'"
                                                                                                                                                   " END as \"%s\",\n"
                                                                                                                                                   "  ",
      gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("table"),
      gettext_noop("view"), gettext_noop("materialized view"),
      gettext_noop("sequence"), gettext_noop("foreign table"),
      gettext_noop("partitioned table"), gettext_noop("Type"));
  ...
  if (pset.sversion >= 80400) {
    appendPQExpBuffer(&buf,
                      ...
                      "    WHERE attrelid = c.oid AND NOT attisdropped AND "
                      "attacl IS NOT NULL\n"
                      "  ), E'\\n') AS \"%s\"",
  ...
  appendPQExpBufferStr(
      ...
      "WHERE c.relkind IN (" CppAsString2(RELKIND_RELATION) "," CppAsString2(RELKIND_VIEW) "," CppAsString2(RELKIND_MATVIEW) "," CppAsString2(
          RELKIND_SEQUENCE) "," CppAsString2(RELKIND_FOREIGN_TABLE) "," CppAsString2(RELKIND_PARTITIONED_TABLE) ")\n");
  ...
  processSQLNamePattern(
      pset.db, &buf, pattern, true, false, "n.nspname", "c.relname", NULL,
...
bool listDefaultACLs(const char *pattern) {
  ...
  printfPQExpBuffer(
      ...
      "  n.nspname AS \"%s\",\n"
      "  CASE d.defaclobjtype WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN "
      "'%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' END AS \"%s\",\n"
      "  ",
      gettext_noop("Owner"), gettext_noop("Schema"), DEFACLOBJ_RELATION,
      gettext_noop("table"), DEFACLOBJ_SEQUENCE, gettext_noop("sequence"),
      DEFACLOBJ_FUNCTION, gettext_noop("function"), DEFACLOBJ_TYPE,
      gettext_noop("type"), DEFACLOBJ_NAMESPACE, gettext_noop("schema"),
      gettext_noop("Type"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "n.nspname",
                        "pg_catalog.pg_get_userbyid(d.defaclrole)", NULL);
...
bool objectDescription(const char *pattern, bool showSystem) {
  ...
  appendPQExpBuffer(&buf,
                    "SELECT DISTINCT tt.nspname AS \"%s\", tt.name AS \"%s\", "
                    "tt.object AS \"%s\", d.description AS \"%s\"\n"
                    "FROM (\n",
                    gettext_noop("Schema"), gettext_noop("Name"),
                    gettext_noop("Object"), gettext_noop("Description"));
...
static bool describeOneTableDetails(const char *schemaname,
  ...
  int attname_col = -1, /* column indexes in "res" */
      atttype_col = -1, attrdef_col = -1, attnotnull_col = -1, attcoll_col = -1,
      attidentity_col = -1, attgenerated_col = -1, isindexkey_col = -1,
      indexdef_col = -1, fdwopts_col = -1, attstorage_col = -1,
      attstattarget_col = -1, attdescr_col = -1;
  int numrows;
  struct {
    int16 checks;
    char relkind;
  ...
  tableinfo.reloptions =
      (pset.sversion >= 80200) ? pg_strdup(PQgetvalue(res, 0, 9)) : NULL;
  tableinfo.tablespace =
      (pset.sversion >= 80000) ? atooid(PQgetvalue(res, 0, 10)) : 0;
  tableinfo.reloftype =
      (pset.sversion >= 90000 && strcmp(PQgetvalue(res, 0, 11), "") != 0)
          ? pg_strdup(PQgetvalue(res, 0, 11))
          : NULL;
  tableinfo.relpersistence =
      (pset.sversion >= 90100) ? *(PQgetvalue(res, 0, 12)) : 0;
  tableinfo.relreplident =
      (pset.sversion >= 90400) ? *(PQgetvalue(res, 0, 13)) : 'd';
  if (pset.sversion >= 120000) {
    tableinfo.relam = PQgetisnull(res, 0, 14)
                          ? (char *)NULL
                          : pg_strdup(PQgetvalue(res, 0, 14));
  } else {
  ...
  if (tableinfo.relkind == RELKIND_SEQUENCE) {
    ...
    if (pset.sversion >= 100000) {
      printfPQExpBuffer(
          ...
          "       seqcache AS \"%s\"\n",
          gettext_noop("Type"), gettext_noop("Start"), gettext_noop("Minimum"),
          gettext_noop("Maximum"), gettext_noop("Increment"),
          gettext_noop("yes"), gettext_noop("no"), gettext_noop("Cycles?"),
          gettext_noop("Cache"));
      appendPQExpBuffer(&buf,
    ...
    printfPQExpBuffer(&title, _("Sequence \"%s.%s\""), schemaname,
                      relationname);
  ...
  if (tableinfo.relkind == RELKIND_INDEX ||
    ...
    if (pset.sversion >= 110000) {
      appendPQExpBuffer(&buf,
                        ",\n  CASE WHEN a.attnum <= (SELECT i.indnkeyatts FROM "
                        "pg_catalog.pg_index i WHERE i.indexrelid = '%s') THEN "
                        "'%s' ELSE '%s' END AS is_key",
                        oid, gettext_noop("yes"), gettext_noop("no"));
      isindexkey_col = cols++;
    }
    appendPQExpBufferStr(&buf, ",\n  pg_catalog.pg_get_indexdef(a.attrelid, "
                               "a.attnum, TRUE) AS indexdef");
    indexdef_col = cols++;
  }
  ...
  if (verbose) {
    ...
    if (tableinfo.relkind == RELKIND_RELATION ||
        ...
        tableinfo.relkind == RELKIND_FOREIGN_TABLE ||
        tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      appendPQExpBufferStr(&buf, ",\n  CASE WHEN a.attstattarget=-1 THEN NULL "
                                 "ELSE a.attstattarget END AS attstattarget");
  ...
  case RELKIND_RELATION:
    if (tableinfo.relpersistence == 'u') {
      printfPQExpBuffer(&title, _("Unlogged table \"%s.%s\""), schemaname,
                        relationname);
    } else {
  ...
  case RELKIND_MATVIEW:
    if (tableinfo.relpersistence == 'u') {
      ...
      printfPQExpBuffer(&title, _("Materialized view \"%s.%s\""), schemaname,
                        relationname);
    }
    break;
  case RELKIND_INDEX:
    if (tableinfo.relpersistence == 'u') {
      printfPQExpBuffer(&title, _("Unlogged index \"%s.%s\""), schemaname,
                        relationname);
    } else {
  ...
  case RELKIND_PARTITIONED_INDEX:
    if (tableinfo.relpersistence == 'u') {
      ...
      printfPQExpBuffer(&title, _("Partitioned index \"%s.%s\""), schemaname,
                        relationname);
    }
    break;
  case 's':
    /* not used as of 8.2, but keep it for backwards compatibility */
    printfPQExpBuffer(&title, _("Special relation \"%s.%s\""), schemaname,
                      relationname);
    break;
  case RELKIND_TOASTVALUE:
    printfPQExpBuffer(&title, _("TOAST table \"%s.%s\""), schemaname,
                      relationname);
    break;
  case RELKIND_COMPOSITE_TYPE:
    printfPQExpBuffer(&title, _("Composite type \"%s.%s\""), schemaname,
                      relationname);
    break;
  case RELKIND_FOREIGN_TABLE:
    printfPQExpBuffer(&title, _("Foreign table \"%s.%s\""), schemaname,
                      relationname);
    break;
  case RELKIND_PARTITIONED_TABLE:
    if (tableinfo.relpersistence == 'u') {
      ...
      printfPQExpBuffer(&title, _("Partitioned table \"%s.%s\""), schemaname,
                        relationname);
    }
    break;
  default:;
    /* untranslated unknown relkind */
    printfPQExpBuffer(&title, "?%c? \"%s.%s\"", tableinfo.relkind, schemaname,
                      relationname);
    break;
  }
  ...
  for (i = 0; i < numrows; i++) {
    ...
    if (attstattarget_col >= 0) {
      printTableAddCell(&cont, PQgetvalue(res, i, attstattarget_col), false,
                        false);
    }
  ...
  if (tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
    ...
    printfPQExpBuffer(
        &buf, "SELECT pg_catalog.pg_get_partkeydef('%s'::pg_catalog.oid);",
        oid);
    result = PSQLexec(buf.data);
    if (!result) {
      goto error_return;
  ...
  if (tableinfo.relkind == RELKIND_INDEX ||
    ...
    printfPQExpBuffer(
        &buf, "SELECT i.indisunique, i.indisprimary, i.indisclustered, ");
    ...
    if (!result) {
      ...
      appendPQExpBuffer(&tmpbuf, _("for table \"%s.%s\""), schemaname,
                        indtable);
    ...
    if (tableinfo.hasindex) {
      printfPQExpBuffer(&buf, "SELECT c2.relname, i.indisprimary, "
      ...
      appendPQExpBufferStr(
          &buf, "pg_catalog.pg_get_indexdef(i.indexrelid, 0, true),\n  ");
      if (pset.sversion >= 90000) {
        appendPQExpBufferStr(&buf,
      ...
      if (pset.sversion >= 90000) {
        appendPQExpBufferStr(&buf,
                             ...
                             "(conrelid = i.indrelid AND conindid = "
    ...
    if (tableinfo.checks) {
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          printfPQExpBuffer(&buf, "    \"%s\" %s", PQgetvalue(result, i, 0),
    ...
    if (pset.sversion >= 90500) {
      ...
      appendPQExpBuffer(
          ...
          "pg_catalog.pg_roles where oid = any (pol.polroles) order by 1),',') "
          "END,\n"
      ...
      if (tableinfo.rowsecurity && tableinfo.forcerowsecurity && tuples == 0) {
        printTableAddFooter(
            &cont, _("Policies (forced row security enabled): (none)"));
      }
    ...
    if (pset.sversion >= 100000) {
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          appendPQExpBuffer(&buf, "\"%s\".\"%s\" (", PQgetvalue(result, i, 2),
                            PQgetvalue(result, i, 3));
          ...
          appendPQExpBuffer(&buf, ") ON %s FROM %s", PQgetvalue(result, i, 4),
                            PQgetvalue(result, i, 1));
  ...
  if ((tableinfo.relkind == RELKIND_VIEW ||
    ...
    printfPQExpBuffer(
        &buf, "SELECT pg_catalog.pg_get_viewdef('%s'::pg_catalog.oid, true);",
        oid);
    result = PSQLexec(buf.data);
    if (!result) {
      goto error_return;
  ...
  if (tableinfo.hastriggers) {
    ...
    if (pset.sversion >= 110000) {
      appendPQExpBuffer(
          ...
          "    OR EXISTS (SELECT 1 FROM pg_catalog.pg_depend WHERE objid = "
          "t.oid \n"
          "        AND refclassid = "
      ...
      appendPQExpBufferStr(&buf, "(t.tgconstraint = 0 OR (t.tgconstraint <> 0 "
                                 "AND t.tgenabled = 'D'))");
    } else {
      appendPQExpBufferStr(&buf,
                           "(NOT tgisconstraint "
                           ...
                           "   WHERE d.classid = t.tableoid AND d.objid = "
  ...
  if (tableinfo.relkind == RELKIND_RELATION ||
    ...
    printfPQExpBuffer(&buf,
                      ...
                      " AND c.relkind != " CppAsString2(
                          RELKIND_PARTITIONED_TABLE) " ORDER BY inhseqno;",
                      oid);
    ...
    if (tableinfo.relkind == RELKIND_PARTITIONED_TABLE && tuples == 0) {
      ...
      if (tuples > 0) {
        if (tableinfo.relkind != RELKIND_PARTITIONED_TABLE) {
          printfPQExpBuffer(
              &buf, _("Number of child tables: %d (Use \\d+ to list them.)"),
              tuples);
        } else {
          printfPQExpBuffer(
              &buf, _("Number of partitions: %d (Use \\d+ to list them.)"),
              tuples);
        }
        printTableAddFooter(&cont, buf.data);
      }
      ...
      for (i = 0; i < tuples; i++) {
        if (tableinfo.relkind != RELKIND_PARTITIONED_TABLE) {
          ...
          if (i == 0) {
            printfPQExpBuffer(&buf, "%s: %s %s%s", ct, PQgetvalue(result, i, 0),
                              PQgetvalue(result, i, 1), partitioned_note);
          } else {
            printfPQExpBuffer(&buf, "%*s  %s %s%s", ctw, "",
                              PQgetvalue(result, i, 0),
                              PQgetvalue(result, i, 1), partitioned_note);
          }
        }
    ...
    if (tableinfo.reloftype) {
      printfPQExpBuffer(&buf, _("Typed table of type: %s"),
                        tableinfo.reloftype);
      printTableAddFooter(&cont, buf.data);
    }
...
static void add_tablespace_footer(printTableContent *const cont, char relkind,
  ...
  if (relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW ||
      relkind == RELKIND_INDEX || relkind == RELKIND_PARTITIONED_TABLE ||
...
bool describeRoles(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (pset.sversion >= 80100) {
    ...
    if (verbose && pset.sversion >= 80200) {
      appendPQExpBufferStr(&buf, "\n, pg_catalog.shobj_description(r.oid, "
                                 "'pg_authid') AS description");
      ncols++;
    }
    if (pset.sversion >= 90100) {
      appendPQExpBufferStr(&buf, "\n, r.rolreplication");
    ...
    printfPQExpBuffer(&buf,
                      ...
                      "  ARRAY(SELECT g.groname FROM pg_catalog.pg_group g "
                      "WHERE u.usesysid = ANY(g.grolist)) as memberof"
  ...
  for (i = 0; i < nrows; i++) {
    ...
    if (conns >= 0) {
      ...
      if (conns == 0) {
        ...
        appendPQExpBuffer(
            &buf, ngettext("%d connection", "%d connections", conns), conns);
      }
    }
...
bool listDbRoleSettings(const char *pattern, const char *pattern2) {
  ...
  havewhere = processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL,
                                    "r.rolname", NULL, NULL);
  processSQLNamePattern(pset.db, &buf, pattern2, havewhere, false, NULL,
                        "d.datname", NULL, NULL);
  appendPQExpBufferStr(&buf, "ORDER BY 1, 2;");
...
bool listTables(const char *tabtypes, const char *pattern, bool verbose,
                                                                                                            ...
                                                                                                            " WHEN " CppAsString2(
                                                                                                                RELKIND_FOREIGN_TABLE) " THEN '%s'"
                                                                                                                                       " WHEN " CppAsString2(
                                                                                                                                           RELKIND_PARTITIONED_TABLE) " THEN '%s'"
                                                                                                                                                                      " WHEN " CppAsString2(
                                                                                                                                                                          RELKIND_PARTITIONED_INDEX) " THEN '%s'"
                                                                                                                                                                                                     " END as \"%s\",\n"
                                                                                                                                                                                                     "  pg_catalog.pg_get_userbyid(c.relowner) as \"%s\"",
      gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("table"),
      gettext_noop("view"), gettext_noop("materialized view"),
      gettext_noop("index"), gettext_noop("sequence"), gettext_noop("special"),
      gettext_noop("foreign table"), gettext_noop("partitioned table"),
      gettext_noop("partitioned index"), gettext_noop("Type"),
      gettext_noop("Owner"));
  ...
  appendPQExpBufferStr(&buf, "\nWHERE c.relkind IN (");
  if (showTables) {
    appendPQExpBufferStr(&buf, CppAsString2(RELKIND_RELATION) "," CppAsString2(
 ...
 * and you can mix and match these in any order.
 */
bool listPartitionedTables(const char *reltypes, const char *pattern,
                           bool verbose) {
  ...
  PGresult *res;
  printQueryOpt myopt = pset.popt;
  bool translate_columns[] = {false, false, false, false, false,
                              false, false, false, false};
  ...
  if (pset.sversion < 100000) {
    ...
    pg_log_error(
        "The server (version %s) does not support declarative table "
        "partitioning.",
        formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));
    return true;
  }
  ...
  if (mixed_output) {
    appendPQExpBuffer(
        ...
        " WHEN " CppAsString2(
            RELKIND_PARTITIONED_TABLE) " THEN '%s'"
                                       " WHEN " CppAsString2(
                                           RELKIND_PARTITIONED_INDEX) " THEN "
                                                                      "'%s'"
                                                                      " END as "
                                                                      "\"%s\"",
    ...
    if (pset.sversion < 120000) {
      appendPQExpBuffer(
          &buf, ",\n     LATERAL (WITH RECURSIVE d\n"
                ...
                "                SELECT "
                "pg_catalog.pg_size_pretty(sum(pg_catalog.pg_table_size("
                "d.oid))) AS tps,\n"
  ...
  appendPQExpBufferStr(
      &buf, !showNested && !pattern ? " AND NOT c.relispartition\n" : "");
...
bool listLanguages(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (pset.sversion >= 80300) {
    appendPQExpBuffer(
        &buf, "       pg_catalog.pg_get_userbyid(l.lanowner) as \"%s\",\n",
        gettext_noop("Owner"));
  }

  appendPQExpBuffer(&buf, "       l.lanpltrusted AS \"%s\"",
  ...
  if (verbose) {
    appendPQExpBuffer(
        ...
        gettext_noop("Internal language"), gettext_noop("Call handler"),
        gettext_noop("Validator"));
    if (pset.sversion >= 90000) {
      appendPQExpBuffer(
          &buf, "l.laninline::pg_catalog.regprocedure AS \"%s\",\n       ",
...
bool listDomains(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (pset.sversion >= 90100) {
    appendPQExpBuffer(
        ...
        "t.typcollation <> bt.typcollation) as \"%s\",\n",
...
bool listConversions(const char *pattern, bool verbose, bool showSystem) {
  ...
  printfPQExpBuffer(
      ...
      "       ELSE '%s' END AS \"%s\"",
      gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Source"),
      gettext_noop("Destination"), gettext_noop("yes"), gettext_noop("no"),
...
bool listEventTriggers(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(
      ...
      gettext_noop("Name"), gettext_noop("Event"), gettext_noop("Owner"),
      gettext_noop("enabled"), gettext_noop("replica"), gettext_noop("always"),
      gettext_noop("disabled"), gettext_noop("Enabled"),
      gettext_noop("Function"), gettext_noop("Tags"));
  if (verbose) {
    appendPQExpBuffer(
        &buf,
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "evtname",
                        NULL, NULL);
...
bool listCasts(const char *pattern, bool verbose) {
  ...
  if (pset.sversion >= 80400) {
    appendPQExpBuffer(
        ...
        "            ELSE p.proname\n"
        "       END AS \"%s\",\n",
        COERCION_METHOD_BINARY, COERCION_METHOD_INOUT,
        gettext_noop("Function"));
  } else {
    appendPQExpBuffer(
        &buf,
  ...
  appendPQExpBuffer(&buf,
                    ...
                    "            ELSE '%s'\n"
                    "       END AS \"%s\"",
                    COERCION_CODE_EXPLICIT, gettext_noop("no"),
                    COERCION_CODE_ASSIGNMENT, gettext_noop("in assignment"),
                    gettext_noop("yes"), gettext_noop("Implicit?"));
   ...
   */
  processSQLNamePattern(pset.db, &buf, pattern, true, false, "ns.nspname",
                        "ts.typname", "pg_catalog.format_type(ts.oid, NULL)",
                        "pg_catalog.pg_type_is_visible(ts.oid)");

  appendPQExpBufferStr(&buf, ") OR (true");

  processSQLNamePattern(pset.db, &buf, pattern, true, false, "nt.nspname",
                        "tt.typname", "pg_catalog.format_type(tt.oid, NULL)",
...
bool listCollations(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (pset.sversion >= 120000) {
    appendPQExpBuffer(&buf,
                      ",\n       CASE WHEN c.collisdeterministic THEN '%s' "
                      "ELSE '%s' END AS \"%s\"",
                      gettext_noop("yes"), gettext_noop("no"),
                      gettext_noop("Deterministic?"));
  } else {
    appendPQExpBuffer(&buf, ",\n       '%s' AS \"%s\"", gettext_noop("yes"),
                      gettext_noop("Deterministic?"));
  }
...
bool listTSParsers(const char *pattern, bool verbose) {
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, "n.nspname",
                        "p.prsname", NULL,
...
static bool describeOneTSParser(const char *oid, const char *nspname,
  ...
  printfPQExpBuffer(
      ...
      " FROM pg_catalog.pg_ts_parser p\n"
      " WHERE p.oid = '%s';",
      gettext_noop("Start parse"), gettext_noop("Method"),
      gettext_noop("Function"), gettext_noop("Description"), oid,
      gettext_noop("Get next token"), oid, gettext_noop("End parse"), oid,
      gettext_noop("Get headline"), oid, gettext_noop("Get token types"), oid);
  ...
  initPQExpBuffer(&title);
  if (nspname) {
    printfPQExpBuffer(&title, _("Text search parser \"%s.%s\""), nspname,
                      prsname);
  } else {
  ...
  myopt.nullPrint = NULL;
  if (nspname) {
    printfPQExpBuffer(&title, _("Token types for parser \"%s.%s\""), nspname,
                      prsname);
  } else {
...
bool listTSDictionaries(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    appendPQExpBuffer(
        &buf,
        "  ( SELECT COALESCE(nt.nspname, '(null)')::pg_catalog.text || '.' || "
        "t.tmplname FROM\n"
...
bool listTSTemplates(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    printfPQExpBuffer(
        ...
        gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Init"),
        gettext_noop("Lexize"), gettext_noop("Description"));
  } else {
    printfPQExpBuffer(
        &buf,
        "SELECT\n"
...
static bool describeOneTSConfig(const char *oid, const char *nspname,
  ...
  printfPQExpBuffer(
      ...
      "           WHERE mm.mapcfg = m.mapcfg AND mm.maptokentype = "
      "m.maptokentype\n"
  ...
  if (nspname) {
    appendPQExpBuffer(&title, _("Text search configuration \"%s.%s\""), nspname,
                      cfgname);
  } else {
...
bool listForeignDataWrappers(const char *pattern, bool verbose) {
  ...
  appendPQExpBuffer(&buf, "  fdw.fdwvalidator::pg_catalog.regproc AS \"%s\"",
                    gettext_noop("Validator"));
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "fdwname",
                        NULL, NULL);
...
bool listForeignServers(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    ...
    appendPQExpBuffer(
        ...
        "  END AS \"%s\",\n"
        "  d.description AS \"%s\"",
        gettext_noop("Type"), gettext_noop("Version"),
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "s.srvname",
                        NULL, NULL);
...
bool listForeignTables(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "  c.relname AS \"%s\",\n"
                    "  s.srvname AS \"%s\"",
                    gettext_noop("Schema"), gettext_noop("Table"),
                    gettext_noop("Server"));
...
bool listExtensions(const char *pattern) {
  ...
  printfPQExpBuffer(
      ...
      gettext_noop("Name"), gettext_noop("Version"), gettext_noop("Schema"),
      gettext_noop("Description"));

  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "e.extname",
                        NULL, NULL);
...
bool listPublications(const char *pattern) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "  pubdelete AS \"%s\"",
                    gettext_noop("Name"), gettext_noop("Owner"),
                    gettext_noop("All tables"), gettext_noop("Inserts"),
                    gettext_noop("Updates"), gettext_noop("Deletes"));
  if (pset.sversion >= 110000) {
    appendPQExpBuffer(&buf, ",\n  pubtruncate AS \"%s\"",
  ...
  processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "pubname",
                        NULL, NULL);
...
bool describePublications(const char *pattern) {
  ...
  for (i = 0; i < PQntuples(res); i++) {
    ...
    if (!puballtables) {
      ...
      for (j = 0; j < tables; j++) {
        printfPQExpBuffer(&buf, "    \"%s.%s\"", PQgetvalue(tabres, j, 0),
...
bool describeSubscriptions(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("Name"), gettext_noop("Owner"),
                    gettext_noop("Enabled"), gettext_noop("Publication"));
  ...
  appendPQExpBufferStr(
      &buf, "FROM pg_catalog.pg_subscription\n"
            "WHERE subdbid = (SELECT oid\n"
            "                 FROM pg_catalog.pg_database\n"
            "                 WHERE datname = pg_catalog.current_database())");

  processSQLNamePattern(pset.db, &buf, pattern, true, false, NULL, "subname",
                        NULL, NULL);
