...
main(int argc, char *argv[])
{
  ...
  /*
   * If there was a database specified on the command line, use that,* otherwise try to connect to database "postgres", and failing that
  ...
  if (!data_only) {
     ...
     * dependency analysis because databases never depend on each other,* and tablespaces never depend on each other.  Roles could have
...
dropRoles(PGconn *conn)
{
  ...
  if (server_version >= 90600) {
    printfPQExpBuffer(buf,"SELECT rolname FROM %s WHERE rolname !~ '^pg_' ORDER BY 1",role_catalog);
  } else if (server_version >= 80100) {
    printfPQExpBuffer(buf,"SELECT rolname FROM %s ORDER BY 1",role_catalog);
  } else {
    printfPQExpBuffer(buf, "SELECT usename as rolname FROM pg_shadow UNION SELECT groname as rolname FROM pg_group ORDER BY 1");
  }
...
dumpRoles(PGconn *conn)
{
  ...
  if (server_version >= 90600) {
    printfPQExpBuffer(buf,"SELECT oid, rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolpassword, rolvaliduntil, rolreplication, rolbypassrls, pg_catalog.shobj_description(oid, '%s') as rolcomment, rolname = current_user AS is_current_user FROM %s WHERE rolname !~ '^pg_' ORDER BY 2",role_catalog, role_catalog);
  } else if (server_version >= 90500) {
    printfPQExpBuffer(buf,"SELECT oid, rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolpassword, rolvaliduntil, rolreplication, rolbypassrls, pg_catalog.shobj_description(oid, '%s') as rolcomment, rolname = current_user AS is_current_user FROM %s ORDER BY 2",role_catalog, role_catalog);
  } else if (server_version >= 90100) {
    printfPQExpBuffer(buf,"SELECT oid, rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolpassword, rolvaliduntil, rolreplication, false as rolbypassrls, pg_catalog.shobj_description(oid, '%s') as rolcomment, rolname = current_user AS is_current_user FROM %s ORDER BY 2",role_catalog, role_catalog);
  } else if (server_version >= 80200) {
    printfPQExpBuffer(buf,"SELECT oid, rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolpassword, rolvaliduntil, false as rolreplication, false as rolbypassrls, pg_catalog.shobj_description(oid, '%s') as rolcomment, rolname = current_user AS is_current_user FROM %s ORDER BY 2",role_catalog, role_catalog);
  } else if (server_version >= 80100) {
    printfPQExpBuffer(buf,"SELECT oid, rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolpassword, rolvaliduntil, false as rolreplication, false as rolbypassrls, null as rolcomment, rolname = current_user AS is_current_user FROM %s ORDER BY 2",role_catalog);
  } else {
    printfPQExpBuffer(buf, "SELECT 0 as oid, usename as rolname, usesuper as rolsuper, true as rolinherit, usesuper as rolcreaterole, usecreatedb as rolcreatedb, true as rolcanlogin, -1 as rolconnlimit, passwd as rolpassword, valuntil as rolvaliduntil, false as rolreplication, false as rolbypassrls, null as rolcomment, usename = current_user AS is_current_user FROM pg_shadow UNION ALL SELECT 0 as oid, groname as rolname, false as rolsuper, true as rolinherit, false as rolcreaterole, false as rolcreatedb, false as rolcanlogin, -1 as rolconnlimit, null::text as rolpassword, null::timestamptz as rolvaliduntil, false as rolreplication, false as rolbypassrls, null as rolcomment, false AS is_current_user FROM pg_group WHERE NOT EXISTS (SELECT 1 FROM pg_shadow  WHERE usename = groname) ORDER BY 2");
  }
  ...
  for (i = 0; i < PQntuples(res); i++) {
     ...
     * have failed to drop it.  binary_upgrade cannot generate any errors,* so we assume the current role is already created.
     */
...
dumpTablespaces(PGconn *conn)
{
   ...
   */
  if (server_version >= 90600) {
    res = executeQuery(conn, "SELECT oid, spcname, pg_catalog.pg_get_userbyid(spcowner) AS spcowner, pg_catalog.pg_tablespace_location(oid), (SELECT array_agg(acl ORDER BY row_n) FROM   (SELECT acl, row_n FROM      unnest(coalesce(spcacl,acldefault('t',spcowner)))      WITH ORDINALITY AS perm(acl,row_n)    WHERE NOT EXISTS (      SELECT 1      FROM unnest(acldefault('t',spcowner))        AS init(init_acl)      WHERE acl = init_acl)) AS spcacls)  AS spcacl, (SELECT array_agg(acl ORDER BY row_n) FROM   (SELECT acl, row_n FROM      unnest(acldefault('t',spcowner))      WITH ORDINALITY AS initp(acl,row_n)    WHERE NOT EXISTS (      SELECT 1      FROM unnest(coalesce(spcacl,acldefault('t',spcowner)))        AS permp(orig_acl)      WHERE acl = orig_acl)) AS rspcacls)  AS rspcacl, array_to_string(spcoptions, ', '),pg_catalog.shobj_description(oid, 'pg_tablespace') FROM pg_catalog.pg_tablespace WHERE spcname !~ '^pg_' ORDER BY 1");
  } else if (server_version >= 90200) {
    res = executeQuery(conn, "SELECT oid, spcname, pg_catalog.pg_get_userbyid(spcowner) AS spcowner, pg_catalog.pg_tablespace_location(oid), spcacl, '' as rspcacl, array_to_string(spcoptions, ', '),pg_catalog.shobj_description(oid, 'pg_tablespace') FROM pg_catalog.pg_tablespace WHERE spcname !~ '^pg_' ORDER BY 1");
  } else if (server_version >= 90000) {
    res = executeQuery(conn, "SELECT oid, spcname, pg_catalog.pg_get_userbyid(spcowner) AS spcowner, spclocation, spcacl, '' as rspcacl, array_to_string(spcoptions, ', '),pg_catalog.shobj_description(oid, 'pg_tablespace') FROM pg_catalog.pg_tablespace WHERE spcname !~ '^pg_' ORDER BY 1");
  } else if (server_version >= 80200) {
    res = executeQuery(conn, "SELECT oid, spcname, pg_catalog.pg_get_userbyid(spcowner) AS spcowner, spclocation, spcacl, '' as rspcacl, null, pg_catalog.shobj_description(oid, 'pg_tablespace') FROM pg_catalog.pg_tablespace WHERE spcname !~ '^pg_' ORDER BY 1");
  } else {
    res = executeQuery(conn, "SELECT oid, spcname, pg_catalog.pg_get_userbyid(spcowner) AS spcowner, spclocation, spcacl, '' as rspcacl, null, null FROM pg_catalog.pg_tablespace WHERE spcname !~ '^pg_' ORDER BY 1");
  }
...
dropDBs(PGconn *conn)
{
   ...
   */
  res = executeQuery(conn, "SELECT datname FROM pg_database d WHERE datallowconn ORDER BY datname");
...
dumpUserConfig(PGconn *conn, const char *username)
{
  ...
  for (;;) {
    ...
    if (server_version >= 90000) {
      printfPQExpBuffer(buf,"SELECT setconfig[%d] FROM pg_db_role_setting WHERE setdatabase = 0 AND setrole = (SELECT oid FROM %s WHERE rolname = ",count, role_catalog);
    } else if (server_version >= 80100) {
...
dumpDatabases(PGconn *conn)
{
   ...
   */
  res = executeQuery(conn, "SELECT datname FROM pg_database d WHERE datallowconn ORDER BY (datname <> 'template1'), datname");
