...
_check_database_version(ArchiveHandle *AH)
{
  ...
  /*
   * When running against 9.0 or later, check if we are in recovery mode,* which means we are on a hot standby.
   */
...
/*
 * Reconnect to the server.  If dbname is not NULL, use that database,* else the one associated with the archive handle.
 */
void
...
ConnectDatabase(Archive *AHX, const ConnParams *cparams, bool isReconnect)
{
  ...
  if (PQstatus(AH->connection) == CONNECTION_BAD) {
    if (isReconnect) {
      fatal("reconnection to database \"%s\" failed: %s", PQdb(AH->connection) ? PQdb(AH->connection) : "", PQerrorMessage(AH->connection));
    } else {
      fatal("connection to database \"%s\" failed: %s", PQdb(AH->connection) ? PQdb(AH->connection) : "", PQerrorMessage(AH->connection));
    }
  }
...
ExecuteSqlQueryForSingleRow(Archive *fout, const char *query)
{
  ...
  ntups = PQntuples(res);
  if (ntups != 1) {
    fatal(ngettext("query returned %d row instead of one: %s", "query returned %d rows instead of one: %s", ntups), ntups, query);
  }
...
ExecuteSqlCommand(ArchiveHandle *AH, const char *qry, const char *desc)
{
  ...
  default:;
    /* trouble */
    warn_or_exit_horribly(AH, "%s: %sCommand was: %s", desc, PQerrorMessage(conn), qry);
    break;
  }
 ...
 *
 * The commands have been run together as one long string for compressibility,* and we are receiving them in bufferloads with arbitrary boundaries, so we
 ...
 * multiple SQL commands that might be split across bufferloads.  Fortunately,* that data won't contain anything complicated to lex either.
 */
static void
...
ExecuteSqlCommandBuf(Archive *AHX, const char *buf, size_t bufLen)
{
  ...
  if (AH->outputKind == OUTPUT_COPYDATA) {
    ...
    if (AH->pgCopyIn && PQputCopyData(AH->connection, buf, bufLen) <= 0) {
      fatal("error returned by PQputCopyData: %s", PQerrorMessage(AH->connection));
    }
  } else if (AH->outputKind == OUTPUT_OTHERDATA) {
    /*
     * Table data expressed as INSERT commands; or, in old dump files,* BLOB COMMENTS data (which is expressed as COMMENT ON commands).
     */
...
EndDBCopyMode(Archive *AHX, const char *tocEntryTag)
{
  ...
  if (AH->pgCopyIn) {
    ...
    if (PQputCopyEnd(AH->connection, NULL) <= 0) {
      fatal("error returned by PQputCopyEnd: %s", PQerrorMessage(AH->connection));
    }

    /* Check command status and return to normal libpq state */
    res = PQgetResult(AH->connection);
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
      warn_or_exit_horribly(AH, "COPY failed for table \"%s\": %s", tocEntryTag, PQerrorMessage(AH->connection));
    }
    PQclear(res);

    /* Do this to ensure we've pumped libpq back to idle state */
    if (PQgetResult(AH->connection) != NULL) {
      pg_log_warning("unexpected extra results during COPY of table \"%s\"", tocEntryTag);
    }
