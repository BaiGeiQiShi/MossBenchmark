...
typedef enum { POSTMASTER_READY, POSTMASTER_STILL_STARTING, POSTMASTER_FAILED } WaitPMResult;

typedef enum { NO_COMMAND = 0, INIT_COMMAND, START_COMMAND, STOP_COMMAND, RESTART_COMMAND, RELOAD_COMMAND, STATUS_COMMAND, PROMOTE_COMMAND, LOGROTATE_COMMAND, KILL_COMMAND, REGISTER_COMMAND, UNREGISTER_COMMAND, RUN_AS_SERVICE_COMMAND } CtlCommand;
...
static void
write_stderr(const char *fmt, ...) pg_attribute_printf(1, 2);
static void
do_advice(void);
static void
do_help(void);
...
static void WINAPI
pgwin32_ServiceMain(DWORD, LPTSTR *);
static void
...
write_eventlog(int level, const char *line)
{
  ...
  ReportEvent(evtHandle, level, 0, 0, /* All events are Id 0 */
      NULL, 1, 0, &line, NULL);
}
#endif
...
readfile(const char *path, int *numlines)
{
  ...
  *numlines = 0; /* in case of failure or empty file */
  ...
  /*
   * Count newlines. We expect there to be a newline after each full line,* including one at the end of file. If there isn't a newline at the end,* any characters after the last newline will be ignored.
   */
  nlines = 0;
  for (i = 0; i < len; i++) {
    if (buffer[i] == '\n') {
 ...
 * So the PID is usable for "is the postmaster still running" checks,* but cannot be compared directly to postmaster.pid.
 *
...
start_postmaster(void)
{
   ...
   */
  if (log_file != NULL) {
    cmd = psprintf("exec \"%s\" %s%s < \"%s\" >> \"%s\" 2>&1", exec_path, pgdata_opt, post_opts, DEVNULL, log_file);
  } else {
    cmd = psprintf("exec \"%s\" %s%s < \"%s\" 2>&1", exec_path, pgdata_opt, post_opts, DEVNULL);
  }
  ...
  if (log_file != NULL) {
    ...
    cmd = psprintf("CMD /C \"\"%s\" %s%s < \"%s\" >> \"%s\" 2>&1\"", exec_path, pgdata_opt, post_opts, DEVNULL, log_file);
  } else {
    cmd = psprintf("CMD /C \"\"%s\" %s%s < \"%s\" 2>&1\"", exec_path, pgdata_opt, post_opts, DEVNULL);
  }
 ...
 *
 * On Unix, pm_pid is the PID of the just-launched postmaster.  On Windows,* it may be the PID of an ancestor shell process, so we can't check the
 ...
 * Forwards the SIGINT to the postmaster process, asking it to shut down,* before terminating pg_ctl itself. This way, if the user hits CTRL-C while
...
do_start(void)
{
...
#ifndef WIN32
  {
    ...
    snprintf(env_var, sizeof(env_var), "PG_GRANDPARENT_PID=%d", (int)getppid());
    putenv(env_var);
  }
#endif
...
do_stop(void)
{
  ...
  if (!do_wait) {
     ...
     */
    if (shutdown_mode == SMART_MODE && stat(backup_file, &statbuf) == 0 && get_control_dbstate() != DB_IN_ARCHIVE_RECOVERY) {
...
do_logrotate(void)
{
  ...
  {
    pid = -pid;
    write_stderr(_("%s: cannot rotate log file; single-user server is running (PID: %ld)\n"),progname, pid);
    exit(1);
  }
...
postmaster_is_alive(pid_t pid)
{
   ...
   *
   * Don't believe that our own PID or parent shell's PID is the postmaster,* either.  (Windows hasn't got getppid(), though.)
   */
  if (pid == getpid()) {
    return false;
  }
...
pgwin32_CommandLine(bool registration)
{
  ...
  if (strlen(cmdPath) < 4 || pg_strcasecmp(cmdPath + strlen(cmdPath) - 4, ".exe") != 0) {
...
pgwin32_doRegister(void)
{
  ...
  if ((hService = CreateService(hSCM, register_servicename, register_servicename, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, pgctl_start_type, SERVICE_ERROR_NORMAL, pgwin32_CommandLine(true), NULL, NULL, "RPCSS\0", register_username, register_password)) == NULL) {
    CloseServiceHandle(hSCM);
    write_stderr(_("%s: could not register service \"%s\": error code %lu\n"), progname, register_servicename, (unsigned long)GetLastError());
    exit(1);
  }
  CloseServiceHandle(hService);
  CloseServiceHandle(hSCM);
 ...
 * a whole lot of API functions dynamically. Since we have to do this anyway,* also load the couple of functions that *do* exist in minwg headers but not
...
CreateRestrictedProcess(char *cmd, PROCESS_INFORMATION *processInfo, bool as_service)
{
  ...
  b = _CreateRestrictedToken(origToken, 0, sizeof(dropSids) / sizeof(dropSids[0]), dropSids, delPrivs->PrivilegeCount, delPrivs->Privileges, 0, NULL, &restrictedToken);
  ...
  if (_IsProcessInJob == NULL || _CreateJobObject == NULL || _SetInformationJobObject == NULL || _AssignProcessToJobObject == NULL || _QueryInformationJobObject == NULL) {
    ...
    if (_IsProcessInJob(processInfo->hProcess, NULL, &inJob)) {
      if (!inJob) {
        /*
         * Job objects are working, and the new process isn't in one,* so we can create one safely. If any problems show up when
        ...
        if (job) {
          ...
          uiRestrictions.UIRestrictionsClass = JOB_OBJECT_UILIMIT_DESKTOP | JOB_OBJECT_UILIMIT_DISPLAYSETTINGS | JOB_OBJECT_UILIMIT_EXITWINDOWS | JOB_OBJECT_UILIMIT_READCLIPBOARD | JOB_OBJECT_UILIMIT_SYSTEMPARAMETERS | JOB_OBJECT_UILIMIT_WRITECLIPBOARD;
...
/*
 * Get a list of privileges to delete from the access token. We delete all
 * privileges except SeLockMemoryPrivilege which is needed to use large pages,* and SeChangeNotifyPrivilege which is enabled by default in
...
GetPrivilegesToDelete(HANDLE hToken)
{
  ...
  if (!LookupPrivilegeValue(NULL, SE_LOCK_MEMORY_NAME, &luidLockPages) || !LookupPrivilegeValue(NULL, SE_CHANGE_NOTIFY_NAME, &luidChangeNotify)) {
    write_stderr(_("%s: could not get LUIDs for privileges: error code %lu\n"), progname, (unsigned long)GetLastError());
    return NULL;
  }

  if (!GetTokenInformation(hToken, TokenPrivileges, NULL, 0, &length) && GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
  ...
  for (i = 0; i < tokenPrivs->PrivilegeCount; i++) {
    if (memcmp(&tokenPrivs->Privileges[i].Luid, &luidLockPages, sizeof(LUID)) == 0 || memcmp(&tokenPrivs->Privileges[i].Luid, &luidChangeNotify, sizeof(LUID)) == 0) {
...
adjust_data_dir(void)
{
  ...
  cmd = psprintf("\"%s\" -C data_directory %s%s", my_exec_path, pgdata_opt ? pgdata_opt : "", post_opts ? post_opts : "");
...
{
  static struct option long_options[] = {{"help", no_argument, NULL, '?'}, {"version", no_argument, NULL, 'V'}, {"log", required_argument, NULL, 'l'}, {"mode", required_argument, NULL, 'm'}, {"pgdata", required_argument, NULL, 'D'}, {"options", required_argument, NULL, 'o'}, {"silent", no_argument, NULL, 's'}, {"timeout", required_argument, NULL, 't'}, {"core-files", no_argument, NULL, 'c'}, {"wait", no_argument, NULL, 'w'}, {"no-wait", no_argument, NULL, 'W'}, {NULL, 0, NULL, 0}};
  ...
  while (optind < argc) {
    ...
    if (optind < argc) {
      ...
      if (strcmp(argv[optind], "init") == 0 || strcmp(argv[optind], "initdb") == 0) {
