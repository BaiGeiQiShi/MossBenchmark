...
PLy_add_exceptions(PyObject *plpy)
{
  ...
  PLy_spi_exceptions = hash_create("PL/Python SPI exceptions", 256, &hash_ctl, HASH_ELEM | HASH_BLOBS);
...
PLy_generate_spi_exceptions(PyObject *mod, PyObject *base)
{
  ...
  for (i = 0; exception_map[i].name != NULL; i++) {
    ...
    exc = PLy_create_exception(exception_map[i].name, base, dict, exception_map[i].classname, mod);
...
PLy_output(volatile int level, PyObject *self, PyObject *args, PyObject *kw)
{
  ...
  if (sqlstatestr != NULL) {
    ...
    sqlstate = MAKE_SQLSTATE(sqlstatestr[0], sqlstatestr[1], sqlstatestr[2], sqlstatestr[3], sqlstatestr[4]);
  }
  ...
  PG_TRY();
  {
    ...
    ereport(level, ((sqlstate != 0) ? errcode(sqlstate) : 0, (message != NULL) ? errmsg_internal("%s", message) : 0, (detail != NULL) ? errdetail_internal("%s", detail) : 0, (hint != NULL) ? errhint("%s", hint) : 0, (column_name != NULL) ? err_generic_string(PG_DIAG_COLUMN_NAME, column_name) : 0, (constraint_name != NULL) ? err_generic_string(PG_DIAG_CONSTRAINT_NAME, constraint_name) : 0, (datatype_name != NULL) ? err_generic_string(PG_DIAG_DATATYPE_NAME, datatype_name) : 0, (table_name != NULL) ? err_generic_string(PG_DIAG_TABLE_NAME, table_name) : 0, (schema_name != NULL) ? err_generic_string(PG_DIAG_SCHEMA_NAME, schema_name) : 0));
  }
  PG_CATCH();
  {
    ErrorData *edata;
