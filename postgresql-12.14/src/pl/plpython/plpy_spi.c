...
PLy_spi_prepare(PyObject *self, PyObject *args)
{
  ...
  plan->mcxt = AllocSetContextCreate(TopMemoryContext, "PL/Python plan context", ALLOCSET_DEFAULT_SIZES);
  ...
  PG_TRY();
  {
    ...
    for (i = 0; i < nargs; i++) {
      ...
      plan->types[i] = typeId;
      PLy_output_setup_func(&plan->args[i], plan->mcxt, typeId, typmod, exec_ctx->curr_proc);
    }
