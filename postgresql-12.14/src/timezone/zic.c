...
extern int
link(const char *target, const char *linkname);
#ifndef AT_SYMLINK_FOLLOW
#define linkat(targetdir, target, linknamedir, linkname, flag) (itssymlink(target) ? (errno = ENOTSUP, -1) : link(target, linkname))
#endif

static void
memory_exhausted(const char *msg) pg_attribute_noreturn();
static void
verror(const char *string, va_list args) pg_attribute_printf(1, 0);
static void
error(const char *string, ...) pg_attribute_printf(1, 2);
static void
warning(const char *string, ...) pg_attribute_printf(1, 2);
static void
usage(FILE *stream, int status) pg_attribute_noreturn();
static void
addtt(zic_t starttime, int type);
static int
addtype(zic_t, char const *, bool, bool, bool);
static void
leapadd(zic_t, int, int);
static void
adjleap(void);
static void
associate(void);
static void
dolink(const char *, const char *, bool);
static char **
getfields(char *buf);
static zic_t
gethms(const char *string, const char *errstring);
static zic_t
getsave(char *, bool *);
static void
inexpires(char **, int);
...
static bool
inzsub(char **, int, bool);
static bool
itsdir(char const *);
static bool
itssymlink(char const *);
static bool
is_alpha(char a);
static char
lowerit(char);
static void
mkdirs(char const *, bool);
static void
newabbr(const char *abbr);
static zic_t
oadd(zic_t t1, zic_t t2);
static void
outzone(const struct zone *zp, ptrdiff_t ntzones);
static zic_t
rpytime(const struct rule *rp, zic_t wantedy);
static void
rulesub(struct rule *rp, const char *loyearp, const char *hiyearp, const char *typep, const char *monthp, const char *dayp, const char *timep);
static zic_t
...
static struct lookup const zi_line_codes[] = {{"Rule", LC_RULE}, {"Zone", LC_ZONE}, {"Link", LC_LINK}, {NULL, 0}};
static struct lookup const leap_line_codes[] = {{"Leap", LC_LEAP}, {"Expires", LC_EXPIRES}, {NULL, 0}};

static struct lookup const mon_names[] = {{"January", TM_JANUARY}, {"February", TM_FEBRUARY}, {"March", TM_MARCH}, {"April", TM_APRIL}, {"May", TM_MAY}, {"June", TM_JUNE}, {"July", TM_JULY}, {"August", TM_AUGUST}, {"September", TM_SEPTEMBER}, {"October", TM_OCTOBER}, {"November", TM_NOVEMBER}, {"December", TM_DECEMBER}, {NULL, 0}};

static struct lookup const wday_names[] = {{"Sunday", TM_SUNDAY}, {"Monday", TM_MONDAY}, {"Tuesday", TM_TUESDAY}, {"Wednesday", TM_WEDNESDAY}, {"Thursday", TM_THURSDAY}, {"Friday", TM_FRIDAY}, {"Saturday", TM_SATURDAY}, {NULL, 0}};

static struct lookup const lasts[] = {{"last-Sunday", TM_SUNDAY}, {"last-Monday", TM_MONDAY}, {"last-Tuesday", TM_TUESDAY}, {"last-Wednesday", TM_WEDNESDAY}, {"last-Thursday", TM_THURSDAY}, {"last-Friday", TM_FRIDAY}, {"last-Saturday", TM_SATURDAY}, {NULL, 0}};
...
{
  char const *e = (ferror(stream) ? _("I/O error") : fclose(stream) != 0 ? strerror(errno) : NULL);

  if (e) {
    fprintf(stderr, "%s: %s%s%s%s%s\n", progname, dir ? dir : "", dir ? "/" : "", name ? name : "", name ? ": " : "", e);
    exit(EXIT_FAILURE);
  }
}
...
{
  fprintf(stream,_("%s: usage is %s [ --version ] [ --help ] [ -v ] [ -P ] \\\n\t[ -b {slim|fat} ] [ -d directory ] [ -l localtime ] [ -L leapseconds ] \\\n\t[ -p posixrules ] [ -r '[@lo][/@hi]' ] [ -t localtime-link ] \\\n\t[ filename ... ]\n\nReport bugs to %s.\n"),progname, progname, PACKAGE_BUGREPORT);
...
main(int argc, char **argv)
{
  ...
  for (i = 0; i < nlinks; ++i) {
    ...
    if (noise) {
      for (j = 0; j < nlinks; ++j) {
        if (strcmp(links[i].l_linkname, links[j].l_target) == 0) {
...
componentcheck(char const *name, char const *component, char const *component_end)
{
  ...
  if (component_len == 0) {
    if (!*name) {
      ...
      error(_(component == name ? "file name '%s' begins with '/'" : *component_end ? "file name '%s' contains '//'" : "file name '%s' ends with '/'"), name);
    }
    return false;
  }
...
namecheck(const char *name)
{
  ...
  for (cp = name; *cp; cp++) {
    ...
    if (noise && !strchr(benign, c)) {
      warning((strchr(printable_and_not_benign, c) ? _("file name '%s' contains byte '%c'") : _("file name '%s' contains byte '\\%o'")), name, c);
    }
    if (c == '/') {
  ...
  for (num = 1;; ++num) {
    ...
    *cp = '\0';
    ...
    if (nfields == 0) {
      ...
      struct lookup const *line_codes = name == leapsec ? leap_line_codes : zi_line_codes;
...
gethms(char const *string, char const *errstring)
{
  ...
  return oadd(sign * (zic_t)hh * SECSPERHOUR, sign * (mm * SECSPERMIN + ss));
}
...
inzsub(char **fields, int nfields, bool iscont)
{
  ...
  if (hasuntil) {
    ...
    rulesub(&z.z_untilrule, fields[i_untilyear], "only", "", (nfields > i_untilmonth) ? fields[i_untilmonth] : "Jan", (nfields > i_untilday) ? fields[i_untilday] : "1", (nfields > i_untiltime) ? fields[i_untiltime] : "0");
    z.z_untiltime = rpytime(&z.z_untilrule, z.z_untilrule.r_loyear);
    if (iscont && nzones > 0 && z.z_untiltime > min_time && z.z_untiltime < max_time && zones[nzones - 1].z_untiltime > min_time && zones[nzones - 1].z_untiltime < max_time && zones[nzones - 1].z_untiltime >= z.z_untiltime) {
...
rulesub(struct rule *rp, const char *loyearp, const char *hiyearp, const char *typep, const char *monthp, const char *dayp, const char *timep)
{
  ...
  if ((lp = byword(dp, lasts)) != NULL) {
    ...
    if (sscanf(ep, "%d%c", &rp->r_dayofmonth, &xs) != 1 || rp->r_dayofmonth <= 0 || (rp->r_dayofmonth > len_months[1][rp->r_month])) {
      error(_("invalid day of month"));
      free(dp);
      return;
    }
...
limitrange(struct timerange r, zic_t lo, zic_t hi, zic_t const *ats, unsigned char const *types)
{
...
writezone(const char *const name, const char *const string, char version, int defaulttype)
{
   ...
   */
  if (WORK_AROUND_QTBUG_53071 && timecnt != 0 && want_bloat() && ats[timecnt - 1] < y2038_boundary - 1 && strchr(string, '<')) {
  ...
  for (pass = 1; pass <= 2; ++pass) {
    ...
    if (pass == 1) {
       ...
       * 32-bit timestamps.
       */
      thisdefaulttype = (lo_time <= PG_INT32_MIN ? range64.defaulttype : range32.defaulttype);
...
static size_t
doabbr(char *abbr, struct zone const *zp, char const *letters, bool isdst, zic_t save, bool doquotes)
{
  char *cp;
  char *slashp;
  size_t len;
...
outzone(const struct zone *zpfirst, ptrdiff_t zonecount)
{
  ...
  if (do_extend) {
     ...
     * specifies that its rules always have and always will be in effect,* we only need one cycle to define the zone.
     */
    if (prodstic) {
      min_year = 1900;
    ...
    *startbuf = '\0';
    ...
    if (zp->z_nrules == 0) {
      ...
      for (year = min_year; year <= max_year; ++year) {
        ...
        for (;;) {
          ...
          if (!want_bloat() && !useuntil && !do_extend && prevrp && rp->r_hiyear == ZIC_MAX && prevrp->r_hiyear == ZIC_MAX) {
            break;
          }
    ...
    if (usestart) {
      ...
      if (*startbuf == '\0') {
        ...
        type = addtype(startoff, startbuf, isdst, startttisstd, startttisut);
...
addtype(zic_t utoff, char const *abbr, bool isdst, bool ttisstd, bool ttisut)
{
  ...
  if (j == charcnt) {
    ...
    for (i = 0; i < typecnt; i++) {
      if (utoff == utoffs[i] && isdst == isdsts[i] && j == desigidx[i] && ttisstd == ttisstds[i] && ttisut == ttisuts[i]) {
        return i;
      }
    }
  }
...
/*
 * Given a rule, and a year, compute the date (in seconds since January 1,* 1970, 00:00 LOCAL time) in that year that the rule refers to.
 */
...
/* Ensure that the directories of ARGNAME exist, by making any missing
   ones.  If ANCESTORS, do this only for ARGNAME's ancestors; otherwise,do it for ARGNAME too.  Exit with failure if there is trouble.
...
mkdirs(char const *argname, bool ancestors)
{
  ...
  /*
   * On MS-Windows systems, do not worry about drive letters or backslashes,* as this should suffice in practice.  Time zone names do not use drive
  ...
  while (cp && ((cp = strchr(cp, '/')) || !ancestors)) {
    ...
    if (mkdir(name, MKDIR_UMASK) != 0) {
      ...
      if (err != EEXIST && !itsdir(name)) {
        error(_("%s: Cannot create directory %s: %s"), progname, name, strerror(err));
        exit(EXIT_FAILURE);
      }
    }
    if (cp) {
