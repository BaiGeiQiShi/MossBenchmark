...
def DEBOP(_rid):
    ...
    try:
        subprocess.check_output(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -m {debop_samplenum} -i {iternum} -t {CURRDIR}/moss-out.{_rid} -a {alpha} -e {beta} -k {k} -P {CURRDIR}/programlist -s ./test.sh --build -- make > log/{_rid}.txt",shell=True)
