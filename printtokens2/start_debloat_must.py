...
def DEBOP(_rid):
    try:
        best=subprocess.check_output(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -M Cov_info.txt -T COVBLOATBEST.c -I BaseInputs.txt -m {debop_samplenum} -i {iternum} -t moss-out.{_rid} -a {alpha} -e {beta} -k {k} -s ./test.sh {PROGNAME}.c > log/{_rid}.txt",shell=True)
...
def BASICBLOCK(_rid):
    try:
        best=subprocess.check_output(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -B -m {debop_samplenum} -i {iternum} -I BaseInputs.txt -t moss-out.{_rid} -a {alpha} -e {beta} -k {k} -s ./test.sh {PROGNAME}.c > log/{_rid}.txt",shell=True)
        os.system(f"chattr -i {CURRDIR}/*")
    except subprocess.CalledProcessError as e:
        os.system(f"chattr -i {CURRDIR}/*")
        if(e.returncode==137):pass
...
def COVBLOAT(_rid):
    try:
        ...
        subprocess.run(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -F ./Cov_info.txt -T TMCMCBEST.c -m {debop_samplenum} -I BaseInputs.txt -i {iternum} -t moss-out.{_rid} -a {alpha} -e {beta} -k {k} -s ./test.sh {PROGNAME}.c > log/{_rid}.txt",shell=True)
        os.system(f"chattr -i {CURRDIR}/*")
    except subprocess.CalledProcessError as e:
        os.system(f"chattr -i {CURRDIR}/*")
        if(e.returncode==137):pass
