...
def DEBOP(_rid):
    try:
        best=subprocess.check_output(f"timeout -s 9 {TIMEOUT} {DEBOP_BIN} -m {debop_samplenum} -i {iternum} -t moss-out.{_rid} -a {alpha} -e {beta} -k {k} -s ./test.sh {PROGNAME}.c > log/{_rid}.txt",shell=True)
